...
When you create a website, Azure provides a friendly subdomain on the  azurewebsites.net  chinacloudsites.cn  domain so your users can access your website using a URL like  http://&lt;mysite>.azurewebsites.net  http://&lt;mysite>.chinacloudsites.cn . However, if you configure your websites for Shared or Standard mode, you can map your website to your own domain name.
...
A CNAME record maps a *specific* domain, such as **contoso.com** or **www.contoso.com**, to a canonical domain name. In this case, the canonical domain name is the either the  **&lt;myapp>.azurewebsites.net**  **&lt;myapp>.chinacloudsites.cn**  domain name of your Azure website or the **&lt;myapp>.trafficmgr.com** domain name of your Traffic Manager profile. Once created, the CNAME creates an alias for the  **&lt;myapp>.azurewebsites.net**  **&lt;myapp>.chinacloudsites.cn**  or **&lt;myapp>.trafficmgr.com** domain name. The CNAME entry will resolve to the IP address of your  **&lt;myapp>.azurewebsites.net**  **&lt;myapp>.chinacloudsites.cn**  or **&lt;myapp>.trafficmgr.com** domain name automatically, so if the IP address of the website changes, you do not have to take any action.
...
1. In your browser, open the  [Management  [Classic Management  Portal][portal].
...
5. On the **Manage custom domains** dialog, note awverify domain name at the end of the text at the top of the dialog. It should be  **awverify.mysite.azurewebsites.net**  **awverify.mysite.chinacloudsites.cn**  where **mysite** is the name of your website. Copy this, as it is the domain name used when creating the verification CNAME record.
...
7. Next, create a CNAME record that has an alias of **awverify**, and a canonical domain of  **awverify.mysite.azurewebsites.net**  **awverify.mysite.chinacloudsites.cn**  that you obtained earlier.
