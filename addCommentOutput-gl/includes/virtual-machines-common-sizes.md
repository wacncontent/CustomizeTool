...
The standard sizes consist of several series: A,  D, DS  D and DS  , F, Fs, G, and GS . Considerations for some of these sizes include:
...
*   DS-series and DSv2-series VMs can use Premium Storage, which provides high-performance, low-latency storage for I/O intensive workloads. These VMs use solid-state drives (SSDs) to host a virtual machine's disks and also provide a local SSD disk cache. Premium Storage is available in certain regions. For details, see [Premium Storage: High-performance storage for Azure virtual machine workloads](/documentation/articles/storage-premium-storage/).
	...
	>[AZURE.NOTE] Currently, DS-series can not be created directly through Classic Management Portal, because Premium Storage is not yet manageable by the Classic Management Portal. However, you can create Premium Storage with Azure PowerShell, assign that storage to be the default storage when creating a Virtual Machine, and then, you can get a DS Virtual Machine. For details, see [Premium Storage: High-performance storage for Azure virtual machine workloads](/documentation/articles/storage-premium-storage/).

