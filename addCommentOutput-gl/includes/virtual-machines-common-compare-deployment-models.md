...
|DNS Name| A cloud service would get an implicit globally unique DNS Name. For example:  `mycoffeeshop.cloudapp.net`  `mycoffeeshop.chinacloudapp.cn` . | DNS Names are optional parameters that can be specified on a Public IP Address resource. The FQDN will be in the following format -  `<domainlabel>.<region>.cloudapp.azure.com`  `<domainlabel>.<region>.chinacloudapp.cn` . |
...
You can find the detailed REST API documentation for the Compute, Network & Storage Resource Providers  [here](https://msdn.microsoft.com/library/azure/dn790568.aspx)  [here](https://msdn.microsoft.com/zh-cn/library/azure/dn790568.aspx) .
