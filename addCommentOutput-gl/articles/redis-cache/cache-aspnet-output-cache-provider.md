...
Configure the attributes with the values from your cache blade in the  Microsoft  Azure portal, and configure the other values as desired. For instructions on accessing your cache properties, see [Configure Redis cache settings](/documentation/articles/cache-configure/#configure-redis-cache-settings).
...
In this example the cached page data will remain in the cache for 60 seconds, and a different version of the page will be cached for each parameter combination. For more information about the OutputCache directive, see  [@OutputCache](http://go.microsoft.com/fwlink/?linkid=320837)  [@OutputCache](https://msdn.microsoft.com/zh-cn/library/hdxfb6cy(v=vs.100).aspx) .
