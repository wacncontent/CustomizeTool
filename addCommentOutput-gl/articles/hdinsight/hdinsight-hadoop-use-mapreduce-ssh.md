...
Connect to the fully qualified domain name (FQDN) of your HDInsight cluster by using the SSH command. The FQDN will be the name you gave the cluster, followed by  **.azurehdinsight.net**  **.azurehdinsight.cn** . For example, the following would connect to a cluster named **myhdinsight**:
