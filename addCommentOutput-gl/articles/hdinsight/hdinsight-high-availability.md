...
For the SDK, the story is similar. The creation and provisioning of a cluster by using the SDK is documented in [Using HDInsight .NET  SDK](/documentation/articles/hdinsight-provision-clusters/#sdk)  SDK](/documentation/articles/hdinsight-provision-clusters-v1/#sdk) . The configuration of an extra-large head node requires the addition of the `HeadNodeSize = NodeVMSize.ExtraLarge` parameter to the `ClusterCreateParameters()` method used in this code.
