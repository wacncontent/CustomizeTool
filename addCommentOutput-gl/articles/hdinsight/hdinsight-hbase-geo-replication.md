...
2. Set the variables at the beginning of the script, and then run the script.
		...
		$vNetName =  "Contoso-VNet-EU"  "Contoso-VNet-CN"   # This name must match your Europe virtual network name.
...
- Use the same script with the following values:
		...
		$vNetName =  "Contoso-VNet-US"  "Contoso-VNet-CE"   # This name must match your Europe virtual network name.
...
5.	Enter the following information:
	- **DNS Domain**: enter the DNS suffix of the Contoso-HBase-US. For example:  Contoso-HBase-US.f5.internal.cloudapp.net  Contoso-HBase-US.f5.internal.chinacloudapp.cn .
	- **IP addresses of the master servers**: enter 10.2.0.4, which is the  Contoso-DNS-US's  Contoso-DNS-CE's  IP address. Please verify the IP. Your DNS server can have a different IP address.
6.	Press **ENTER**, and then click **OK**.  Now you will be able to resolve the  Contoso-DNS-US's  Contoso-DNS-CE's  IP address from  Contoso-DNS-EU  Contoso-DNS-CN .
7.	Repeat the steps to add a DNS conditional forwarder to the DNS service on the  Contoso-DNS-US  Contoso-DNS-CE  virtual machine with the following values:
	- **DNS Domain**: enter the DNS suffix of the Contoso-HBase-EU. 
	- **IP addresses of the master servers**: enter 10.2.0.4, which is the  Contoso-DNS-EU's  Contoso-DNS-CN's  IP address.
