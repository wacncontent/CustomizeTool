...
Each of these methods will require the public key. For complete information on creating a Linux-based HDInsight cluster, see [Provision Linux-based HDInsight  clusters](/documentation/articles/hdinsight-hadoop-provision-linux-clusters/)  clusters](/documentation/articles/hdinsight-provision-clusters-v1/) .
...
For more information on using this command, see [Provision Hadoop Linux clusters in HDInsight using custom  options](/documentation/articles/hdinsight-hadoop-provision-linux-clusters/)  options](/documentation/articles/hdinsight-provision-clusters-v1/) .
...
3. In **Category**, select **Session**. From the **Basic options for your PuTTY session** screen, enter the SSH address of your HDInsight server in the **Host name (or IP address)** field. There are two possible SSH addresses you may use when connecting to a cluster:
    ...
    * __Head node address__: To connect to the head node ofthe cluster, use your cluster name, then  **-ssh.azurehdinsight.net**  **-ssh.azurehdinsight.cn** . For example,  **mycluster-ssh.azurehdinsight.net**  **mycluster-ssh.azurehdinsight.cn** .
    
    * __Edge node address__: If you are connecting to an R Server on HDInsight cluster, you can connect to the R Server edge node using the address  __RServer.CLUSTERNAME.ssh.azurehdinsight.net__  __RServer.CLUSTERNAME.ssh.azurehdinsight.cn__ , where CLUSTERNAME is the name of your cluster. For example,  __RServer.mycluster.ssh.azurehdinsight.net__  __RServer.mycluster.ssh.azurehdinsight.cn__ .
...
> [AZURE.NOTE] The above steps assume you are using port 22, which will connect to head node 0 on the HDInsight cluster. If you use port 23, you will connect to head node 1. For more information on the head nodes, see [Availability and reliability of Hadoop clusters in  HDInsight](/documentation/articles/hdinsight-high-availability-linux/)  HDInsight](/documentation/articles/hdinsight-high-availability/) .
...
8. Once you have a list of the worker nodes you want to connect to, use the following command from the PuTTY session to open a connection to a worker node:
    ...
    Replace *USERNAME* with your SSH user name and *FQDN* with the FQDN for the worker node. For example,  `workernode0.workernode-0-e2f35e63355b4f15a31c460b6d4e1230.j1.internal.cloudapp.net`  `workernode0.workernode-0-e2f35e63355b4f15a31c460b6d4e1230.j1.internal.chinacloudapp.cn` .
