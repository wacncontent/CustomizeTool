...
- **A Linux-based Storm on HDInsight cluster**: see [Get started with Apache Storm on  HDInsight](/documentation/articles/hdinsight-apache-storm-tutorial-get-started-linux/)  HDInsight](/documentation/articles/hdinsight-apache-storm-tutorial-get-started/)  for steps on creating a cluster
...
The Storm UI provides a web interface for working with running topologies, and is included on your HDInsight cluster. To view the Storm UI, use a web browser to open  __https://CLUSTERNAME.azurehdinsight.net/stormui__  __https://CLUSTERNAME.azurehdinsight.cn/stormui__ , where __CLUSTERNAME__ is the name of your cluster.
...
 .azurehdinsight.net/api/v1/clusters/CLUSTERNAME/services/STORM/components/STORM_UI_SERVER"`  .azurehdinsight.cn/api/v1/clusters/CLUSTERNAME/services/STORM/components/STORM_UI_SERVER"`  to retrieve information about the node that the Storm UI and REST API are running on. Replace __PASSWORD__ with the admin password for the cluster. Replace __CLUSTERNAME__ with the cluster name. In the response, the "host_name" entry contains the FQDN of the node.
