...

    ...
    * **-G**: Indicates that this is a GET request
    ...
    The beginning of the URI,  **https://CLUSTERNAME.azurehdinsight.net/templeton/v1**  **https://CLUSTERNAME.azurehdinsight.cn/templeton/v1** , is the same for all requests.

2. To submit a MapReduce job, use the following command:

		curl -u USERNAME:PASSWORD -d user.name=USERNAME -d jar=wasb:///example/jars/hadoop-mapreduce-examples.jar -d class=wordcount -d arg=wasb:///example/data/gutenberg/davinci.txt -d arg=wasb:///example/data/CurlOut  https://CLUSTERNAME.azurehdinsight.net/templeton/v1/mapreduce/jar  https://CLUSTERNAME.azurehdinsight.cn/templeton/v1/mapreduce/jar 
...
3. To check the status of the job, use the following command. Replace the **JOBID** with the value returned in the previous step. For example, if the return value was `{"id":"job_1415651640909_0026"}`, then the JOBID would be `job_1415651640909_0026`.
        ...
        curl -G -u USERNAME:PASSWORD -d user.name=USERNAME  https://CLUSTERNAME.azurehdinsight.net/templeton/v1/jobs/JOBID  https://CLUSTERNAME.azurehdinsight.cn/templeton/v1/jobs/JOBID  | jq .status.state
