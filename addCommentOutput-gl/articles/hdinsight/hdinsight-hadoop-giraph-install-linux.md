...
1. Use the following to create a new file named __tiny_graph.txt__:
	...
	![tiny_graph.txt drawn as circles with lines of varying distance  between](./media/hdinsight-hadoop-giraph-install-linux/giraph-graph.png)  between](./media/hdinsight-hadoop-giraph-install-v1/giraph-graph.png) 
...
5. Once the job has finished, the results will be stored in the __wasb:///example/out/shotestpaths__ directory. The files created will begin with __part-m-__ and end with a number indicating the first, second, etc. file. Use the following to view the output:
	...
	![Drawing of objects as circles with shortest paths drawn  between](./media/hdinsight-hadoop-giraph-install-linux/giraph-graph-out.png)  between](./media/hdinsight-hadoop-giraph-install-v1/giraph-graph-out.png) 
...
- [Install Solr on HDInsight  clusters](/documentation/articles/hdinsight-hadoop-solr-install-linux/)  clusters](/documentation/articles/hdinsight-hadoop-solr-install-v1/) . Use cluster customization to install Solr on HDInsight Hadoop clusters. Solr allows you to perform powerful search operations on data stored.
