...
- __An HDInsight cluster__. See [Get started using Hadoop with Hive in HDInsight on  Linux](/documentation/articles/hdinsight-hadoop-linux-tutorial-get-started/)  Linux](/documentation/articles/hdinsight-hadoop-tutorial-get-started-windows-v1/)  for steps on creating a new Linux-based HDInsight cluster.
...

	...
	> [AZURE.NOTE] If you use a password to authenticate your SSH login, you will be prompted for the password. If you used a public key, you may need to use the `-i` parameter and specify the path to the matching private key. For example `scp -i ~/.ssh/id_rsa FILENAME.csv  USERNAME@CLUSTERNAME-ssh.azurehdinsight.net:`  USERNAME@CLUSTERNAME-ssh.azurehdinsight.cn:` .
...
1. From your develoment environment where Azure CLI is installed, use the following command to create a new Azure SQL Database:
	...
	For exmaple, `azure sql server create admin password  "West US"`  "China North"` .
...
> [AZURE.IMPORTANT] Note the server name returned by this command. This is the short name of the SQL Database server that was created. The fully qualified domain name (FQDN) is  `<shortname>.database.windows.net`  `<shortname>.database.chinacloudapi.cn` .
...
4. Once FreeTDS has been installed, use the following command to connect to the SQL Database server you created previously:
        ...
        TDSVER=8.0 tsql -H  <serverName>.database.windows.net  <serverName>.database.chinacloudapi.cn  -U <adminLogin> -P <adminPassword> -p 1433 -D sqooptest
...
2. Use the following command to verify that Sqoop can see your SQL Database:
		...
		sqoop list-databases --connect  jdbc:sqlserver://<serverName>.database.windows.net:1433  jdbc:sqlserver://<serverName>.database.chinacloudapi.cn:1433  --username <adminLogin> --password <adminPassword>
...
3. Use the following command to export data from hivesampletable to the mobiledata table:
		...
		sqoop export --connect  'jdbc:sqlserver://<serverName>.database.windows.net:1433;database=sqooptest'  'jdbc:sqlserver://<serverName>.database.chinacloudapi.cn:1433;database=sqooptest'  --username <adminLogin> --password <adminPassword> --table 'delays' --export-dir 'wasb:///tutorials/flightdelays/output' --fields-terminated-by '\t' -m 1
