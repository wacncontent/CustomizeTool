...
1. From the [Azure Classic  Portal](https://manage.windowsazure.com)  Management Portal](https://manage.windowsazure.cn) , select **NEW | Service Bus | Event Hub | Custom Create**.
...
1. To deploy the dashboard to an Azure Website, follow the steps in [Build and deploy a Node.js website to Azure](/documentation/articles/web-sites-nodejs-develop-deploy-mac/). Note the URL of the website, which will be similar to  **mywebsite.azurewebsites.net**  **mywebsite.chinacloudsites.cn** .
