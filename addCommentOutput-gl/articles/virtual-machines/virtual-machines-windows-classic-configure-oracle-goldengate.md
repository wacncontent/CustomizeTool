...
- You have created two Virtual Machines (VMs) in Azure using Oracle Enterprise Edition on Windows Server. Make sure that the Virtual Machines are in the [same cloud service](/documentation/articles/virtual-machines-linux-load-balance/) and in the same [Virtual Network](/documentation/services/networking/) to ensure they can access each other over the persistent private IP address.

- You've set the Virtual Machine names as "MachineGG1" for Site A and "MachineGG2" for Site B at the Azure  classic  Classic   portal  Management Portal .
...
First, remote desktop to Site A and Site B via the Azure  classic  Classic   portal  Management Portal . Open up a Windows command prompt and create a home directory for Oracle GoldenGate setup files:
...
First, remote desktop to Site A and Site B via the Azure  classic  Classic   portal  Management Portal . Switch the database to archivelog mode using the SQL*Plus command window:
...
Remote desktop to Site A and Site B via the Azure  classic  Classic   portal  Management Portal . Open up GGSCI command interpreter window. Run the following commands on Site A:
