...
In the [Azure classic  portal](https://manage.windowsazure.com)  Management portal](https://manage.windowsazure.cn) , for VMs that were created by using the classic deployment model:
...
- The SSH service is not listening on TCP port 22. To test this, install a telnet client on your local computer and run "telnet  *cloudServiceName*.cloudapp.net  *cloudServiceName*.chinacloudapp.cn  22". This will determine if the virtual machine allows inbound and outbound communication to the SSH endpoint.
