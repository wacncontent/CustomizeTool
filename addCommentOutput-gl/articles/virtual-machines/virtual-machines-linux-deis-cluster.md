...
9. Modify the **publicDomainName** parameter. This will become part of the DNS name associated with the load balancer public IP. The final FQDN will have the format of _[value of this  parameter]_._[region]_.cloudapp.azure.com  parameter]_._[region]_.chinacloudapp.cn . For example, if you specify the name as deishbai32, and the resource group is deployed to the  West US  China North  region, then the final FQDN to your load balancer will be  deishbai32.westus.cloudapp.azure.com  deishbai32.chinanorth.chinacloudapp.cn .
...

  ...
  or Azure CLI:
        ...
        ./deploy-deis.sh -n "[resource group name]" -l  "West US"  "China North"  -f ./azuredeploy.json -e ./azuredeploy-parameters.json
        -c ./cloud-config.yaml  
