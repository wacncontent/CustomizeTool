...
1. For external load balancing, you must obtain the public virtual IP address of the cloud service that contains your replicas. Log into the Azure  classic  Classic   portal  Management Portal . Navigate to the cloud service that contains your availability group VM. Open the **Dashboard** view.
...
In order to access the listener from outside the virtual network, you must be using external/public load balancing (described in this topic) rather than ILB, which is only accesible within the same VNet. In the connection string, you specify the cloud service name. For example, if you had a cloud service with the name *mycloudservice*, the sqlcmd statement would be as follows:
	...
	sqlcmd -S  "mycloudservice.cloudapp.net,<EndpointPort>"  "mycloudservice.chinacloudapp.cn,<EndpointPort>"  -d "<DatabaseName>" -U "<LoginId>" -P "<Password>"  -Q "select @@servername, db_name()" -l 15
