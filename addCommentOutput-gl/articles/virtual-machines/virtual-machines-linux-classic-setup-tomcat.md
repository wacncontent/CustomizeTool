...
In the [Azure  portal](https://portal.azure.com/)  portal Preview](https://portal.azure.cn/) , click **New** in the task bar to create an image, choosing the Linux image based on your needs. The following example uses the Ubuntu 14.04 image.
![][3]

For **Host Name** specify the name for the URL that you and Internet clients will use to access this virtual machine. Define the last part of the DNS name, for example tomcatdemo, and Azure will generate the URL as  tomcatdemo.cloudapp.net  tomcatdemo.chinacloudapp.cn .
...
1.	In the Azure portal  Preview , click **Browse** -> **Virtual Machine**, and then click the virtual machine that you created.
![][5]
...
3.	Click **Add**.  
	1.	For the **endpoint**, type a name for the endpoint in Endpoint, and then type 80 in **Public Port**.  
		...
		If you set it to 80, don't need to include the port number in the URL that allows you to access tomcat. For example,  http://tomcatdemo.cloudapp.net  http://tomcatdemo.chinacloudapp.cn .

		If you set it to another value, such as 81, you need to add the port number to the URL to access tomcat. For example,   http://tomcatdemo.cloudapp.net:81/  http://tomcatdemo.chinacloudapp.cn:81/ .
...
First, get the DNS name of your virtual machine from the Azure portal  Preview . **Click Browse** -> **Virtual machines** -> the name of your virtual machine -> **Properties**, and then look in the **Domain Name** field of the **Properties** tile.
...
To check if tomcat7 is successfully installed, browse to your tomcat server's DNS name  (http://tomcatexample.cloudapp.net/  (http://tomcatexample.chinacloudapp.cn/  is the example URL in this article). If you see a page like the following, you have tomcat7 installed correct.
![][16]
...
Open your browser, and enter the URL **http://<your tomcat server DNS name>/manager/html**. For the example in this article, the URL is  http://tomcatexample.cloudapp.net/manager/html  http://tomcatexample.chinacloudapp.cn/manager/html .
