...
6. In the next **Virtual machine configuration** dialog box:
    ...
    2. The value for **Cloud service DNS name** must be unique across  cloudapp.net  chinacloudapp.cn . If needed, modify this value so that Azure indicates it is unique.
    2. Specify a region, affinity group, or virtual network. For the purposes of this tutorial, specify a region such as  **West US**  **China North** .
...
At this point, your Tomcat website should be viewable from an external browser by using a URL of the form  **http://*your\_DNS\_name*.cloudapp.net**  **http://*your\_DNS\_name*.chinacloudapp.cn** , where ***your\_DNS\_name*** is the DNS name you specified when you created the virtual machine.
