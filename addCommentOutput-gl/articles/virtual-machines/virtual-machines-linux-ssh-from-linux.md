...
This topic describes how to use **ssh-keygen** and **openssl** on Linux and Mac to create and use **ssh-rsa** format and **.pem** format files to secure communication with Azure VMs based on Linux. Creating Linux-based Azure Virtual Machines using the Resource Manager deployment model is recommended for new deployments and takes an *ssh-rsa* type public key file or string (depending on the deployment client). The [Azure  portal](https://portal.azure.com)  portal Preview](https://portal.azure.cn)  currently accepts only the **ssh-rsa** format strings, whether for classic or Resource Manager deployments.
...
A basic ssh setup for Azure includes an **ssh-rsa** public and private key pair of 2048 bits (by default, **ssh-keygen** stores these files as **~/.ssh/id_rsa** and **~/.ssh/id-rsa.pub** unless you change the defaults) as well as a `.pem` file generated from the **id_rsa** private key file for use with the classic deployment model of the  classic  Classic   portal  Management Portal .

Here are the deployment scenarios, and the types of files you use in each:

1. **ssh-rsa** keys are required for any deployment using the [Azure  portal](https://portal.azure.com)  portal Preview](https://portal.azure.cn) , regardless of the deployment model.
2. .pem file are required to create VMs using the  [classic  [Classic   portal](https://manage.windowsazure.com)  Management Portal](https://manage.windowsazure.cn) . .pem files are also supported in classic deployments that use the [Azure CLI](/documentation/articles/xplat-cli-install/).
...
3. Create a .pem file from your `~/.ssh/id_rsa` file to enable you to work with the  classic  Classic   portal  Management Portal . Use the **openssl** as follows:
...
You can use ssh-rsa (`.pub`) keys for all new work, especially with the Resource Manager deployment model and the  preview  portal  Preview ; you may need to create a `.pem` file from your keys if you need to use the  classic  Classic   portal  Management Portal .
...

	...
	data:    ubuntuOSVersion        String   14.04.4-LTS  14.04.3-LTS 
	info:    group deployment create command OK


### Example: Creating a VM with a .pem file

You can then use the .pem file with either the  classic  Classic   portal  Management Portal  or with the classic deployment mode (`azure config mode asm`) and `azure vm create`, as in the following example:
...

	...
	data:    OSDisk mediaLink  "https://portalvhds4blttsxgjj1rf.blob.core.windows.net/vhd-store/testpemasm-2747c9c432b043ff.vhd"  "https://portalvhds4blttsxgjj1rf.blob.core.chinacloudapi.cn/vhd-store/testpemasm-2747c9c432b043ff.vhd" 
	data:    OSDisk sourceImageName  "b39f27a8b8c64d52b05eac6a62ebad85__Ubuntu-14_04_3-LTS-amd64-server-20150908-en-us-30GB"  "b549f4301d0b4295b8e76ceb65df47d4__Ubuntu-14_04_3-LTS-amd64-server-20150908-en-us-30GB" 
...

	...
	Warning: Permanently added  'testpemasm.cloudapp.net,40.83.178.221'  'testpemasm.chinacloudapp.cn,40.83.178.221'  (RSA) to the list of known hosts.
	
    Welcome to Ubuntu  14.04.4  14.04.3  LTS (GNU/Linux 3.19.0-49-generic x86_64)
