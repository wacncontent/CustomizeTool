...

>[AZURE.NOTE] If you want to get started with Azure App Service before signing up for an account, go to [Try App Service](https://tryappservice.azure.com/), where you can immediately create a short-lived starter web app in App Service. No credit cards required, no commitments. 

...
2. Click the **+ New** icon on the  top  bottom  left of the Azure  Classic Management  Portal.
...
7. To enable FTP publishing, you must provide a user name and password.  Save the credentials and make  Make  a note of the user name and password you create.
...
After uploading both `index.php` and `createtable.php`, browse to **http://[site  name].azurewebsites.net/createtable.php**  name].chinacloudsites.cn/createtable.php**  to create the MySQL table for the application, then browse to **http://[site  name].azurewebsites.net/index.php**  name].chinacloudsites.cn/index.php**  to begin using the application.

## Next steps
