...
web app that is mobile-friendly and deploy it to Azure  Web  App  Service . For this tutorial, you need
...
-	Browser Emulator in  [Google  Google  Chrome  DevTools][EmulatorChrome]  DevTools . It contains presets for numerous Android devices, as well as Apple iPhone, Apple iPad, and Amazon Kindle Fire. It also emulates touch events.
...
the conference application  (*<prefix>*.azurewebsites.net)  (*<prefix>*.chinacloudsites.cn)  into the emulator, and then click the
...
In the mobile browser, tap the **Tag** button, then enter  `asp`  **asp**  in the
search box.
...
-	Deploy an ASP.NET MVC 5 application to an  App Service  Azure  web app
