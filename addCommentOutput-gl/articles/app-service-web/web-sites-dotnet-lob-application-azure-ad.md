...
In this article, you will learn how to create an ASP.NET MVC line-of-business application in [Azure  App Service  Web Apps](/documentation/services/web-sites/) using [Azure Active Directory](/home/features/identity/) as the identity provider. You will also learn how to use the [Azure Active Directory Graph Client Library](http://blogs.msdn.com/b/aadgraphteam/archive/2014/06/02/azure-active-directory-graph-client-library-1-0-publish.aspx) to query directory data in the application.
...
>[AZURE.NOTE] For Azure  App Service  Web Apps you can configure authentication against an Azure Active Directory tenant with a few clicks of a button. For more information, see [Use Active Directory for authentication in Azure  App Service](/documentation/articles/web-sites-authentication-authorization/)  Web App](/documentation/articles/web-sites-authentication-authorization/) .
...
You will build a simple line-of-business Create-Read-Update-Delete (CRUD) application in  App Service  Azure  Web Apps that tracks work items with the following features:
...
- Queries Azure Active Directory data using [Azure Active Directory Graph  API](http://msdn.microsoft.com/library/azure/hh974476.aspx)  API](http://msdn.microsoft.com/zh-cn/library/azure/hh974476.aspx) 
...
3.	In the [Azure  classic  Classic   portal](https://manage.windowsazure.com)  Management Portal](https://manage.windowsazure.cn)  view for your Azure Active Directory application you just created, click the **USERS** tab. Then, assign the desired users to the desired roles.

	>[AZURE.NOTE] If you want to assign roles to groups in addition to users, you must upgrade your Azure Active Directory tenant to [Azure Active Directory Premium](/home/features/identity/pricing/). In your application's  classic  Classic   portal  Management Portal  UI, if you see the **USERS** tab instead of the **USERS AND GROUPS tab, you can try Azure Active Directory Premium by going to your Azure Active Directory tenant's **LICENCES** tab.

3.	Once you're finished configuring the application, type `F5` in Visual Studio to run the ASP.NET application.

4.	Once the application loads, click **Sign In** and sign in with a user that has the Admin role in the Azure  classic  Classic   portal  Management Portal .

5.	If you configured the Azure Active Directory application properly and set the corresponding settings in Web.config, you should be redirected to the log in. Simply log in with the account you used to create the Azure Active Directory application in the Azure  classic  Classic   portal  Management Portal , since it's the Azure Active Directory application's default owner.

...

Here, you will publish the application to a web app in Azure  App Service . There are already instructions at [README.md](https://github.com/Azure-Samples/active-directory-dotnet-webapp-roleclaims/blob/master/README.md) for deploying to  App Service  Azure  Web Apps, but those steps also annul the configuration for your local debug environment. I'll show you how to deploy while preserving the debug configuration.
...
2. In the [Azure  classic  Classic   portal](https://manage.windowsazure.com)  Management Portal](https://manage.windowsazure.cn) , go to your Azure Active Directory tenant and click the **Applications** tab.
...
5. In App Properties, set **Sign-On URL** to the web app URL that you saved earler (e.g.  `https://<site-name>.azurewebsites.net/`)  `https://<site-name>.chinacloudsites.cn/`) , and the **APP ID URI** to `https://<aad-tenanet-name>/<app-name>`. Then, click **Complete**.
...
&lt;appSettings&gt;
   ...
   &lt;add key="ida:PostLogoutRedirectUri" value="<mark>[e.g.  https://mylobapp.azurewebsites.net/]</mark>"  https://mylobapp.chinacloudsites.cn/]</mark>"  xdt:Transform="SetAttributes" xdt:Locator="Match(key)" /&gt;
&lt;/appSettings&gt;</pre>
...
11. Add the highlighted [Authorize] decorations to the respective actions below.
	...
	Since you take care of role mappings in the Azure  classic  Classic   portal  Management Portal  UI, all you need to do is make sure that each action authorizes the right roles.
...
15. Open the [Package Manger Console](http://docs.nuget.org/Consume/Package-Manager-Console) and run **Enable-Migrations -EnableAutomaticMigrations**. Similar to the option you selected when you published the app to Azure, this command helps update your app's database schema in  [LocalDB](https://msdn.microsoft.com/library/hh510202.aspx)  [LocalDB](https://msdn.microsoft.com/zh-cn/library/hh510202.aspx)  when you debug it in Visual Studio.

15. Now, either run the app in the Visual Studio debugger or publish again to  App Service  Azure  Web Apps. Log in as the application owner and navigate to  `https://<webappname>.azurewebsites.net/WorkItems/Create`  `https://<webappname>.chinacloudsites.cn/WorkItems/Create` . You'll see now that you can pick an Azure Active Directory user or group from the drop down list, or type in something to filter the list.
...

- [Use Active Directory for authentication in Azure App Service](/documentation/articles/web-sites-authentication-authorization/)


- [Use Active Directory for authentication in Azure Web App](/documentation/articles/web-sites-authentication-authorization/)

- [Create a .NET MVC web app in Azure  App Service  with AD FS authentication](/documentation/articles/web-sites-dotnet-lob-application-adfs/)

...
- [Similarities between Active Directory and Azure Active  Directory](http://technet.microsoft.com/en-us/library/dn518177.aspx)  Directory](http://technet.microsoft.com/zh-cn/library/dn518177.aspx) 

