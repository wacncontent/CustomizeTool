...
This walkthrough shows you how to create an Azure SDK for Java application that creates a Web App in [Azure  App Service][]  Web App][] , then deploy an application to it. It consists of two parts:

- Part 1 demonstrates how to build a Java application that creates a web app.
- Part 2 demonstrates how to create a simple JSP "Hello World" application, then use an FTP client to deploy code to  Azure Web  App  Service .
...
4. In **Domain**, enter a domain name. This is a basic domain name that is included by default with your directory; it has the form  `<domain_name>.onmicrosoft.com`  `<domain_name>.partner.onmschina.cn` . You can name it based on the directory name or another domain name that you own. Later, you can add another domain name that your organization already uses.
...
The code in this procedure uses a self-signed certificate to authenticate with Azure. For this procedure, you need to create a certificate and upload it to the [Azure  classic  Classic   portal][]  Management Portal][]  beforehand. This involves the following steps:
...
To upload a self-signed certificate to Azure, go to the **Settings** page in the  classic  Classic   portal  Management Portal , then click the **Management Certificates** tab. Click **Upload** at the bottom of the page and navigate to the location of the CER file you created.
...
6. In **Dependencies**, click **Add**. In **Enter Group ID...** enter `azure-management`. Select the packages for base management and  App Service  Azure  Web Apps management:
...
Next, write the code that calls APIs in the Azure SDK for Java to create the  App Service  Azure  web app.
...
### Calling the Azure API to Create an  App Service  Azure  Web App
...
In WebCreator.java, add the following imports; these imports provide access to classes in the management libraries for consuming Azure APIs:
    ...
    // Imports for Azure  Web  App  Service  management configuration
    import com.microsoft.windowsazure.Configuration;
    import com.microsoft.windowsazure.management.configuration.ManagementConfiguration;
    
    // Service management imports for  App Service  Azure  Web Apps creation
...
Because the purpose of the AzureWebDemo application is to create an  App Service  Azure  Web App, name the main class for this application `WebAppCreator`. This class provides the main entry point code that calls the Azure Service Management API to create the web app.
...
- `webAppName` can be any name you choose; this procedure uses the name `WebDemoWebApp`. The full domain name is the `webAppName` with the `domainName` appended, so in this case the full domain is  `webdemowebapp.azurewebsites.net`  `webdemowebapp.chinacloudsites.cn` .
...
> of `webAppName` and `appServicePlanName` (or delete the web app on the Azure  Classic Management 
...
Next, define a method to create the web app. This method, `createWebApp`, specifies the parameters of the web app and the webspace. It also creates and configures the  App Service  Azure  Web Apps management client, which is defined by the [WebSiteManagementClient][] object. The management client is key to creating Web Apps. It provides RESTful web services that allow applications to manage web apps (performing operations such as create, update, and delete) by calling the service management API.

    private static void createWebApp() throws Exception {

        // Specify configuration settings for the  App Service  Azure  management client.
        ...
        // Create the  App Service  Azure  Web Apps management client to call Azure APIs
        // and pass it the  Azure Web  App  Service  management configuration object.
...
Log into the Azure  classic  Classic   portal  Management Portal  and click **Web Apps**. The new web app should appear in the Web Apps list within a few minutes.


## Deploying an Application to the Web App

After you have run AzureWebDemo and created the new web app, log into the  classic  Classic   portal  Management Portal , click **Web Apps**, and select **WebDemoWebApp** in the **Web Apps** list. In the web app's dashboard page, click **Browse** (or click the URL,  `webdemowebapp.azurewebsites.net`)  `webdemowebapp.chinacloudsites.cn`)  to navigate to it. You will see a blank placeholder page, because no content has been published to the web app yet.
...
In order to demonstrate how to deploy an application to the web, the following procedure shows you how to create a simple "Hello World" Java application and upload it to the  App Service  Azure  Web App that your application created.
...
For more information on deployment to an  App Service  Azure  web app using FTP, see the following topics:
...
1. Log into the  classic  Classic   portal  Management Portal  and click **Web Apps**. Make sure **WebDemoWebApp** appears in the list of web apps, and make sure that it is running. Click **WebDemoWebApp** to open its **Dashboard** page.
...
To obtain FTP connection information from the web app's  blade  page  in the [Azure  Classic Management  Portal][]:
...
1. In the  classic  Classic   portal  Management Portal , go to the web app's **Dashboard** page and click **Configure**. On the **Configure** page, specify the following settings.
...
1. After you have uploaded the WAR file and verified that Tomcat server has created an unpacked `JSPHello` directory, browse to  `http://webdemowebapp.azurewebsites.net/JSPHello`  `http://webdemowebapp.chinacloudsites.cn/JSPHello`  to run the application.
...
This procedure creates an  App Service  Azure  web app. You will be billed for the resource as long as it exists. Unless you plan to continue using the web app for testing or development, you should consider stopping or deleting it. A web app that has been stopped will still incur a small charge, but you can restart it at any time. Deleting a web app erases all data you have uploaded to it.

...
[What is an Azure AD directory]: http://technet.microsoft.com/en-us/library/jj573650.aspx


