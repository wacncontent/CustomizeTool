...
This article provides instructions on using a custom domain name that was purchased directly from [GoDaddy](https://godaddy.com) with  [App Service  [Azure  Web Apps](/documentation/services/web-sites/).
...
To associate your custom domain with a web app in  App Service  Azure , you must add a new entry in the DNS table for your custom domain by using tools provided by GoDaddy. Use the following steps to locate the DNS tools for GoDaddy.com
...
4. When adding a record, you must first select the record type.
	...
	* When adding a **CNAME (alias) record** - you must set the **Host** field to the sub-domain you wish to use. For example, **www**. You must set the **Points to** field to the  **.azurewebsites.net**  **.chinacloudsites.cn**  domain name of your Azure web app. For example, **contoso.azurwebsites.net**.

5. Click **Add Another**.
6. Select **CNAME** as the record type, then specify a **Host** value of **awverify** and a **Points to** value of  **awverify.&lt;yourwebappname&gt;.azurewebsites.net**  **awverify.&lt;yourwebappname&gt;.chinacloudsites.cn** .
