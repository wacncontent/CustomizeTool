...
This tutorial shows how to use Visual Studio tools that help debug a web app in  [App Service](/documentation/services/web-sites/)  [Azure Web App](/documentation/services/web-sites/) , by running in [debug mode](http://www.visualstudio.com/get-started/debug-your-app-vs.aspx) remotely or by viewing application logs and web server logs.

...
If you have Visual Studio Ultimate, you can also use  [IntelliTrace](http://msdn.microsoft.com/library/vstudio/dd264915.aspx)  [IntelliTrace](http://msdn.microsoft.com/zh-cn/library/vstudio/dd264915.aspx)  for debugging. IntelliTrace is not covered in this tutorial.
...

	For more information about connecting to Azure resources from Visual Studio, see [Manage Accounts, Subscriptions, and Administrative Roles](https://msdn.microsoft.com/library/azure/hh531793.aspx).
...
3. Expand the resource group that includes the web app that you created in [Getting started with Azure and ASP.NET][GetStarted], and then right-click the web app node and click **View Settings**.
	...
	The  **Azure Web App**  **App Services**  tab appears, and you can see there the web app management and configuration tasks that are available in Visual Studio.
...
1. In **Server Explorer**, expand **Azure**, expand **App  Service**  Services** , expand the resource group that your web app is located in, and then expand the node for your web app.
...
5. In **Server Explorer** expand **Azure >  App Service  Azure  > your resource group > your web app > WebJobs > Continuous**, and then right-click **ContosoAdsWebJob**.
...
* If you find that the debugger won't step into code that you want to debug, you might have to change the Just My Code setting.  For more information, see [Restrict stepping to Just My  Code](http://msdn.microsoft.com/library/vstudio/y740d9d3.aspx#BKMK_Restrict_stepping_to_Just_My_Code)  Code](http://msdn.microsoft.com/zh-cn/library/vstudio/y740d9d3.aspx#BKMK_Restrict_stepping_to_Just_My_Code) .
...
* You can manually attach the debugger to any process, not only the web app process (w3wp.exe). For more information about how to use debug mode in Visual Studio, see [Debugging in Visual  Studio](http://msdn.microsoft.com/library/vstudio/sc65sadd.aspx)  Studio](http://msdn.microsoft.com/zh-cn/library/vstudio/sc65sadd.aspx) .
...
* **Application tracing logs**<br/>
  The application creates these logs by calling methods of the  [System.Diagnostics.Trace](http://msdn.microsoft.com/library/system.diagnostics.trace.aspx)  [System.Diagnostics.Trace](http://msdn.microsoft.com/zh-cn/library/system.diagnostics.trace.aspx)  class.
* **Web server logs**<br/>
...
3. Add a <a  href="http://msdn.microsoft.com/library/vstudio/6915t83k(v=vs.100).aspx">trace  href="http://msdn.microsoft.com/zh-cn/library/vstudio/6915t83k(v=vs.100).aspx">trace  element</a> under `<system.web>` in the Web.config file, such as the following example:
...
Application tracing logs can also be sent to an Azure storage account, and you can view them in Visual Studio. To do that you'll create a storage account, enable storage logs in the classic portal, and view them in the **Logs** tab of the  **Azure Web App**  **App Services**  window.
...
3. In the **Create Storage Account** dialog, enter a name for the storage account. 
	...
	The URL to access your storage account will be  *{name}*.core.windows.net  *{name}*.core.chinacloudapi.cn .
...
1. In the Visual Studio  **Azure Web App**  **App Services**  window, click the **Logs** tab, and then click **Configure Logging in Management Portal**.
...
8. In the **Logs** tab of the  **Azure Web App**  **App Services**  window in Visual Studio, click **Refresh** under **Diagnostic Summary**.
...
1. In the **Configuration** tab of the  **Azure Web App**  **App Services**  window that you opened from **Server Explorer**, change **Failed Request Tracing** to **On**, and then click **Save**.
...
2. In Visual Studio, in the **Configuration** tab of the  **Azure Web App**  **App Services**  window, click **Open in Management Portal**.
...
For more information about how to use debug mode in Visual Studio, see the [Debugging in Visual  Studio](http://msdn.microsoft.com/library/vstudio/sc65sadd.aspx)  Studio](http://msdn.microsoft.com/zh-cn/library/sc65sadd.aspx)  MSDN topic and [Debugging Tips with Visual Studio 2010](http://weblogs.asp.net/scottgu/archive/2010/08/18/debugging-tips-with-visual-studio-2010.aspx).

