...
At a high level an  App Service  Azure  Environment consists of several major components:
...
The compute resources are used for your 4 resource pools.  Each  App Service  Azure  Environment has a set of Front Ends and 3 possible Worker Pools.  You don't need to use all 3 Worker Pools and if you want you can just use one or two.  The resource pools, Front Ends and Workers, are not directly accessible to tenants.  You cannot RDP to them, change their provisioning or act as an admin on them but you can set their quantity and size. In an ASE you have 4 size options labeled P1 through P4.  For details around those sizes and their pricing please see here  [App Service Pricing](/documentation/services/web-sites/)  [Azure Pricing](/documentation/services/web-sites) .
...
Because this capability places the Azure  App Service  into your VNET it means that your apps hosted in your ASE can now access resources made available through ExpressRoute or Site to Site VPNs directly.  The apps within your  App Service  Azure  Environments do not require additional networking features to access resources available to the VNET hosting your  App Service  Azure  Environment. This means you don't need to use VNET Integration or Hybrid Connections to get to resources in or connected to your VNET.  You can still use both of those features though to access resources in networks that are not connected to your VNET.  For example you can use VNET Integration to integrate with a VNET that is in your subscription but isn't connected to the VNET that your ASE is in.  You can still also use Hybrid Connections to access resources in other networks just like you normally can.

If you do have your VNET configured with an ExpressRoute VPN you should be aware of some of the routing needs that an ASE has.  There are some user defined route (UDR) configurations that are incompatible with an ASE.  For more details around running an ASE in a VNET with ExpressRoute see the document here: [Running an  App Service  Azure  Environment in a VNET with ExpressRoute][ExpressRoute]

You can also now control access to your apps using Network Security Groups.  This capability allows you to lock down your  App Service  Azure  Environment to just the IP addresses you wish to restrict it to.  For more information around how to do that see the document here [How to Control Inbound Traffic in an  App Service  Azure  Environment](/documentation/articles/app-service-app-service-environment-control-inbound-traffic/).

## Portal

The UI to manage and monitor your  App Service  Azure  Environment is available from the Azure Portal.  If you have an ASE then you are likely to see the  App Service  Azure  symbol on your sidebar.  This symbol is used to represent  App Service  Azure  Environments in the Azure Portal.

![][1]

You can use the icon or select the chevron (greater than symbol) at the bottom of the sidebar and select  App Service  Azure  Environments.  Both do the same thing which is to open up the UI that lists all of your  App Service  Azure  Environments.  Selecting one of the ASEs listed opens up the UI used to monitor and manage it.
...
All of the metrics that can be tracked in the charts can also be used to set up Alerts.  Setting up Alerts works the same as they do elsewhere in  Azure Web  App  Service . You can set an alert from either the Alerts UI part or from drilling into any metrics UI and hitting Add Alert.
 
![][3]

The metrics that were just discussed are the  App Service  Azure  Environment metrics.  There are also metrics available at the App Service Plan level.  This is where monitoring CPU and memory makes a lot of sense.  In an ASE, all of the ASPs are dedicated ASPs.  That is to say that the only apps that are running on the hosts allocated to that ASP are the apps in that ASP.
...
An  App Service  Azure  Environment can be configured to use up to 55 total compute resources.  Of those 55 compute resources, only 50 can be used to host workloads. The reason for that is two fold.  There are a minimum of 2 Front End compute resources.  That leaves up to 53 to support worker pool allocation. In order to provide fault tolerance, you need to have an additional compute resource allocated according to the following rules:
...
If you want to delete an  App Service  Azure  Environment then simply use the Delete action at the top of the  App Service  Azure  Environment blade.  When you do this you will be prompted to enter the name of your  App Service  Azure  Environment to confirm that you really want to do this.  NOTE: When you delete an  App Service  Azure  Environment you delete all the content within it as well.
