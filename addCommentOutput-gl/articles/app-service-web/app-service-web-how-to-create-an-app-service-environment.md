...
 App Service  Azure  Environments (ASE) are a Premium service option of Azure  App Service  that delivers an enhanced configuration capability that is not available in the multi-tenant stamps.  The ASE feature essentially deploys the Azure  App Service  into a customer's virtual network.  To gain a greater understanding of the capabilities offered by  App Service  Azure  Environments read the [What is an  App Service  Azure  Environment][WhatisASE] documentation.
...
There are two ways to access the ASE creation UI.  It can be found by searching in the Azure  Marketplace  gallery  for  ***App Service  ***Azure  Environment*** or by going through New -> Web + Mobile.
...
The name that is specified for the ASE will be used for the apps created in the ASE.  If name of the ASE is appsvcenvdemo then the domain name would be  .*appsvcenvdemo.p.azurewebsites.net*  .*appsvcenvdemo.p.chinacloudsites.cn* .  If you thus created an app named *mytestapp* then it would be addressable at  *mytestapp.appsvcenvdemo.p.azurewebsites.net*  *mytestapp.appsvcenvdemo.p.chinacloudsites.cn* .  You cannot use white space in the name of your ASE.  If you use upper case characters in the name, the domain name will be the total lowercase version of that name.
...
### Virtual Network ###
While there is a quick create capability that will automatically create a new VNET, the feature also supports selection of an existing VNET and manual creation of a VNET.  You can select an existing VNET (only classic "v1" virtual networks are supported at this time) if it is large enough to support an  App Service  Azure  Environment deployment.  The VNET must have at least 8 addresses or more.
...
In addition to being able to manage the quantity of compute resources that you can assign to a given pool you also have control over the size.  With  App Service  Azure  Environments you can choose from 4 different sizes labeled P1 through P4.  For details around those sizes and their pricing please see here  [App Service  [Azure  Pricing][AppServicePricing].  The P1 to P3 compute resource sizes are the same as what is available in the multi-tenant environments.  The P4 compute resource gives 8 cores with 14 GB of RAM and is only available in an  App Service  Azure  Environment.

Pricing for  App Service  Azure  Environments is against the compute resources assigned.  You pay for the compute resources allocated to your  App Service  Azure  Environment regardless if they are hosting workloads or not.
...
There are more details around manual scaling, management and monitoring of  App Service  Azure  Environments here: [How to configure an  App Service  Azure  Environment][ASEConfig]
...
There are additional dependencies that are not available for customization such as the database and storage.  These are handled by Azure and come with the system.  The system storage supports up to 500 GB for the entire  App Service  Azure  Environment and the database is adjusted by Azure as needed by the scale of the system.
