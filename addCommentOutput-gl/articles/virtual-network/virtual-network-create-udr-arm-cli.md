...
3. Run the **`azure network route-table create`** command to create a route table for the front end subnet.
		...
		azure network route-table create -g TestRG -n UDR-FrontEnd -l  uswest  chinanorth 
	...
	- **-l (or --location)**. Azure region where the new NSG will be created. For our scenario,  *westus*  *chinanorth* .
...
1. Run the **`azure network route-table create`** command to create a route table for the back end subnet.
		...
		azure network route-table create -g TestRG -n UDR-BackEnd -l  westus  chinanorth 
