...
使用 SSH 命令连接到 HDInsight 群集的完全限定域名 (FQDN)。FQDN 将是你为群集提供的名称，后跟 **.azurehdinsight.cn**。例如，以下命令将连接到名为 **myhdinsight** 的群集：
...
3. 在作业完成时，使用以下命令列出存储在 ****wasb://example/data/WordCountOutput** 上的输出文件：
