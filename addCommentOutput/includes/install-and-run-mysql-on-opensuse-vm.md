...
4. Start the MySQL daemon (mysqld) manually with this command:
	...
	> [AZURE.IMPORTANT] After installation, the MySQL root password is empty by default. We recommended that you run <!-- deleted by customization **mysql\_secure\_installation** --><!-- keep by customization: begin --> **mysql\_secure_installation** <!-- keep by customization: end -->, a script that helps secure MySQL. The script prompts you to change the MySQL root password, remove anonymous user accounts, disable remote root logins, remove test databases, and reload the privileges table. We recommended that you answer yes to all of these options and change the root password.
