...
Whether <!-- deleted by customization an application or service --><!-- keep by customization: begin --> software <!-- keep by customization: end --> runs in the cloud or on premises, it often needs to interact with other <!-- deleted by customization applications or services --><!-- keep by customization: begin --> software <!-- keep by customization: end -->. To provide a broadly useful way to do this, Azure offers Service Bus. This article takes a look at this technology, describing what it is and why you might want to use it.
...
Within a namespace, you can use one or more instances of <!-- deleted by customization four --><!-- keep by customization: begin --> three <!-- keep by customization: end --> different communication mechanisms, each of which connects applications in a different way. The choices are:

- *Queues*, which allow one-directional communication. Each queue acts as an intermediary (sometimes called a *broker*) that stores sent messages until they are received. <!-- deleted by customization Each message is received by a single recipient. -->
- *Topics*, which provide one-directional communication using <!-- deleted by customization *subscriptions*-a single topic can have multiple subscriptions --><!-- keep by customization: begin --> *subscriptions* <!-- keep by customization: end -->. Like a queue, a topic acts as a broker, but <!-- keep by customization: begin --> it allows <!-- keep by customization: end --> each subscription <!-- deleted by customization can optionally use a filter --> to <!-- deleted by customization receive --><!-- keep by customization: begin --> see <!-- keep by customization: end --> only messages that match specific criteria.
...
The process is simple: A sender sends a message to a Service Bus queue, and a receiver picks up that message at some later time. A queue can have just a single receiver, as [Figure 2](#Fig2) shows, or multiple applications can read from the same queue. In the latter situation, each message is <!-- keep by customization: begin --> typically <!-- keep by customization: end --> read by just one <!-- deleted by customization receiver-for --><!-- keep by customization: begin --> receiver-queues don't provide <!-- keep by customization: end --> a multi-cast service <!-- deleted by customization you should use a topic instead -->.
...
A receiver can read a message from a Service Bus queue in two different ways. The first option, called <!-- deleted by customization *ReceiveAndDelete* --><!-- keep by customization: begin --> ReceiveAndDelete <!-- keep by customization: end -->, removes a message from the queue and immediately deletes it. This is simple, but if the receiver crashes before it finishes processing the message, the message will be lost. Because it's been removed from the queue, no other receiver can access it.

The second option, <!-- deleted by customization *PeekLock* --><!-- keep by customization: begin --> PeekLock <!-- keep by customization: end -->, is meant to help with this problem. Like ReceiveAndDelete, a PeekLock read removes a message from the queue. It doesn't delete the message, however. Instead, it locks the message, making it invisible to other receivers, then waits for one of three events:

- If the receiver processes the message successfully, it calls <!-- deleted by customization *Complete* --><!-- keep by customization: begin --> Complete <!-- keep by customization: end -->, and the queue deletes the message.
- If the receiver decides that it can't process the message successfully, it calls <!-- deleted by customization *Abandon* --><!-- keep by customization: begin --> Abandon <!-- keep by customization: end -->. The queue then removes the lock from the message and makes it available to other receivers.
...
- Subscriber 3 has set its filter to *True*, which means that it receives all messages. <!-- deleted by customization For example, this --><!-- keep by customization: begin --> This <!-- keep by customization: end --> application might be responsible for maintaining an audit trail <!-- keep by customization: begin -->, for example, <!-- keep by customization: end --> and <!-- deleted by customization therefore --><!-- keep by customization: begin --> so <!-- keep by customization: end --> it needs to see <!-- deleted by customization all the messages --><!-- keep by customization: begin --> everything <!-- keep by customization: end -->.
...
Suppose you want to connect two on-premises applications, both running inside corporate datacenters. Each of these applications sits behind a firewall, and each datacenter probably uses network address translation (NAT). The firewall blocks incoming data on all but a few ports, and NAT implies that the machine each application is running on <!-- keep by customization: begin --> probably <!-- keep by customization: end --> doesn't have a fixed IP address <!-- deleted by customization that you can reach directly from outside the datacenter -->. Without some extra help, connecting these applications over the public Internet is problematic.

A Service Bus relay provides this help. To communicate bi-directionally through a relay, each application establishes an outbound TCP connection with Service Bus, then keeps it open. All communication between the two applications will travel over these connections. Because each connection was established from inside the datacenter, the firewall will allow incoming traffic to each <!-- deleted by customization application without --><!-- keep by customization: begin --> application"data sent through the relay"without <!-- keep by customization: end --> opening new ports. This approach also gets around the NAT problem, because each application has a consistent endpoint <!-- deleted by customization in the cloud --> throughout the communication. By exchanging data through the relay, the applications can avoid the problems that would otherwise make communication difficult.
...
Unlike queues and topics, applications don't explicitly create relays. Instead, when an application that wishes to receive messages establishes a TCP connection with Service Bus, a relay is created automatically. When the connection is dropped, the relay is deleted. To let an application find the relay created by a specific listener, Service Bus provides a registry that <!-- deleted by customization enables applications to locate --><!-- keep by customization: begin --> allows locating <!-- keep by customization: end --> a specific relay by name.

Relays are the right solution when you need direct communication <!-- deleted by customization between applications -->. <!-- deleted by customization For example, consider --><!-- keep by customization: begin --> Think about <!-- keep by customization: end --> an airline reservation system running in an on-premises datacenter <!-- keep by customization: begin -->, for example, <!-- keep by customization: end --> that must be accessed from check-in kiosks, mobile devices, and other computers. Applications running on all of these systems could rely on Service Bus relays in the cloud to communicate, wherever they might be running.
