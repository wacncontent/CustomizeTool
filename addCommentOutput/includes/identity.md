...
- You can run Windows Server Active Directory (commonly called just AD) in the cloud using virtual machines created with Azure Virtual <!-- deleted by customization machines --><!-- keep by customization: begin --> Machines <!-- keep by customization: end -->. This approach makes sense when you're using Azure to extend your on-premises datacenter into the cloud.
...
## <a name="adinvm"></a>Running Windows Server Active Directory in <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end -->

Running Windows Server AD in Azure <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end --> is much like running it <!-- deleted by customization on-premises --><!-- keep by customization: begin --> on premises <!-- keep by customization: end -->. [Figure 1](#fig1) shows a typical example of how this looks.

![Azure Active Directory in Virtual Machine](./media/identity/identity_01_ADinVM.png)


<a name="Fig1"></a>Figure 1: Windows Server Active Directory can run in Azure <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end --> connected to an organization's on-premises datacenter using Azure Virtual Network.

In the example shown here, Windows Server AD is running in VMs created using Azure Virtual Machines, the platform's IaaS technology. These VMs and a few others are grouped into a virtual network <!-- keep by customization: begin --> (VNET) <!-- keep by customization: end --> connected to an on-premises datacenter using Azure Virtual Network. The <!-- deleted by customization virtual network --><!-- keep by customization: begin --> VNET <!-- keep by customization: end --> carves out a group of cloud <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end --> that interact with the on-premises network via a virtual private network (VPN) connection. Doing this lets these Azure <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end --> look like just another subnet to the on-premises datacenter. As the figure shows, two of those VMs are running Windows Server AD domain controllers. The other <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end --> in the <!-- deleted by customization virtual network --><!-- keep by customization: begin --> VNET <!-- keep by customization: end --> might be running applications, such as SharePoint, or being used in some other way, such as for development and testing. The on-premises datacenter is also running two Windows Server AD domain controllers.

There are several options for connecting the domain controllers in the cloud with those running on premises <!-- keep by customization: begin -->. They include these <!-- keep by customization: end -->:
...
- Create separate AD forests in the cloud and on-premises, then connect the forests using cross-forest trusts or Windows Server Active Directory Federation Services (AD FS), which can also run in <!-- deleted by customization virtual machines --><!-- keep by customization: begin --> VMs <!-- keep by customization: end --> on Azure.
...
David Chappell is Principal of Chappell & Associates [www.davidchappell.com](http://www.davidchappell.com) in San Francisco, California. <!-- keep by customization: begin --> Through his speaking, writing, and consulting, <!-- keep by customization: end -->
