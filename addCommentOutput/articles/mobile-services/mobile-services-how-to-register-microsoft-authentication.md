...
2. Select **Create a new app by reserving a unique name** and click **Continue**, then type a name for your app in **App name**, click **Reserve app name** <!-- keep by customization: begin -->, <!-- keep by customization: end --> and then **Save**.
...
2. In the Microsoft account page for your app, click **API Settings**, select enable **Mobile or desktop client app**, set the mobile service URL as the **Target domain**, supply a value of <!-- deleted by customization `https://<mobile_service>.azure-mobile.cn/` --><!-- keep by customization: begin --> `https://<mobile_service>.azure-mobile.cn/login/microsoftaccount/` <!-- keep by customization: end --> in **Redirect URL**, then click **Save**.
