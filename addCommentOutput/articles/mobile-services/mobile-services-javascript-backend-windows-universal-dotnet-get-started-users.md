...
This tutorial is based on the Mobile Services quickstart. You must also first complete the tutorial [Get started with Mobile Services] <!-- keep by customization: begin --> or [Add Mobile Services to an existing app](/documentation/articles/mobile-services-javascript-backend-windows-universal-dotnet-get-started-data) <!-- keep by customization: end -->.
