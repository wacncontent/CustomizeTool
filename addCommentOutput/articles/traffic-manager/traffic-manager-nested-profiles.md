...
Suppose your application is deployed in multiple Azure regions--China North , West Europe,  and China East. You use Traffic Manager's 'Performance' traffic-routing method to distribute traffic to the region closest to the user.
...
However, suppose you prefer the  West Europe  China East site 1  traffic fail over to China North by preference, and only direct elsewhere if both of those endpoints are unavailable. You can do this by creating a child profile which uses the 'Priority' traffic-routing method, as shown:

!['Performance' traffic routing with preferential failover][6]

Since the  West Europe  China East site 1  endpoint has higher priority than the China North endpoint, all traffic will be sent to the  West Europe  China East site 1  endpoint when both are online.  If  West Europe  China East site 1  fails, its traffic is directed to China North.  Only if China North also fails would  West Europe  China East site 1  traffic be directed to China East  site 2 .
