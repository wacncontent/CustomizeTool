...
When you contact support, it is recommended to include the following information:
 ...
 - **General description of the error** <!-- deleted by customization - --><!-- keep by customization: begin -->  <!-- keep by customization: end --> what exact error message did the user see?  If there was no error message, describe the unexpected behavior you noticed, in detail.
 - **Page** <!-- deleted by customization - --><!-- keep by customization: begin -->  <!-- keep by customization: end --> what page were you on when you saw the error (include the URL)?
 - **ErrorCode** - the specific error code you are receiving.
 - **SessionId** - the specific session id you are receiving.
 - **Correlation ID** <!-- deleted by customization - --><!-- keep by customization: begin -->  <!-- keep by customization: end --> what was the correlation id code generated when the user saw the error.
 - **Timestamp** <!-- deleted by customization - --><!-- keep by customization: begin -->  <!-- keep by customization: end --> what was the precise date and time you saw the error (include the timezone)?
 
![Correlation ID](./media/multi-factor-authentication-end-user-manage/correlation.png) 

 - **User ID** <!-- deleted by customization - --><!-- keep by customization: begin -->  <!-- keep by customization: end --> what was the ID of the user who saw the error (e.g. user@contoso.com)?
 - **Information about the user** <!-- deleted by customization - --><!-- keep by customization: begin -->  <!-- keep by customization: end --> was the user federated, password hash synced, cloud only?  Did the user have an Azure AD Premium, Enterprise Mobility, or Azure AD Basic license assigned?  Is the user using Office 365? etc.
