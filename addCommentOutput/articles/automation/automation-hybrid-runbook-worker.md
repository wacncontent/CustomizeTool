...
- SMA requires a local installation of Windows Azure Pack that has <!-- keep by customization: begin --> higher <!-- keep by customization: end --> more local resources and <!-- deleted by customization higher --> maintenance costs than Azure Automation which only needs an agent installed on local runbook workers.  The agents are managed by Operations Management Suite furthering decreasing their maintenance costs.
