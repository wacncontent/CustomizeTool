...
A *webhook* allows you to start a particular runbook in Azure Automation through a single HTTP request. This allows external services such as Visual Studio <!-- deleted by customization Team Services --><!-- keep by customization: begin --> Online <!-- keep by customization: end -->, GitHub, or custom applications to start runbooks without implementing a full solution using the Azure Automation API.
...
If you specify a value for $WebhookData when you create the webhook, that value will be overriden when the webhook starts the runbook with the data from the client POST request, even if the client does not include any data in the request body.  If you start a runbook that has $WebhookData using a method other than a webhook, you can provide a value for $Webhookdata that will be recognized by the runbook.  This value should be an object with the same <!-- deleted by customization [properties](#details-of-a-webhook) --><!-- keep by customization: begin --> properties <!-- keep by customization: end --> as $Webhookdata so that the runbook can properly work with it <!-- deleted by customization as if it was working with actual WebhookData passed by a webhook -->.
<!-- deleted by customization

For example, if you are starting the following runbook from the Azure Management Portal and want to pass some sample WebhookData for testing, since WebhookData is an object, it should be passed as JSON in the UI.
...
The client cannot determine when the runbook job completes or its completion status from the webhook.  It can determine this information using the job id with another method such as [Windows PowerShell](http://msdn.microsoft.com/zh-cn/library/azure/dn690263.aspx) or the [Azure Automation API](https://msdn.microsoft.com/zh-cn/library/azure/mt163826.aspx).
