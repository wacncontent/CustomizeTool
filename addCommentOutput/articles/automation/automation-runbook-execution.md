...
|Suspended|The job was suspended by the user, by the system, or by a command in the runbook. A job that is suspended can be started again and will resume from its last checkpoint or from the beginning of the runbook if it has no checkpoints. The runbook will only be suspended by the system in the case of an exception. By default, ErrorActionPreference is set to **Continue** meaning that the job will keep running on an error. If this preference variable is set to **Stop** then the job will suspend on an error.  Applies to <!-- deleted by customization [Graphical and --> PowerShell Workflow <!-- deleted by customization runbooks](/documentation/articles/automation-runbook-types) --><!-- keep by customization: begin --> runbooks <!-- keep by customization: end --> only.|
|Suspending|The system is attempting to suspend the job at the request of the user. The runbook must reach its next checkpoint before it can be suspended. If it has already passed its last checkpoint, then it will complete before it can be <!-- deleted by customization suspended.  Applies to [Graphical and PowerShell Workflow runbooks](/documentation/articles/automation-runbook-types) only.| --><!-- keep by customization: begin --> suspended.| <!-- keep by customization: end -->
...
In order to share resources among all runbooks in the cloud, Azure Automation will temporarily unload any job after it has been running for 3 hours. <!-- deleted by customization  [Graphical](/documentation/articles/automation-runbook-types#graphical-runbooks) and [PowerShell Workflow](/documentation/articles/automation-runbook-types#powershell-workflow-runbooks) --><!-- keep by customization: begin --> PowerShell Workflow <!-- keep by customization: end --> runbooks will be resumed from their last [checkpoint](http://technet.microsoft.com/zh-cn/library/dn469257.aspx#bk_Checkpoints). During this time, the job will show a status of Running, Waiting for Resources. If the runbook has no checkpoints or the job had not reached the first checkpoint before being unloaded, then it will restart from the beginning. <!-- deleted by customization  [PowerShell](/documentation/articles/automation-runbook-types#powershell-runbooks) runbooks are always restarted from the beginning since they don't support checkpoints. -->
