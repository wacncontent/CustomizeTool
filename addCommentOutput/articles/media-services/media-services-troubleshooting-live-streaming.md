...
- **Potential issue**: Many RTP encoders do not allow for host names, and an IP address will need to be acquired.  
	...
	**Troubleshooting steps**: To find the IP address, open a command prompt on any computer. To do this in Windows, open the Run launcher (WIN + R) and type <!-- deleted by customization âcmdâ --><!-- keep by customization: begin --> "cmd" <!-- keep by customization: end --> to open.
