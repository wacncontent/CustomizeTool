...
6.	Paste the token that you got from your STS in the Token textbox. 
	...
	The castLab license server does not need the <!-- deleted by customization âBearer=â --><!-- keep by customization: begin --> "Bearer=" <!-- keep by customization: end --> prefix in front of the token. So please remove that before submitting the token.
7.	Update the player.
