...
The emulator simulates the Azure Compute service and runs in your local environment so that you can test and debug your cloud service before you deploy it. The emulator handles the lifecycle of your role instances and provides access to simulated resources, such as local storage. When you debug or run your service from Visual Studio, it automatically starts the emulator as a background application and then deploys your service to the emulator. You can use the emulator to view your service when it runs in the local environment. You can run the full version or the express version of the emulator. (Starting with Azure 2.3, the express version of the emulator is the default.) See [Using Emulator Express to Run and Debug a Cloud Service <!-- deleted by customization Locally](https://msdn.microsoft.com/zh-cn/library/dn339018.aspx) --><!-- keep by customization: begin --> Locally](http://go.microsoft.com/fwlink/p/?LinkId=623044) <!-- keep by customization: end -->.

### To debug your cloud service on your local computer

1. On the menu bar, choose <!-- deleted by customization **Debug** --><!-- keep by customization: begin --> Debug <!-- keep by customization: end -->, <!-- deleted by customization **Start Debugging** --><!-- keep by customization: begin --> Start Debugging <!-- keep by customization: end --> to run your Azure cloud service project. As an alternative, you can press <!-- deleted by customization F5. Youâll --><!-- keep by customization: begin --> F5.Youâll <!-- keep by customization: end --> see a message that the Compute Emulator is starting. When the emulator starts, the system tray icon confirms it.
...
>[AZURE.NOTE] When you publish a cloud service from Visual Studio, you can enable <!-- deleted by customization **IntelliTrace** --><!-- keep by customization: begin --> IntelliTrace <!-- keep by customization: end --> for any roles in that service that target the .NET Framework 4 or the .NET Framework 4.5. By using <!-- deleted by customization **IntelliTrace** --><!-- keep by customization: begin --> IntelliTrace <!-- keep by customization: end -->, you can examine events that occurred in a role instance in the past and reproduce the context from that time. See [Debugging a published cloud service with IntelliTrace and Visual Studio](/documentation/articles/vs-azure-tools-intellitrace-debug-published-cloud-services/) and [Using IntelliTrace](https://msdn.microsoft.com/zh-cn/library/dd264915.aspx).

### To enable remote debugging for a cloud service

1. Open the shortcut menu for the Azure project, and then click <!-- deleted by customization **Publish** --><!-- keep by customization: begin --> Publish <!-- keep by customization: end -->.
...
- The debugger uses ports 30400 to 30424 <!-- deleted by customization, --><!-- keep by customization: begin --> and <!-- keep by customization: end --> 31400 to 31424 <!-- deleted by customization and 32400 to 32424 -->. If you try to use any of these ports, you won't be able to publish your service, and one of the following error messages will appear in the activity log for Azure:
...
1. In Visual Studio, create a new ASP.NET <!-- deleted by customization web site --><!-- keep by customization: begin --> Web Site <!-- keep by customization: end -->.
...
-->
<!-- keep by customization: begin -->
1. Select the various settings for the virtual machine and then click OK. See [Virtual Machines]( https://msdn.microsoft.com/zh-cn/library/azure/jj156003.aspx) for more information.The name you enter for DNS name will be the name of the virtual machine. 
...
- Use <!-- deleted by customization **Intellitrace** --><!-- keep by customization: begin --> Intellitrace <!-- keep by customization: end --> to collect a log of calls and events from a release server. See [Debugging a Published Cloud Service with IntelliTrace and Visual Studio](/documentation/articles/vs-azure-tools-intellitrace-debug-published-cloud-services/).
- Use <!-- deleted by customization **Azure Diagnostics** --><!-- keep by customization: begin --> use Azure Diagnostics <!-- keep by customization: end --> to log detailed information from code running within roles, whether the roles are running in the development environment or in Azure. See [Collecting logging data by using Azure Diagnostics](https://msdn.microsoft.com/zh-cn/library/gg433048.aspx).
