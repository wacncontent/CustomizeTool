<properties 
	pageTitle="Learn how to use Stream Analytics Management .NET SDK | Windows Azure" 
...
For the .NET API reference, see [Stream Analytics Management .NET SDK](https://msdn.microsoft.com/zh-cn/library/azure/dn889315.aspx).
...
-	Set up an input source and output target to use. For futher instructions see [Add Inputs](/documentation/articles/stream-analytics-add-inputs) to set up a sample input and [Add Outputs](/documentation/articles/stream-analytics-add-outputs) to setup a sample output.
...
4. Add the following **appSettings** section to the App.config file:
		...
		<appSettings>
		  ...
		  <add key="ResourceManagerEndpoint" value="https://manage.windowsazure.cn/" />
...
The following code creates a Stream Analytics transformation with the query "select * from Input" and specifies to allocate one streaming unit for the Stream Analytics job. For more information on adjusting streaming units, see [Scale Azure Stream Analytics jobs](/documentation/articles/stream-analytics-scale-jobs).
