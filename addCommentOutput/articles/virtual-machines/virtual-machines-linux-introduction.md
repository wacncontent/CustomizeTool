...
Azure provides an inbound packet filter that restricts connectivity to ports specified in the <!-- deleted by customization Azure --> Management Portal. By default, the only allowed port is SSH. You may open up access to additional ports on your Linux virtual machine by configuring endpoints in the <!-- deleted by customization Azure --> Management Portal:
...
3. Click *Capture* in the <!-- deleted by customization Azure --> Management Portal or use the Powershell or CLI tools to capture the virtual machine as an image.
