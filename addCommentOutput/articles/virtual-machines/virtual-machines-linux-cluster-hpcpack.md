...
In this example, we create an Azure File share named rdma on our storage account allvhdsje. To mount the share on the head node, <!-- keep by customization: begin --> we <!-- keep by customization: end --> open a Command window and enter the following commands:
