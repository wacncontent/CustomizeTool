...
10.	Create an endpoint for your Virtual Machine:
	...
	1. Log in to the [Azure Management <!-- deleted by customization Portal](https://ms.portal.azure.com/) --><!-- keep by customization: begin --> Portal](hhttps://manage.windowsazure.cn) <!-- keep by customization: end -->.
...
12.	To see the shopping cart application running on the Internet, open a browser to the URL in the form of `http://<<unique_domain_name>>/shoppingcart`. (You can determine the value for <<*unique_domain_name*>> within the [Azure Management <!-- deleted by customization Portal](https://ms.portal.azure.com/) --><!-- keep by customization: begin --> Portal](https://manage.windowsazure.cn) <!-- keep by customization: end --> by clicking **Virtual Machines** and then selecting the Virtual Machine that you are using to run Oracle WebLogic Server).
