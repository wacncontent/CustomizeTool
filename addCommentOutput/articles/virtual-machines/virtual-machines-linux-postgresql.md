...
5. If  you want to build everything that can be built, including the documentation (HTML and man <!-- deleted by customization pages) and --><!-- keep by customization: begin --> pages)and <!-- keep by customization: end --> additional modules (contrib), run the following command instead:
...
3. Create a non-root user and modify that user’s profile. Then <!-- deleted by customization, --> switch to this new user (called *postgres* in our example):
...
4. Edit the *bash_profile* <!-- deleted by customization file --> by entering the commands below. These lines will be added to the end of the *bash_profile* file:
...
6. Validate your installation <!-- deleted by customization by using --><!-- keep by customization: begin --> with <!-- keep by customization: end --> the following command:
...
Modify two variables in the /etc/init.d/postgresql file. <!-- deleted by customization The --><!-- keep by customization: begin --> Thhe <!-- keep by customization: end --> prefix is set to the installation path of PostgreSQL: **/opt/pgsql**. PGDATA is set to the data storage path of PostgreSQL: **/opt/pgsql_data**.
...
Connect to the events database <!-- deleted by customization that you --> just created:
...
4. The “date” column <!-- deleted by customization shows --><!-- keep by customization: begin --> will show <!-- keep by customization: end --> when they signed up for the event. Postgres requires that dates be written as yyyy-mm-dd.

You should see the following if <!-- deleted by customization your --><!-- keep by customization: begin --> you <!-- keep by customization: end --> table has been successfully created:

![image](./media/virtual-machines-linux-postgresql/no4.png)

You can also check the table structure <!-- deleted by customization by using --><!-- keep by customization: begin --> with <!-- keep by customization: end --> the following command:
...
Use the following command to update data in a table. For this one, Sandy has confirmed that she is attending <!-- deleted by customization, --> so we will change her RSVP from "N" to "Y":
