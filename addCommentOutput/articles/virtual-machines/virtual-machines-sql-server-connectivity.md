...
>[AZURE.NOTE] It is important to note that when you use this technique to communicate with SQL Server, all data returned is considered outgoing traffic from the datacenter. It is subject to normal [pricing on outbound data transfers](/home/features/data-transfers). This is true even if you use this technique from another machine or cloud service within the same Azure datacenter, because traffic still goes through Azure's public load balancer.

### Connect to SQL Server in the same virtual network

[Virtual <!-- deleted by customization Network](..\virtual-network\virtual-networks-overview.md) --><!-- keep by customization: begin --> Network](/#pricedocumentation/articles/virtual-networks-overview) <!-- keep by customization: end --> enables additional scenarios. You can connect VMs in the same virtual network, even if those VMs exist in different cloud services. And with a [site-to-site <!-- deleted by customization VPN](/#pricedocumentation/articles/vpn-gateway-site-to-site-create) --><!-- keep by customization: begin --> VPN](/documentation/articles/vpn-gateway-site-to-site-create) <!-- keep by customization: end -->, you can create a hybrid architecture that connects VMs with on-premises networks and machines.
...
It is important to review all of the security best practices for SQL Server running on an Azure virtual machine. For more information, see [Security Considerations for SQL Server in Azure Virtual <!-- deleted by customization Machines](/documentation/articles/virtual-machines-sql-server-security-considerations) --><!-- keep by customization: begin --> Machines](https://msdn.microsoft.com/zh-cn/library/azure/dn133147.aspx) <!-- keep by customization: end -->.
