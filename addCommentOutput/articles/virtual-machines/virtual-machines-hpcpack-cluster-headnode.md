...
to create the head node of an HPC cluster by using the Azure Management Portal. This HPC Pack
...
* **Azure virtual network** - As shown in this article, when you deploy the HPC Pack head node by using the Resource Manager deployment model in the Azure Management Portal, you specify or create an Azure virtual network (VNet). You'll need to use the VNet later to add cluster compute node VMs to the HPC cluster, or if you join the head node to an existing Active Directory domain.
...
 These  The following  are high level steps to create an Azure VM for the HPC

Pack head node by using the Resource Manager deployment model in the Azure Management Portal. 
...
2. In the [Azure Management Portal](https://manage.windowsazure.cn), select the HPC Pack 2012 R2 image from the Azure gallery. To do this, click **New** and search the Marketplace for **HPC Pack**. Select **HPC Pack 2012 R2 on Windows Server 2012 R2**.
...
4. Use the portal to configure the settings and create the VM. For detailed steps, follow the tutorial [Create a Windows virtual machine in the Azure Management Portal](/documentation/articles/virtual-machines-windows-tutorial-classic-portal). For a first deployment you can usually accept many default or recommended settings.
...
5. Join the VM to an existing domain forest, or create a new domain forest on the VM itself.
    ...
    * If you created  the VM  use standard Server Manager or Windows PowerShell tools  to install and configure the Active Directory Domain Services role. For detailed steps, see [Install a New Windows Server 2012 Active Directory Forest](https://technet.microsoft.com/zh-cn/library/jj574166.aspx).


...
* You can now work with the head node of your  Windows  HPC  Pack  cluster. For

