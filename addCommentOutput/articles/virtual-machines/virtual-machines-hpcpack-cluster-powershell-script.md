...
```
New-HPCIaaSCluster.ps1 -ConfigFile MyConfigFile.xml -AdminUserName <!-- deleted by customization <username> -AdminPassword <password> -->
...
* The script uses the HPC Pack VM image in the Azure Marketplace to create the cluster head node. The current image is based on Windows Server 2012 R2 Datacenter with HPC Pack 2012 R2 Update <!-- deleted by customization 3 --><!-- keep by customization: begin --> 2 <!-- keep by customization: end --> installed.
...
* For <!-- deleted by customization tutorials --><!-- keep by customization: begin --> a tutorial <!-- keep by customization: end --> that <!-- deleted by customization use --><!-- keep by customization: begin --> uses <!-- keep by customization: end --> the script to create a cluster and run an HPC workload, see [Get started with an HPC Pack cluster in Azure to run Excel and SOA workloads](/documentation/articles/virtual-machines-excel-cluster-hpcpac) <!-- deleted by customization, --><!-- keep by customization: begin --> or <!-- keep by customization: end --> [Run NAMD with Microsoft HPC Pack on Linux compute nodes in Azure](/documentation/articles/virtual-machines-linux-cluster-hpcpack-namd) <!-- deleted by customization, or [Run OpenFOAM with Microsoft HPC Pack on Linux compute nodes in Azure](/documentation/articles/virtual-machines-linux-cluster-hpcpack-openfoam) -->.
