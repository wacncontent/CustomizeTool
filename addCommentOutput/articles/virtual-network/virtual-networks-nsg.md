...
You can associate an NSG to VMs <!-- deleted by customization, NICs -->, and subnets, depending on the deployment model you are using.
...
You can associate different NSGs to a VM <!-- deleted by customization (or NIC, depending on the deployment model) --> and the subnet that a <!-- deleted by customization NIC or --> VM is bound to. When that happens, all network access rules are applied to the traffic in the following order:

- **Inbound traffic**
	1. NSG applied to subnet.
	2. NSG applied to <!-- deleted by customization NIC (Resource Manager) or --> VM (classic).
- **Outbound traffic**
	1. NSG applied to <!-- deleted by customization NIC (Resource Manager) or --> VM (classic).
	3. NSG applied to subnet.

![NSG ACLs](./media/virtual-network-nsg-overview/figure2.png)

>[AZURE.NOTE] Although you can only associate a single NSG to a subnet, VM <!-- deleted by customization, or NIC -->; you can associate the same NSG to as many resources as you want.

## Planning

Before implementing NSGs, you need to answer the questions below:	

1. What types of resources do you want to filter traffic to or from <!-- deleted by customization (NICs in the same VM --><!-- keep by customization: begin --> (VM <!-- keep by customization: end -->, VMs or other resources such as cloud services or application service environments connected to the same subnet, or between resources connected to different subnets)?
...
For more information on planning for network security in Azure, read the [best practices for colud services and network security](/documentation/articles/best-practices-network-security). 
...
|---|---|---|
|Number of NSGs you can associate to a subnet, <!-- deleted by customization VM, or NIC|1|This --><!-- keep by customization: begin --> VM|1|This <!-- keep by customization: end --> means you cannot combine NSGs. Ensure all the rules needed for a given set of resources are included in a single NSG.|
...
### NSG for single VM <!-- deleted by customization (NIC) --> in FrontEnd for RDP from Internet
