...
A cloud service can contain multiple IaaS VMs, or PaaS role instances, all exposed through the same cloud service VIP. You can also assign  [multiple  multiple  VIPs to a cloud  service](/documentation/articles/load-balancer-multivip)  service , which enables multi-VIP scenarios like multi-tenant environment with SSL-based websites.
