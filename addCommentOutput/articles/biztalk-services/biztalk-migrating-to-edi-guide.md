...
Electronic Data Interchange (EDI) is one of the most prevalent means by which businesses exchange data electronically, also termed as Business-to-Business or B2B transactions. BizTalk Server has had EDI support for over a decade, since the initial BizTalk Sever release. With BizTalk Services, Microsoft continues the support for EDI solutions on the <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure platform. B2B transactions are mostly external to an organization, and hence <!-- deleted by customization itâs --><!-- keep by customization: begin --> it’s <!-- keep by customization: end --> easier to implement if it was implemented on a cloud platform. <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure provides this capability through BizTalk Services.
...
This document discusses some of the differences involved with migrating BizTalk Server EDI artifacts to BizTalk Services. This document assumes a working knowledge of BizTalk Server EDI processing and Trading Partner Agreements. For more information on BizTalk Server EDI, see [Trading Partner Management Using BizTalk <!-- deleted by customization Server](https://msdn.microsoft.com/zh-cn/library/bb259970.aspx) --><!-- keep by customization: begin --> Server](https://msdn.microsoft.com/library/bb259970.aspx) <!-- keep by customization: end -->.
...
- In BizTalk Server, after the EDIReceive pipeline processes the EDI message, the message is dumped to a SQL Server database. The EdiSend pipeline then picks up the message from the SQL Server database, processes it, and then sends it out to the trading partner.
	...
	In BizTalk Services, after the EDI receive bridge processes the EDI message, it routes the message to an external process. The external process could be running on <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure or on-premises. The external process should route the message to the EDI send bridge; the send bridge does not inherently pull the message. After processing the message, the EDI send bridge routes the message to the trading partner.

BizTalk Services provides an easy-to-use configuration experience to quickly create and deploy a B2B agreement between trading partners without configuring any <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure Compute instances (Web or Worker roles), any <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure SQL Databases, or any <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure storage accounts. More complex scenarios will require tying in workflows or other service processing "around the edges" of a Trading Partner agreement, that is, before or after Trading Partner Agreement EDI bridge processing. In detail, the following sequences of events occur during an EDI message processing in BizTalk Services.
...
3. The disassembled XML messages could then be received from the endpoint for further custom processing.  These endpoints could be processed by an on-premises component or a <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure Compute instance to further process the message in a Windows Workflow (WF) or Windows Communication Foundation (WCF) service, for example.
...
In BizTalk Server you set up Receive Locations and Receive Ports to receive EDI/XML messages from trading partners, and you set up Send Ports to send EDI/XML messages to trading partner. You then tie up these ports to a trading partner agreement using the BizTalk Server Administration console. In BizTalk Services, the locations where you receive messages from trading partners and where you send messages to trading partners are configured as part of the trading partner agreement itself (as part of Transport Settings) in the BizTalk Services Portal.  So you do not really have the concept of "send ports" and "receive locations", per se, in BizTalk Services. For more information, see [Creating <!-- deleted by customization Agreements](https://msdn.microsoft.com/zh-cn/library/azure/hh689908.aspx) --><!-- keep by customization: begin --> Agreements](https://msdn.microsoft.com/library/windowsazure/hh689908.aspx) <!-- keep by customization: end -->.

## Pipelines (Bridges)

In BizTalk Server EDI, pipelines are message processing entities that can also include custom logic for specific processing capabilities, as required by the application. For BizTalk Services, the equivalent would an EDI bridge. However in BizTalk Services, for now, the EDI bridges are "closed".  That is, you cannot add your own custom activities to an EDI bridge. Any custom processing must be done outside the EDI bridge in your application, either before or after the message enters the bridge configured as part of the Trading Partner agreement. EAI bridges have the option to do custom processing. If you want custom processing, you can use EAI bridges before or after the message is processed by the EDI bridge. For more information, see [How to Include Custom Code in <!-- deleted by customization Bridges](https://msdn.microsoft.com/zh-cn/library/azure/dn232389.aspx) --><!-- keep by customization: begin --> Bridges](https://msdn.microsoft.com/library/azure/dn232389.aspx) <!-- keep by customization: end -->.
...
If you are familiar with the BizTalk Server 2010 Trading Partner Agreements used for EDI processing, then BizTalk Services trading partner agreements look very familiar. Most of the agreement settings are the same and use the same terminology. In some cases, the agreement settings are much simpler compared to the same settings in BizTalk Server. <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure BizTalk Services supports X12, EDIFACT, and AS2 transport.

<!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure BizTalk Services also provides a **TPM Data Migration** tool to migrate trading partners and agreements from BizTalk Server Trading Partner module to BizTalk Services Portal. The TPM Data Migration tool is available as part of a Tools package, which can be downloaded from the [MABS SDK](http://go.microsoft.com/fwlink/p/?LinkId=235057). The package also includes a readme that provides instructions on how to use the tool, and basic troubleshooting information for the tool.
...
<!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure BizTalk Services provides a tool to migrate BizTalk Server maps to BizTalk Services transforms. The **BTMMigrationTool** is available as part of the **Tools** package provided with the BizTalk Services SDK and can be downloaded from [here](http://go.microsoft.com/fwlink/p/?LinkId=235057). For more information about the tool, see [Convert a BizTalk map to a BizTalk Services <!-- deleted by customization Transform](https://msdn.microsoft.com/zh-cn/library/azure/hh949812.aspx) --><!-- keep by customization: begin --> Transform](https://msdn.microsoft.com/library/windowsazure/hh949812.aspx) <!-- keep by customization: end -->.
...
If you need to migrate BizTalk Server orchestration processing to <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure, the orchestrations would need to be rewritten because <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure does not support running BizTalk Server orchestrations.  You could rewrite the orchestration functionality in a Windows Workflow Foundation 4.0 (WF4) service.  This would be a complete rewrite as there is currently no migration from BizTalk Server orchestrations to WF4. Here are some resources for Windows Workflow:

- *How to integrate a WCF Workflow Service with Service Bus Queues and Topics* by Paolo Salvatori. See <!-- deleted by customization [here](https://msdn.microsoft.com/zh-cn/library/azure/hh709041.aspx) --><!-- keep by customization: begin --> [here](https://msdn.microsoft.com/library/azure/hh709041.aspx) <!-- keep by customization: end -->.
...
- *Windows Workflow Foundation 4 (WF4) documentation on MSDN*. See <!-- deleted by customization [here](https://msdn.microsoft.com/zh-cn/library/dd489441(v=vs.100).aspx) --><!-- keep by customization: begin --> [here](https://msdn.microsoft.com/library/dd489441(v=vs.100).aspx) <!-- keep by customization: end -->.
...
BizTalk Server EDI processing has the concept of "Fallback Agreements".  BizTalk Services does **not** have a Fallback Agreement concept so far.  See BizTalk documentation topics [The Role of Agreements in EDI Processing](http://go.microsoft.com/fwlink/p/?LinkId=237317) and [Configuring Global or Fallback Agreement <!-- deleted by customization Properties](https://msdn.microsoft.com/zh-cn/library/bb245981.aspx) --><!-- keep by customization: begin --> Properties](https://msdn.microsoft.com/library/bb245981.aspx) <!-- keep by customization: end --> for information on how Fallback Agreements are used in BizTalk Server.
...
<!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure BizTalk Services is updated at regular milestones to add more features and capabilities. With each update, we look forward to supporting increased functionality to facilitate creating end-to-end solutions using BizTalk Services and other Azure technologies.
