...
> [AZURE.NOTE] This feature only captures logs for management operations on BizTalk Services, such as when the service was started, backed up, and so on. Such operations are tracked irrespective of whether they are performed from the Azure Management Portal or by using the [BizTalk Service REST <!-- deleted by customization APIs](http://msdn.microsoft.com/zh-cn/library/azure/dn232347.aspx) --><!-- keep by customization: begin --> APIs](http://msdn.microsoft.com/library/azure/dn232347.aspx) <!-- keep by customization: end -->. For a complete list of operations that are tracked using Management Services, see [Operations Tracked Using Azure Management Services](#bizops).<br/><br/>
This does not capture the logs for activities related to BizTalk Service runtime (such as message processed by bridges, and so on.). To view these logs, use the Tracking view from the BizTalk Services portal. For more information, see [Tracking <!-- deleted by customization Messages](http://msdn.microsoft.com/zh-cn/library/azure/hh949805.aspx) --><!-- keep by customization: begin --> Messages](http://msdn.microsoft.com/library/azure/hh949805.aspx) <!-- keep by customization: end -->.
