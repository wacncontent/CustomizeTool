...
The primary enclosure Power and Cooling Module (PCM) on your <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure StorSimple device has an additional battery pack. This pack provides power so that the StorSimple device can save data if there is loss of AC power to the primary enclosure. This battery pack is referred to as the *backup battery module*. The backup battery module exists only for the primary enclosure in your StorSimple device (the EBOD enclosure does not contain a backup battery module).
...
>[AZURE.IMPORTANT] Before removing and replacing a backup battery module, review the safety information in the [Introduction to StorSimple hardware component <!-- deleted by customization replacement](/documentation/articles/storsimple-hardware-component-replacement) --><!-- keep by customization: begin --> replacement](storsimple-hardware-component-replacement.md) <!-- keep by customization: end -->.

## Remove the backup battery module

The backup battery module for your <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure StorSimple device is a field-replaceable unit. Before it is installed in the PCM, the battery module should be stored in its original packaging.
...
3. To remove the PCM with a failed battery, follow the steps in [Remove a <!-- deleted by customization PCM](/documentation/articles/storsimple-power-cooling-module-replacement#remove-a-pcm) --><!-- keep by customization: begin --> PCM](storsimple-power-cooling-module-replacement.md#remove-a-pcm) <!-- keep by customization: end -->.
...
- The system may have experienced a recent power loss or the batteries may be undergoing periodic maintenance. Observe the system for 12 hours before proceeding.
    ...
    - If the state is still **DEGRADED** after 12 hours of continuous connection to AC power with the controllers and PCMs running, then the battery needs to be replaced. Please [contact Microsoft <!-- deleted by customization Support](/documentation/articles/storsimple-contact-microsoft-support) --><!-- keep by customization: begin --> Support](storsimple-contact-microsoft-support.md) <!-- keep by customization: end --> for a replacement backup battery module.

    - If the state becomes OK after 12 hours, the battery is operational, and it only needed a maintenance charge.

- If there has not been an associated loss of AC power and the PCM is turned on and connected to AC power, the battery needs to be replaced. [Contact Microsoft <!-- deleted by customization Support](/documentation/articles/storsimple-contact-microsoft-support) --><!-- keep by customization: begin --> Support](storsimple-contact-microsoft-support.md) <!-- keep by customization: end --> to order a replacement backup battery module.
