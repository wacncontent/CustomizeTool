<properties 
   pageTitle="Use the StorSimple Manager device dashboard | Windows Azure"
...
- **Chart area** <!-- deleted by customization - --><!-- keep by customization: begin --> – <!-- keep by customization: end --> You can see the relevant storage metrics in the chart area at the top of the dashboard. In this chart, you can view metrics for the total primary storage (the amount of data written by hosts to your device) and the total cloud storage consumed by your device over a period of time.
     ...
     For more information, see [Use the StorSimple Manager service to monitor your StorSimple <!-- deleted by customization device](/documentation/articles/storsimple-monitor-device) --><!-- keep by customization: begin --> device](storsimple-monitor-device.md) <!-- keep by customization: end -->.

- **Usage overview** <!-- deleted by customization - --><!-- keep by customization: begin --> – <!-- keep by customization: end --> In the **usage overview** area, you can see the amount of primary storage used, the amount of provisioned storage, and the maximum storage capacity for your device. By comparing these usage numbers to the maximum amount of storage that is available, you can see at a glance if you need to obtain additional storage. Note that this overview is updated every 15 minutes and, because of the difference in update frequency, may show different numbers than those shown in the chart area above, which is updated daily. For more information, see [Use the StorSimple Manager service to monitor your StorSimple <!-- deleted by customization device](/documentation/articles/storsimple-monitor-device) --><!-- keep by customization: begin --> device](storsimple-monitor-device.md) <!-- keep by customization: end -->.


- **Alerts** <!-- deleted by customization - --><!-- keep by customization: begin --> – <!-- keep by customization: end --> The **alerts** area contains an overview of the alerts for your device. Alerts are grouped by severity, and a count is provided of the number of alerts at each severity level. Clicking the alert severity opens a scoped view of the alerts tab to show you only the alerts of that severity level for this device.

- **Jobs** <!-- deleted by customization - --><!-- keep by customization: begin --> – <!-- keep by customization: end --> The **jobs** area shows you the outcome of recent job activity. This can assure you that the system is operating as expected, or it can let you know that you need to take corrective action. To see more information about recently completed jobs, click **Jobs succeeded in the last 24 hours**.
...
- Learn more about the [StorSimple Manager service <!-- deleted by customization dashboard](/documentation/articles/storsimple-service-dashboard) --><!-- keep by customization: begin --> dashboard](storsimple-service-dashboard.md) <!-- keep by customization: end -->.
