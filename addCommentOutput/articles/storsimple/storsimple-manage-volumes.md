...
You [created a <!-- deleted by customization volume](/documentation/articles/storsimple-deployment-walkthrough-u1#step-6-create-a-volume) --><!-- keep by customization: begin --> volume](/documentation/articles/storsimple-deployment-walkthrough#step-6:-create-a-volume) <!-- keep by customization: end --> during deployment of your StorSimple solution. Adding a volume is a similar procedure.
...
4. In the Add a volume wizard, under **Basic Settings**, do the following:
  ...
  3. From the drop-down list, select the **Usage Type** for your volume. Choose **Archival Volume** if you are working with less frequently accessed archival data. For all other use cases, select <!-- deleted by customization **Tiered --><!-- keep by customization: begin --> **Primary <!-- keep by customization: end --> Volume**. <!-- deleted by customization (Tiered volumes were formerly called primary volumes.) -->
  5. Click the arrow icon <!-- deleted by customization ![Arrow icon](./media/storsimple-manage-volumes/HCS_ArrowIcon.png)to --><!-- keep by customization: begin --> ![arrow-icon](./media/storsimple-manage-volumes/HCS_ArrowIcon.png)to <!-- keep by customization: end --> go to the **Additional Settings** page.
...
5. Under **Additional Settings**, add a new access control record (ACR):
  ...
  2. Under **Default backup for this volume**, we recommend that you enable a default backup by selecting the **Enable** check box.
   3. Click the check icon <!-- deleted by customization ![Check icon](./media/storsimple-manage-volumes/HCS_CheckIcon.png) --><!-- keep by customization: begin --> ![check-icon](./media/storsimple-manage-volumes/HCS_CheckIcon.png) <!-- keep by customization: end --> to create the volume with the specified settings.
...
> [AZURE.IMPORTANT] <!-- keep by customization: begin --> If you modify the volume size on the device, the volume size needs to be changed on the host as well. Refer to your host operating system instructions when modifying the volume on the host. <!-- keep by customization: end -->
<!-- deleted by customization > -->
<!-- deleted by customization
...
3. All the volumes associated with this container are displayed in a tabular format. Check the status of the volume you want to delete. If the volume you want to delete is not offline, take it offline first, following the steps in [Take a volume <!-- deleted by customization offline](#take-a-volume-offline) --><!-- keep by customization: begin --> offline](#Take-a-volume-offline) <!-- keep by customization: end -->.
...
<!-- deleted by customization - --> Learn how to [clone a StorSimple volume](/documentation/articles/storsimple-clone-volume).
<!-- deleted by customization
