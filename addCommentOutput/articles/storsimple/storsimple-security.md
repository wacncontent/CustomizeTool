...
<!-- deleted by customization # StorSimple security and data protection -->
...
Because the StorSimple Manager service is hosted in Azure, it is protected by the Azure security features. For more information about the security features provided by Windows Azure, go to the [Windows Azure Trust Center](/support/trust-center/security/).
...
To learn how get a service registration key, go to [Step 2: Get the service registration key](/documentation/articles/storsimple-deployment-walkthrough#step-2-get-the-service-registration-key).
...
Passwords are an important aspect of computer security and are used extensively in the <!-- keep by customization: begin --> Windows Azure <!-- keep by customization: end --> StorSimple solution to help ensure that your data is accessible to authorized users only. StorSimple allows you to configure the following passwords:
...
CHAP is an authentication scheme used by the StorSimple device to validate the identity of remote clients. The verification is based on a shared password. CHAP can be one-way (unidirectional) or mutual (bidirectional). With one-way CHAP, the target (the StorSimple device) authenticates an initiator (host). Mutual <!-- deleted by customization or reverse --> CHAP requires that the target authenticate the initiator and then the initiator authenticate the target. <!-- deleted by customization Your --><!-- keep by customization: begin --> Azure <!-- keep by customization: end --> StorSimple can be configured to use either method.
...
For more information about CHAP and how to configure it for your StorSimple solution, go to [Configure CHAP for your StorSimple device](/documentation/articles/storsimple-configure-chap).
-->
<!-- keep by customization: begin -->
> [AZURE.NOTE] 
> 
...
When you configure a device to use <!-- keep by customization: begin --> the <!-- keep by customization: end --> StorSimple Snapshot Manager, you will be required to provide the StorSimple Snapshot Manager password. This password is first set in <!-- keep by customization: begin --> the <!-- keep by customization: end --> Windows PowerShell for StorSimple during registration. The password can also be set and changed from the StorSimple Manager service. This password authenticates the device with StorSimple Snapshot Manager.
...
For more information about StorSimple Snapshot Manager, go to [What is StorSimple Snapshot Manager?](/documentation/articles/storsimple-what-is-snapshot-manager)
-->
<!-- keep by customization: begin -->
...
We recommend that you use the following guidelines to help ensure that <!-- keep by customization: begin --> Azure <!-- keep by customization: end --> StorSimple passwords are strong and well-protected:
...
This section describes the <!-- keep by customization: begin --> Azure <!-- keep by customization: end --> StorSimple security features that protect <!-- deleted by customization data in transit --><!-- keep by customization: begin --> in-transit <!-- keep by customization: end --> and stored data.

As described in other sections, passwords are used to authorize and authenticate users before they can gain access to your <!-- keep by customization: begin --> Windows Azure <!-- keep by customization: end --> StorSimple solution. Another security consideration is protecting data from unauthorized users while it is being transferred between storage systems and while it is being stored. The following sections describe the data protection features provided with <!-- keep by customization: begin --> Azure <!-- keep by customization: end --> StorSimple.
...
The primary purpose of the StorSimple Manager service is to manage and configure the StorSimple device. The StorSimple Manager service runs in Windows Azure. You use the Azure Management Portal to enter device configuration data, and then Windows Azure uses the StorSimple Manager service to send the data to the device. <!-- keep by customization: begin --> The <!-- keep by customization: end --> StorSimple <!-- keep by customization: begin --> Manager service <!-- keep by customization: end --> uses a system of asymmetric key pairs to help ensure that a compromise of the Azure service will not result in a compromise of stored information. <!-- keep by customization: begin --> The asymmetric key system helps protect the data that flows through the service as follows: <!-- keep by customization: end -->
<!-- deleted by customization
...
2. The data encryption certificate keys are exported into a Personal Information Exchange (.pfx) file that is protected by the service data encryption key, which is a strong 128-bit key that is randomly generated by the <!-- deleted by customization first --> device during registration.
...
The StorSimple device manages data by storing it in tiers locally and in the cloud, depending on frequency of use. All host machines that are connected to the device send data to the device, which then moves data to the cloud, as appropriate. Data is transferred from the device to the cloud <!-- deleted by customization securely --> over the <!-- deleted by customization Internet --><!-- keep by customization: begin --> iSCSI protocol <!-- keep by customization: end -->. Each device has one iSCSI target that surfaces all shared volumes on that device. All data is encrypted before it is sent to cloud storage. <!-- keep by customization: begin --> To help ensure the security and integrity of data moved to the cloud, Azure StorSimple allows you to define cloud storage encryption keys as follows: <!-- keep by customization: end -->
<!-- deleted by customization
...
<!-- deleted by customization
- [Change the service data encryption key](/documentation/articles/storsimple-service-dashboard#change-the-service-data-encryption-key)
- [Key rotation of storage accounts](/documentation/articles/storsimple-manage-storage-accounts#key-rotation-of-storage-accounts)
-->
<!-- keep by customization: begin -->
...
- [View or regenerate the service registration <!-- deleted by customization key](/documentation/articles/storsimple-service-dashboard#view-or-regenerate-the-service-registration-key) --><!-- keep by customization: begin --> key](https://msdn.microsoft.com/zh-CN/library/azure/8158cbe9-1f26-4513-a031-49f88bb3d481#BKMK_viewsrk) <!-- keep by customization: end -->

**Q:** I lost my service data encryption key. What do I do?

**A:** Contact Microsoft Support. They can log on to a support session on your device and help you retrieve the key <!-- deleted by customization (provided at least one device is online) -->. Immediately after you obtain the service data encryption key, you should change it to ensure that the new key is known only to you. For instructions, go to:
...
<!-- deleted by customization
- [Use the StorSimple Manager service to change your storsimple passwords](/documentation/articles/storsimple-change-passwords)
- [Change the service data encryption key](/documentation/articles/storsimple-service-dashboard#change-the-service-data-encryption-key)
- [Configure CHAP for your StorSimple device](/documentation/articles/storsimple-configure-chap)
-->
<!-- keep by customization: begin -->
...
- [Authenticate a device](/documentation/articles/storsimple-snapshot-manager-manage-devices#authenticate-a-device)
-->
<!-- keep by customization: begin -->
...
<!-- deleted by customization
[Deploy your StorSimple device](/documentation/articles/storsimple-deployment-walkthrough).
