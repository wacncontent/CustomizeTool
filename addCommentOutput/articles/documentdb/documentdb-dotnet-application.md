<properties 
	pageTitle="web site Development with ASP.NET MVC using DocumentDB | Windows Azure" 
	description="Learn how to use DocumentDB with .NET to build a To Do List web site. You'll store and access data from an ASP.NET MVC web site hosted on Azure Websites." 
	keywords="Github, visual studio, web site development, application development, database tutorial, mvc applications, json data, documentdb, azure, Microsoft azure"
	services="documentdb" 
...
To highlight how you can efficiently leverage Azure DocumentDB to store and query JSON documents, this article provides an end-to-end walk-through showing you how to build a todo list web site using Azure DocumentDB. The tasks will be stored as JSON documents in Azure DocumentDB.

![Screen shot of the todo list web site created by this tutorial](./media/documentdb-dotnet-application/image1.png)
...
2. In the **Project types** pane, expand **Templates**, **Visual C#**, **Web**, and then select **ASP.NET <!-- deleted by customization web site** --><!-- keep by customization: begin --> Web Application** <!-- keep by customization: end -->.

  	![Screen shot of the New Project dialog box with the ASP.NET <!-- deleted by customization web site --><!-- keep by customization: begin --> Web Application <!-- keep by customization: end --> project type highlighted](./media/documentdb-dotnet-application/image10.png)
...
<!-- deleted by customization
	For more information about choosing an **Azure Websites plan** and **Resource group**, see [Azure Websites plans in-depth overview](/documentation/articles/azure-web-sites-web-hosting-plans-in-depth-overview).
...
## <a name="_Toc395637767"></a>Step 3: Add DocumentDB to your web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end --> project

Now that we have most of the ASP.NET MVC plumbing that we need for
this solution, let's get to the real purpose of this tutorial, adding Azure DocumentDB to our web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end -->.
...
1. The DocumentDB .NET SDK is packaged and distributed as a NuGet package. To get the NuGet package in Visual Studio, use the NuGet package manager in Visual Studio by right-clicking on the project in **Solution Explorer** and then clicking **Manage NuGet Packages**.
  	...
  	![Sreen shot of the right-click options for the web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end --> project in Solution Explorer, with Manage NuGet Packages highlighted.](./media/documentdb-dotnet-application/image21.png)

    The **Manage NuGet Packages** dialog box appears.

2. In the **Search Online** box, type ***Azure DocumentDB***. 
    
    From the results, install the <!-- deleted by customization **Windows --><!-- keep by customization: begin --> **Microsoft <!-- keep by customization: end --> Azure DocumentDB Client Library** package. This will download and install the DocumentDB package as well as all dependencies, like Newtonsoft.Json.

  	![Sreen shot of the Manage NuGet Packages window, with the <!-- deleted by customization Windows --><!-- keep by customization: begin --> Microsoft <!-- keep by customization: end --> Azure DocumentDB Client Library highlighted](./media/documentdb-dotnet-application/nuget.png)
...
### <a name="_Toc395637770"></a>Listing incomplete Items in your web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end -->
...
![Screen shot of the todo list web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end --> created by this database tutorial](./media/documentdb-dotnet-application/image23.png)
...
2. Add the following to the **ItemController** class.
	...
	The first method handles the Http GET that happens when the user clicks on the **Edit** link from the **Index** view. This method fetches a <!-- deleted by customization [**Document**](http://msdn.microsoft.com/zh-cn/library/azure/microsoft.azure.documents.document.aspx) --><!-- keep by customization: begin --> [**Document**](http://msdn.microsoft.com/library/azure/microsoft.azure.documents.document.aspx) <!-- keep by customization: end --> from DocumentDB and passes it to the **Edit** view.
...
1. Hit F5 in Visual Studio to build the application in debug mode. It should build the application and launch a browser with the empty grid page we saw before:
	...
	![Screen shot of the todo list web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end --> created by this database tutorial](./media/documentdb-dotnet-application/image24.png)
...
Now that you have the complete application working correctly with DocumentDB we're going to deploy this web <!-- deleted by customization site --><!-- keep by customization: begin --> app <!-- keep by customization: end --> to Azure Websites. If you selected **Host in the cloud** when you created the empty ASP.NET MVC project then Visual Studio makes this really easy and does most of the work for you.
...
In a few seconds, Visual Studio will finish publishing your web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end --> and launch a browser where you can see your handy work running in Azure!
...
To add additional functionality to your application, review the APIs available in the [Document DB .NET <!-- deleted by customization Library](http://msdn.microsoft.com/zh-cn/library/azure/dn783362.aspx) --><!-- keep by customization: begin --> Library](http://msdn.microsoft.com/library/azure/dn783362.aspx) <!-- keep by customization: end --> and feel free to contribute to the DocumentDB .NET Library on [GitHub][].
