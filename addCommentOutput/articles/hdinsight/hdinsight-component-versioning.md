...
When creating a cluster through the HDInsight Windows PowerShell cmdlets or the HDInsight .NET SDK, you can choose the version for the HDInsight Hadoop cluster by using the  **HDInsight Version**  **Version**   drop-down on the **Optional Configuration** blade in the Azure Management Portal  parameter .
...
- **Cluster Dashboard** - A new web  application  site  that is deployed to your HDInsight cluster. Use it to run Hive queries, check job logs, and browse Azure Blob storage. The URL used to access the web  application  site  is <*ClusterName*>.azurehdinsight.cn.
