...
When creating a cluster through the HDInsight Windows PowerShell cmdlets or the HDInsight .NET SDK, you can choose the version for the HDInsight Hadoop cluster by using the <!-- deleted by customization **HDInsight Version** --><!-- keep by customization: begin --> **Version** <!-- keep by customization: end --> drop-down on the **Optional Configuration** blade in the Azure Management Portal.
...
- **Cluster Dashboard** - A new web <!-- deleted by customization application --><!-- keep by customization: begin --> site <!-- keep by customization: end --> that is deployed to your HDInsight cluster. Use it to run Hive queries, check job logs, and browse Azure Blob storage. The URL used to access the web <!-- deleted by customization application --><!-- keep by customization: begin --> site <!-- keep by customization: end --> is <*ClusterName*>.azurehdinsight.cn.
