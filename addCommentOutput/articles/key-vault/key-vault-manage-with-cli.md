<properties
	pageTitle="Manage Key Vault using CLI | Windows Azure"
...
>
>Currently, you cannot configure Azure Key Vault in the Azure Management Portal. Instead, use these Cross-Platform Command-Line Interface  instructions. Or, for Azure PowerShell instructions, see [this equivalent tutorial](/documentation/articles/key-vault-get-started).
...
- A subscription to Windows Azure. If you do not have one, you can sign up for a <!-- deleted by customization [trial](../../../pricing/1rmb-trial) --><!-- keep by customization: begin --> [trial](/pricing/1rmb-trial) <!-- keep by customization: end -->.
...
- [Using Azure Cross-Platform Command-Line Interface with Azure Resource Manager](/documentation/articles/xplat-cli-azure-resource-manager)
...
5.	Finally, log out of the Azure Management Portal and then log back in using the new organizational account. If this is the first time logging in with this account, you will be prompted to change the password.

For more information about using an organizational account with Windows Azure, see [Sign up for Windows Azure as an Organization](/documentation/articles/sign-up-organization).
...
5.	On the **Tell us about your application** page, specify a name for your application and select <!-- deleted by customization **WEB APPLICATION --><!-- keep by customization: begin --> **Web Site <!-- keep by customization: end --> AND/OR WEB API** (the default). Click the Next icon.
...
When you create the keyvault, add the 'sku' parameter:
    ...
    azure azure keyvault create --vault-name 'ContosoKeyVaultHSM' --resource-group 'ContosoResourceGroup' --location 'China East' --sku 'Premium'
...
The next command imports a âbring your own key" (BYOK) package. This lets you generate your key in your local HSM, and transfer it to HSMs in the Key Vault service, without the key leaving the HSM boundary:

    azure keyvault key import --vault-name 'ContosoKeyVaultHSM' --key-name 'ContosoFirstHSMKey' --byok-file './ITByok.byok' --destination 'HSM'

For more detailed instructions about how to generate this BYOK package, see [How to use HSM-Protected Keys with Azure Key Vault](/documentation/articles/key-vault-hsm-protected-keys).
