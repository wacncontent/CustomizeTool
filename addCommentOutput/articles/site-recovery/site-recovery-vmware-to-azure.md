...
If you run into problems, post your questions on the [Azure Recovery Services <!-- deleted by customization Forum](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr) --><!-- keep by customization: begin --> Forum](https://social.msdn.microsoft.com/forums/zh-cn/home?forum=hypervrecovmgr) <!-- keep by customization: end -->.
...
<!-- deleted by customization Note that the --><!-- keep by customization: begin --> **Note:** The <!-- keep by customization: end --> first four IP addresses in any subnet are reserved for internal Azure usage. Specify any other available IP address.
...
<!-- keep by customization: end -->
    ...
    <!-- deleted by customization >[AZURE.WARNING] --><!-- keep by customization: begin --> **Note:** <!-- keep by customization: end --> Don't delete or change the public or private port number of any of the endpoints created during the master target server deployment.
...
6.  Log onto the server to complete installation and register it with the configuration server. <!-- keep by customization: begin --> If you're running Windows: <!-- keep by customization: end -->
<!-- deleted by customization
...
-->
	...
	2. In **Host Agent Config** specify the internal IP address of the configuration server and port 443. You can use the internal address and private port 443 even if you're not connecting over VPN <!-- keep by customization: begin --> mode <!-- keep by customization: end --> because the virtual machine is attached to the same Azure network as the configuration server. Leave **Use HTTPS** enabled. Enter the passphrase for the configuration server that you noted earlier. Click **OK** to register server. Note that you can ignore the NAT options on the page. They're not used.
<!-- deleted by customization
...
2. <!-- deleted by customization  --> Copy the downloaded zip file to the server on which you're going to install the process server. The zip file contains two installation files:
...
4. Run the <!-- deleted by customization **Microsoft-ASR_CX_TP_8.4.0.0_Windows*** --><!-- keep by customization: begin --> **Microsoft-ASR_CX_TP_8.2.0.0_Windows*** <!-- keep by customization: end --> installation file and follow the instructions. This installs third-party components needed for the deployment.
<!-- deleted by customization
...
2. In **Select Virtual Machine** select a protection group and then select machines in the group to add to the recovery plan. <!-- deleted by customization [Read more](/documentation/articles/site-recovery-create-recovery-plans) about recovery plans. -->

	![Add virtual machines](./media/site-recovery-vmware-to-azure/ASRVMWare_RP2.png)

3. If <!-- deleted by customization needed --><!-- keep by customization: begin --> required <!-- keep by customization: end --> you can customize the plan to create groups and sequence the order in which machines in the recovery plan are failed over. You can also add prompts for manual actions and scripts. The scripts when recovering to Azure can be added by using [Azure Automation Runbooks](/documentation/articles/site-recovery-runbook-automation).
...
The complete file may be found on the [Microsoft Download <!-- deleted by customization Center](http://download.microsoft.com/download/2/5/E/25EB85E9-CDD5-46B5-97B7-A86733AFC525/Third-Party_Notices.txt) --><!-- keep by customization: begin --> Center](http://go.microsoft.com/fwlink/?LinkId=530254) <!-- keep by customization: end -->. Microsoft reserves all rights not expressly granted herein, whether by implication, estoppel or otherwise.
