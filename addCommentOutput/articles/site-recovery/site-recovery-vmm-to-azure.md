...
- You'll need a <!-- deleted by customization [Windows Azure](http://azure.microsoft.com/) --><!-- keep by customization: begin --> Azure <!-- keep by customization: end --> account. If you don't have one, start with a <!-- deleted by customization [trial](http://aka.ms/try-azure) --><!-- keep by customization: begin --> [1rmb trial](http://www.windowsazure.cn/pricing/1rmb-trial/) <!-- keep by customization: end -->. In addition you can read about [Azure Site Recovery Manager <!-- deleted by customization pricing](/home/features/site-recovery/#price) --><!-- keep by customization: begin --> pricing](/home/features/site-recovery/#home_rec_pri) <!-- keep by customization: end -->.
- You'll need an Azure storage account to store data replicated to Azure. The account needs geo-replication enabled. It should be in the same region as the Azure Site Recovery service, and be associated with the same subscription. To learn more about setting up Azure storage, see [Introduction to <!-- deleted by customization Windows --> Azure <!-- deleted by customization Storage](https://azure.microsoft.com/zh-cn/documentation/articles/storage-introduction/) --><!-- keep by customization: begin --> Storage](/documentation/articles/storage-introduction/) <!-- keep by customization: end -->.
- You'll need to make sure that virtual machines you want to protect comply with Azure requirements. See [Virtual machine <!-- deleted by customization support](https://msdn.microsoft.com/zh-cn/library/azure/dn469078.aspx#BKMK_E2A) --><!-- keep by customization: begin --> support](https://msdn.microsoft.com/zh-CN/library/azure/dn469078.aspx#BKMK_E2A) <!-- keep by customization: end --> for details.
...
5. In *Region*<!-- keep by customization: begin --> <b>Region</b> <!-- keep by customization: end -->, select the geographic region for the vault. Available geographic regions include East Asia, West Europe, China North, China East, China North, Southeast Asia.
...
5. In <!-- keep by customization: begin --> <b>Region</b> <!-- keep by customization: end -->, select the geographic region for the vault. Available geographic regions include East Asia, West Europe, China North, China East, China North, Southeast Asia.
...
1. In the <!-- deleted by customization *Recovery Services* --><!-- keep by customization: begin --> **Recovery Services** <!-- keep by customization: end --> page, click the vault to open the <!-- deleted by customization Quick Start --><!-- keep by customization: begin --> **Quick Start** <!-- keep by customization: end --> page. <!-- deleted by customization Quick Start --><!-- keep by customization: begin --> **Quick Start** <!-- keep by customization: end --> can also be opened at any time using the icon.

	![Quick Start Icon](./media/site-recovery-vmm-to-azure/ASRE2AVMM_QuickStartIcon.png)

2. In the <!-- deleted by customization dropdown list --><!-- keep by customization: begin --> shortcut menu <!-- keep by customization: end -->, select **Between an on-premises Hyper-V site and Windows Azure**.
...
4. On the <!-- deleted by customization *Quick Start* --><!-- keep by customization: begin --> <b>Quick Start</b> <!-- keep by customization: end --> page, in **Prepare VMM servers**, click <!-- deleted by customization *Download --><!-- keep by customization: begin --> <b>Download <!-- keep by customization: end --> Windows Azure Site Recovery Provider for installation on VMM <!-- deleted by customization servers* --><!-- keep by customization: begin --> servers</b> <!-- keep by customization: end --> to obtain the latest version of the Provider installation file.
...
3. <!-- deleted by customization The Installer does a few --><!-- keep by customization: begin --> In <!-- keep by customization: end --> **Pre-requirements Check** <!-- deleted by customization and requests permission --><!-- keep by customization: begin --> select <!-- keep by customization: end --> to stop the VMM service to begin Provider setup. The <!-- deleted by customization VMM Service --><!-- keep by customization: begin --> service <!-- keep by customization: end --><!-- keep by customization: begin --> stops and <!-- keep by customization: end --> will <!-- deleted by customization be restarted --><!-- keep by customization: begin --> restart <!-- keep by customization: end --> automatically when setup finishes. If you're installing on a VMM cluster you'll be prompted to stop the Cluster role.
<!-- keep by customization: begin -->
...
5. In **Internet Connection** specify how the Provider running on the VMM server connects to the Internet. Select <!-- keep by customization: begin --> <b>Use <!-- keep by customization: end --> default system proxy <!-- keep by customization: begin --> settings</b> <!-- keep by customization: end --> to use the default Internet connection settings configured on the server.
...
If you don't <!-- deleted by customization ave --><!-- keep by customization: begin --> have <!-- keep by customization: end --> an Azure storage account <!-- keep by customization: begin -->, <!-- keep by customization: end --> click **Add an Azure Storage Account**. The account should have geo-replication enabled. It must in the same region as the Azure Site Recovery service, and be associated with the same subscription.
...
1. On the <!-- deleted by customization Quick Start --><!-- keep by customization: begin --> **Quick Start** <!-- keep by customization: end --> page, click **Set up protection for VMM clouds**.
2. On the **Protected Items** tab, click <!-- deleted by customization on --> the cloud you want to configure and go to the **Configuration** tab.
...
5. Set <b>Encrypt stored data</b> to <b>Off</b>. This setting specifies that data should be encrypted <!-- deleted by customization replicated --> between the on-premises site and Azure.
...
Before you begin network mapping verify that virtual machines on the source VMM server are connected to a VM network. In addition <!-- keep by customization: begin -->, <!-- keep by customization: end --> create one or more Azure virtual networks. Note that multiple VM networks can be mapped to a single Azure network.

1. On the <!-- deleted by customization Quick Start --><!-- keep by customization: begin --> **Quick Start** <!-- keep by customization: end --> page, click **Map networks**.
2. On the **Networks** tab, in **Source location**, select the source VMM server. In **Target location** select <!-- deleted by customization Azure --><!-- keep by customization: begin --> **Azure** <!-- keep by customization: end -->.
...
4. Select the source VM network and <!-- keep by customization: begin --> then <!-- keep by customization: end --> click **Map**.
...
After you save the settings a job starts to track the mapping progress and it can be monitored on the <!-- deleted by customization Jobs --><!-- keep by customization: begin --> **Jobs** <!-- keep by customization: end --> tab. Any existing replica virtual machines that correspond to the source VM network will be connected to the target Azure networks. New virtual machines that are connected to the source VM network will be connected to the mapped Azure network after replication. If you modify an existing mapping with a new network, replica virtual machines will be connected using the new settings.

Note that if the target network has multiple subnets and one of those subnets has the same name as <!-- keep by customization: begin --> the <!-- keep by customization: end --> subnet on which the source virtual machine is located, then the replica virtual machine will be connected to that target subnet after failover. If there’s no target subnet with a matching name, the virtual machine will be connected to the first subnet in the network.
...
- Virtual machines must meet Azure requirements. Check these in <a <!-- deleted by customization href="https://msdn.microsoft.com/zh-cn/library/dn469078.aspx">Prerequisites --><!-- keep by customization: begin --> href="https://msdn.microsoft.com/zh-CN/library/dn469078.aspx">Prerequisites <!-- keep by customization: end --> and support</a> in the Planning guide.
...
1. On the **Confirm Test Failover** page select **None** or a specific Azure network.  Note that if you select <!-- deleted by customization None --><!-- keep by customization: begin --> **None** <!-- keep by customization: end --> the test failover will check that the virtual machine replicated correctly to Azure but doesn't check your replication network configuration.
...
1. When the failover reaches the **Complete testing** phase <!-- deleted by customization  -->, click **Complete Test** to finish <!-- deleted by customization up --> the test failover. You can drill down to the **Job** tab to track failover progress and status, and to perform any actions that are needed.
1. After  failover you'll be able to see the virtual machine test replica in the Azure Management Portal. If you’re set up to access virtual machines from your on-premises network you can initiate a Remote Desktop connection to the virtual machine. <!-- deleted by customization Note that: -->
...
<!-- deleted by customization
    ...
    1. After failover <!-- keep by customization: begin -->, <!-- keep by customization: end --> if you use a public IP address to connect to the virtual machine in Azure using Remote Desktop, ensure you don't have any domain policies that prevent you from connecting to a virtual machine using a public address.
-->
<!-- keep by customization: begin -->
...
<p>You can use the <!-- deleted by customization *Jobs* --><!-- keep by customization: begin --> **Jobs** <!-- keep by customization: end --> tab and <!-- deleted by customization *Dashboard* --><!-- keep by customization: begin --> **Dashboard** <!-- keep by customization: end --> to view and monitor the main jobs performed by the Azure Site Recovery vault, including configuring protection for a cloud, enabling and disabling protection for a virtual machine, running a failover (planned, unplanned, or test), and committing an unplanned failover.</p>

<p>From the <!-- deleted by customization *Jobs* --><!-- keep by customization: begin --> <b>Jobs</b> <!-- keep by customization: end --> tab you view jobs, drill down into job details and errors, run job queries to retrieve jobs that match specific criteria, export jobs to Excel, and restart failed jobs.</p>

<p>From the <!-- deleted by customization *Dashboard* --><!-- keep by customization: begin --> <b>Dashboard</b> <!-- keep by customization: end --> you can download the latest versions of Provider and Agent installation files, get configuration information for the vault, see the number of virtual machines that have protection managed by the vault, see recent jobs, manage the vault certificate, and resynchronize virtual machines.</p>

<p>For more information about interacting with jobs and the dashboard, see the <a <!-- deleted by customization href="/documentation/articles/site-recovery-manage-registration-and-protection/">Operations --><!-- keep by customization: begin --> href="https://msdn.microsoft.com/zh-CN/library/dn788906.aspx">Operations <!-- keep by customization: end --> and Monitoring Guide</a>.</p>
...
<!-- keep by customization: end -->
<UL>
<LI>To plan and deploy Azure Site Recovery in a full production environment, see <a <!-- deleted by customization href="https://msdn.microsoft.com/zh-cn/library/azure/dn469074.aspx">Planning --><!-- keep by customization: begin --> href="https://msdn.microsoft.com/zh-CN/library/azure/dn469074.aspx">Planning <!-- keep by customization: end --> Guide for Azure Site Recovery</a> and <a <!-- deleted by customization href="https://msdn.microsoft.com/zh-cn/library/azure/dn168841.aspx">Deployment --><!-- keep by customization: begin --> href="https://msdn.microsoft.com/zh-CN/library/sazure/dn168841.aspx">Deployment <!-- keep by customization: end --> Guide for Azure Site Recovery</a>.</LI>


<LI>For questions, visit the <a <!-- deleted by customization href="https://social.msdn.microsoft.com:443/forums/azure/zh-cn/home?forum=hypervrecovmgr">Azure --><!-- keep by customization: begin --> href="https://social.msdn.microsoft.com/Forums/zh-CN/home?forum=windowsazucezhchs">Azure <!-- keep by customization: end --> Recovery Services Forum</a>.</LI>
</UL>
