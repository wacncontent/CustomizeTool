<properties
   pageTitle="Azure Disk Encryption for Windows and Linux IaaS VMs| Windows Azure"
   description="The paper provides an overview of Windows Azure Disk Encryption for for Windows and Linux IaaS VMs."
...
Azure Disk Encryption is a new capability that lets you encrypt your Windows and Linux IaaS virtual machine disks. Azure Disk Encryption leverages the industry standard [BitLocker](https://technet.microsoft.com/zh-cn/library/cc732774.aspx) feature of Windows and the [DM-Crypt](https://en.wikipedia.org/wiki/Dm-crypt) feature of Linux to provide volume encryption for the OS and the data disks. The solution is integrated with [Azure Key](/documentation/services/key-vault/) Vault to help you control and manage the disk encryption keys and secrets in your key vault subscription, while ensuring that all data in the virtual machine disks are encrypted at rest in your Azure storage.
...
The solution supports the following for IaaS VMs for public preview release when enabled in Windows Azure:

- Integration with Azure Key Vault

- Standard [A, D and G series IaaS VMs](/home/features/virtual-machines/#price)

- Enable encryption on IaaS VMs created using [Azure Resource Manager](/documentation/articles/resource-group-overview) model
...
- The Key Vault secret and key encryption key (KEK) URLs must be versioned. Azure service management enforces this restriction of versioning. See below examples for valid secret and KEK URL:
	...
	- Example of valid secret URL:
		...
		*https://contosovault.vault.chinacloudapi.cn/secrets/BitLockerEncryptionSecretWithKek/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx*

	- Example of valid KRK KEK:

		*https://contosovault.vault.chinacloudapi.cn/keys/diskencryptionkek/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx*
...
- Azure disk encryption does not support port numbers being specified as part of Key Vault secret and KEK URLs. See below examples for supported Key Vault URL:
 	...
 	- Unaccepted Key Vault URL
		...
		*https://contosovault.vault.chinacloudapi.cn:443/secrets/contososecret/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx*

	- Accepted Key Vault URL:

		*https://contosovault.vault.chinacloudapi.cn/secrets/contososecret/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx*

- To enable Azure Disk Encryption feature, the IaaS VMs must meet the following network endpoint configuration requirements: 

	- The IaaS VM must be able to connect to Azure Active Directory endpoint \[Login.chinacloudapi.cn\] to get a token to connect to Azure key vault
...
**Note:** This document assumes that you understand the basic concepts, such as modules, cmdlets, and sessions. For more information, see Getting started with [Windows PowerShell](https://technet.microsoft.com/zh-cn/library/hh857337.aspx)
...
- To run any of the Azure CLI commands and associate it with your Azure subscription, you must first install Azure CLI version:
	...
	- To install Azure CLI and associate it with your Azure subscription, see [How to install and configure Azure CLI](/documentation/articles/xplat-cli-install)

	- Using the Azure CLI for Mac, Linux, and Windows with Azure Resource Manager, see [here](/documentation/articles/azure-cli-arm-commands)

- Azure disk encryption solution use BitLocker external key protector for Windows IaaS VMs. If your VMs are domain joined, do not push any group policies that enforce TPM protectors. Refer to [this article](https://technet.microsoft.com/zh-cn/library/ee706521) for details on the group policy for “Allow BitLocker without a compatible TPM”. 
...
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Azure AD                   | Azure AD is [Azure Active Directory](/documentation/services/identity/). Azure AD account is a pre-requisite for authenticating, storing, and retrieving secrets from the Key Vault.                                                                                                        |
| Azure Key Vault [AKV] | Azure Key Vault is a cryptographic key management service based on FIPS-validated Hardware Security Modules to safeguard your cryptographic keys and sensitive secrets securely.,Refer to [Key Vault](/home/features/key-vault/) documentation for more details.          |
| ARM                   | Azure Resource Manager                                                                                                                                                                                                                       |
| BitLocker             | [BitLocker](https://technet.microsoft.com/zh-cn/library/hh831713.aspx) is an industry recognized Windows volume encryption technology used to enable disk encryption on Windows IaaS VMs                                                                                                                  |
...
| CLI                   | [Azure Command-Line Interface](/documentation/articles/xplat-cli-install)                                                                                                                                                                                                                 |
...
| KEK                   | Key Encryption Key is the asymmetric key (RSA 2048) used to protect or wrap the secret if desired. You can provide an HSM-protected key or software-protected key. For more details, refer to [Azure Key Vault](/home/features/key-vault/) documentation for more details |
| PS cmdlets            | [Azure PowerShell cmdlets](/documentation/articles/powershell-install-configure)                                                                                                                                                                                                                                             |
...
Disk encryption can be enabled on customer encrypted VHD using the PS cmdlets published [here](https://msdn.microsoft.com/zh-cn/library/azure/mt603746.aspx).  
...
Disk encryption can be enabled on customer encrypted VHD using the CLI command installed from [here](/documentation/articles/xplat-cli-install). Follow the steps below to enable encryption on existing/running IaaS Linux VM in Azure using CLI commands:
...
You can get encryption status using Azure Management (Preview) portal, [PowerShell cmdlets](https://msdn.microsoft.com/zh-cn/library/azure/mt622700.aspx) or CLI commands. The sections below will explain how to use the Azure Management (Preview) portal and CLI commands to get the encryption status.

#### Get encryption status of an encrypted IaaS VM using Azure Management Preview portal

You can get the encryption status of the IaaS VM from Azure management preview portal. Logon to Azure preview portal at https://manage.windowsazure.cn/, click on virtual machines link in the left menu to see summary view of the virtual machines in your subscription. You can filter the virtual machines view by selecting the subscription name from the subscription dropdown. Click on columns located at the top of the virtual machines page menu. Select Disk Encryption column from the choose column blade and click update. You should see the disk encryption column showing the encryption state “Enabled” or “Not Enabled” for each VM as shown in the figure below.
...
You can get the encryption status of the IaaS VM from disk encryption PS cmdlet “Get-AzureRmVMDiskEncryptionStatus”. To get the encryption settings for your VM, type in your Azure PowerShell session:
    ...
    OsVolumeEncryptionSettings : {
      "DiskEncryptionKey": {
       SecretUrl":"https://contosovault.vault.chinacloudapi.cn/secrets/BitLockerEncryptionSecretWithKek/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
            "SourceVault": {
...
1.Start an Azure PowerShell session and sign in to your Azure account with the following command:
    ...
    Login-AzureRmAccount <!-- keep by customization: begin --> -Environment $(GetRmAzureEnvironment -Name AzureChinaCloud) <!-- keep by customization: end -->
...
- Use the Azure PowerShell Key Vault cmdlets as described [here](/documentation/articles/key-vault-get-started)
...
If you wish to use a Key Encryption Key (KEK) for an additional layer of security to wrap the BitLocker encryption keys, you should add a KEK to your Key Vault for use in the provisioning process.  Use the [Add-AzureKeyVaultKey](https://msdn.microsoft.com/zh-cn/library/dn868048.aspx) cmdlet to create a new Key Encryption Key in Key Vault. For more details, see [Key Vault documentation](/documentation/services/key-vault/).
...
Azure AD Client ID and secret can also be provisioned using the Azure Service Management Portal at https://manage.windowsazure.cn, follow the steps below to perform this task:
...
In order to execute the commands below you need the Azure AD PowerShell module, which can be obtained from [here](https://technet.microsoft.com/zh-cn/library/jj151815.aspx).
...
Use the [manage-bde](https://technet.microsoft.com/zh-cn/library/ff829849.aspx) command to enable encryption on the boot volume using an external key protector and place the external key (.bek file) on the external drive or volume. Encryption will be enabled on the system/boot volume after the next reboot.
...
The secret can optionally be encrypted with a Key Encryption Key before uploading to Key vault. Use the wrap [API](https://msdn.microsoft.com/zh-cn/library/azure/dn878066.aspx) to first encrypt the secret using the Key Encryption Key. The output of this wrap operation is a base64 URL encoded string which is then uploaded as a secret using the [Set-AzureKeyVaultSecret](https://msdn.microsoft.com/zh-cn/library/dn868050.aspx) cmdlet.
