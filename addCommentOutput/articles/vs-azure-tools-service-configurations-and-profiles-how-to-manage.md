...
1. (Optional) To give the service configuration a different name, choose the new service configuration from the Name list and then click **Rename**. In the **Name** text box, type the name that you want to use for this service configuration and then click **OK**.
    ...
    A new service configuration file that is named <!-- deleted by customization ServiceConfiguration.[New Name].cscfg --><!-- keep by customization: begin --> ServiceConfiguration.<New Name>.cscfg <!-- keep by customization: end --> is added to the Azure project in Solution Explorer.
