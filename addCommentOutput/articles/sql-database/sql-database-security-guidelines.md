<properties
   pageTitle="Azure SQL Database Security Guidelines and Limitations | Windows Azure"
   description="Learn about Windows Azure SQL Database guidelines and limitations related to security."
...
This topic describes the Windows Azure SQL Database guidelines and limitations related to security. Consider the following points when managing the security of your Azure SQL Databases.
...
Before you can connect to the Azure SQL Database server for the first time, you must use the [Azure Management Portal](https://manage.windowsazure.cn) or [Azure Platform Management Portal](https://manage.windowsazure.cn/microsoft.partner.onmschina.cn#Workspaces/All/dashboard) to configure the Azure SQL Database firewall. You will need to create a server-level firewall setting that enables connection attempts from your computer or Azure to Azure SQL Database server. Further, if you want to control access to certain databases in your Azure SQL Database server, create database-level firewall rules for the respective databases. For more information, see [Azure SQL Database firewall](/documentation/articles/sql-database-firewall-configure).
...
To validate certificates with ADO.NET application code, set ``Encrypt=True`` and ``TrustServerCertificate=False`` in the database connection string. For more information, see [Code sample: Retry logic for connecting to Azure SQL Database with ADO.NET](https://msdn.microsoft.com/zh-cn/library/azure/ee336243.aspx).
