...
In order to connect to Windows Azure SQL Database using the logins you create, you must first grant each login database-level permissions by using the ``CREATE USER`` command. For more information, see <!-- deleted by customization the **Granting database access --><!-- keep by customization: begin -->  [Granting Database-Level Permissions <!-- keep by customization: end --> to a <!-- deleted by customization login** section below --><!-- keep by customization: begin --> Login](https://msdn.microsoft.com/zh-cn/library/ee336235.aspx#DatabasePerms) <!-- keep by customization: end -->.
