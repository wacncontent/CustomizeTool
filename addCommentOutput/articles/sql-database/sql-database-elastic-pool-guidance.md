...
If the sum of the DTUs of performance levels for single databases is more than 1.5x the eDTUs needed for the pool, then an elastic pool is more cost effective. For available sizes, see [eDTU and storage limits for elastic database pools and elastic <!-- deleted by customization databases](/documentation/articles/sql-database-elastic-pool-reference#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases) --><!-- keep by customization: begin --> databases](/documentation/articles/sql-database-elastic-pool-reference/#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases) <!-- keep by customization: end -->.
...
2. Select the smallest available eDTU value for the pool that is greater than the estimate from Step 1. For available eDTU choices, see the valid values for eDTUs listed here: [eDTU and storage limits for elastic database pools and elastic <!-- deleted by customization databases](/documentation/articles/sql-database-elastic-pool-reference#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases) --><!-- keep by customization: begin --> databases](/documentation/articles/sql-database-elastic-pool-reference/#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases) <!-- keep by customization: end -->.
...
For available sizes, see [eDTU and storage limits for elastic database pools and elastic <!-- deleted by customization databases](/documentation/articles/sql-database-elastic-pool-reference#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases) --><!-- keep by customization: begin --> databases](/documentation/articles/sql-database-elastic-pool-reference/#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases) <!-- keep by customization: end -->.
...
For details, see [Elastic database pool pricing tier <!-- deleted by customization recommendations](/documentation/articles/sql-database-elastic-pool-portal#elastic-database-pool-pricing-tier-recommendations) --><!-- keep by customization: begin --> recommendations](/documentation/articles/sql-database-elastic-pool-portal/#elastic-database-pool-pricing-tier-recommendations) <!-- keep by customization: end -->
...
- The [SQL Server 2014 feature <!-- deleted by customization pack](https://www.microsoft.com/download/details.aspx?id=42295) --><!-- keep by customization: begin --> pack](http://www.microsoft.com/zh-cn/download/details.aspx?id=42295) <!-- keep by customization: end -->.


### Script details


You can run the script from your local machine or a VM on the cloud. When running it from your local machine, you may incur data egress charges because the script needs to download data from your target databases. Below shows data volume estimation based on number of target databases and duration of running the script. For Azure data transfer costs refer to [Data Transfer Pricing <!-- deleted by customization Details](/home/features/data-transfers/#price) --><!-- keep by customization: begin --> Details](/home/features/data-transfer/#price) <!-- keep by customization: end -->.
