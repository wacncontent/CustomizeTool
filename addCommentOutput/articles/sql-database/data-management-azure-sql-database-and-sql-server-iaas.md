...
* [SQL Server on Azure Virtual <!-- deleted by customization Machines](/home/features/virtual-machines/sql-server/) --><!-- keep by customization: begin --> Machines](/home/features/virtual-machines#SQL/) <!-- keep by customization: end -->: SQL Server installed and hosted in the cloud on Windows Server Virtual Machines (VMs) running on Azure, also known as an infrastructure as a service (IaaS).
...
<tr>
   ...
   <td valign="middle"><ul><li type=round>SQL Server on Azure VMs lets you to set up a high availability and disaster recovery solution for your database's specific needs. Therefore, you can have a system that is highly optimized for your application. You can test and run failovers by yourself when needed. For more information, see [High Availability and Disaster Recovery for SQL Server on Azure Virtual <!-- deleted by customization Machines]((../virtual-machines/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions.md).</ul></td> --><!-- keep by customization: begin --> Machines](/documentation/articles/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions).</ul></td> <!-- keep by customization: end -->
...
With **SQL Database**, the database software is automatically configured, patched, and upgraded by Microsoft, which reduces your administration costs. In addition, its [built-in backup](/documentation/articles/sql-database-business-continuity) capabilities help you achieve significant cost savings, especially when you have a large number of databases. When using SQL Database, you are not billed for individual queries running against SQL Database or incoming Internet traffic, you are billed for [outgoing Internet <!-- deleted by customization traffic](/home/features/data-transfers/#price) --><!-- keep by customization: begin --> traffic](/home/features/data-transfer/#price) <!-- keep by customization: end -->. If your database has high transactional volume and needs to support many concurrent users, we recommend the Premium service tier.
...
- [Virtual Machine Pricing](/home/features/virtual-machines/#price) for [SQL](/home/features/virtual-machines/#price) and for [Windows](/home/features/virtual-machines/#price)
-->
...
> [AZURE.NOTE] There is a small subset of features on SQL Server that are not applicable or not available with SQL Database. See [SQL Database General Limitations and Guidelines](/documentation/articles/sql-database-general-limitations) and [SQL Database Transact-SQL information](/documentation/articles/sql-database-transact-sql-information) for more information. If you are moving an existing SQL Server solution to the cloud, see [Migrating a SQL Server database to Azure SQL Database](/documentation/articles/sql-database-cloud-migrate). When you migrate an existing on-premises SQL Server application to SQL Database, consider updating the application to take advantage of the capabilities that cloud services offer. For example, you may consider using [Azure Web <!-- deleted by customization Azure Websites](/home/features/web-site/) --><!-- keep by customization: begin --> Site Service](/home/features/web-site) <!-- keep by customization: end --> or [Azure Cloud Services](/home/features/cloud-services/) to host your application layer to increase cost benefits.
...
For **SQL Database** Basic, Standard, and Premium service tiers Microsoft provides an availability SLA of 99.99%. For the latest information, see [Service Level <!-- deleted by customization Agreement](https://azure.microsoft.com/support/legal/sla/sql-database/) --><!-- keep by customization: begin --> Agreement](/support/legal/sla/) <!-- keep by customization: end -->. For the latest information on SQL Database service tiers and the supported business continuity plans, see [Service Tiers](/documentation/articles/sql-database-service-tiers).

For **SQL Server running on Azure VMs**, Microsoft provides an availability SLA of 99.95% that covers just the Virtual Machine. This SLA does not cover the processes (such as SQL Server) running on the VM and requires that you host at least two VM instances in an availability set. For the latest information, see the [VM <!-- deleted by customization SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/) --><!-- keep by customization: begin --> SLA](/support/legal/sla/) <!-- keep by customization: end -->. For database high availability (HA) within VMs, you should configure one of the supported high availability options in SQL Server, such as [AlwaysOn Availability Groups](http://blogs.technet.com/b/dataplatforminsider/archive/2014/08/25/sql-server-alwayson-offering-in-microsoft-azure-portal-gallery.aspx).
...
See [Provision a SQL Server virtual machine in Azure](/documentation/articles/virtual-machines-provision-sql-server) to get started.
