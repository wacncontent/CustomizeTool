...
> [AZURE.NOTE] Elastic database pools are currently in preview <!-- keep by customization: begin -->, <!-- keep by customization: end --> and only available with SQL Database V12 servers.


## Step 1: add a pool to a server
-->
<!-- keep by customization: begin -->
This article shows you how to create an elastic database pool with the [Azure preview portal](https://manage.windowsazure.cn).
...
- An Azure SQL Database version 12 server. If you do not have a V12 server, create one following the steps in this article: [Create your first Azure SQL Database](/documentation/articles/sql-database-get-started).
...
An elastic database pool's pricing tier determines the features available to the elastic databases in the pool, and the maximum number of eDTUs (eDTU MAX), and storage (GBs) available to each database. For details, see [eDTU and storage limits for elastic pools and elastic databases](/documentation/articles/sql-database-elastic-pool-reference#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases).
<!-- keep by customization: end -->
...
- The <!-- deleted by customization elastic --> database min/max eDTU settings.
- List of recommended databases.

The service takes the last 30 days of telemetry into account when recommending elastic database pools. Fora database to be considered as a candidate for an elastic database pool it must exist for at least 7 days. Databases that are already in an elastic database pool are not considered as candidates for elastic database pool recommendations.
...
You configure the performance of the pool by setting the performance parameters for both the pool <!-- keep by customization: begin -->, <!-- keep by customization: end --> and the elastic databases in the pool. Keep in mind <!-- keep by customization: begin -->, <!-- keep by customization: end --> that the **Elastic database settings** apply to all databases in the pool.
...
- Select **Create job** and manage the databases in the pool by creating elastic jobs. Elastic jobs <!-- deleted by customization let you run --><!-- keep by customization: begin --> facilitate running <!-- keep by customization: end --> Transact-SQL scripts against any number of databases in the pool. For more information, see [Elastic database jobs overview](/documentation/articles/sql-database-elastic-jobs-overview).
