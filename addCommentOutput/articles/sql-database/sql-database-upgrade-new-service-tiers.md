...
Azure SQL [Web and Business databases are retiring](/documentation/articles/sql-database-web-business-sunset-faq) so it's time to upgrade existing Web or Business databases to the [Basic, Standard, Premium, or Elastic service tiers](/documentation/articles/sql-database-service-tiers).
...
Depending on your specific environment the service may recommend upgrading some or all of your databases into an [elastic database pool](/documentation/articles/sql-database-elastic-pool).

To see the recommended service tiers for your retired databases, you can use the [Azure preview portal](https://manage.windowsazure.cn) or PowerShell. For step-by-step directions see:

- [Upgrade to SQL Database V12 (Azure Preview Portal)](/documentation/articles/sql-database-v12-upgrade)
- [Upgrade to SQL Database V12 (PowerShell)](/documentation/articles/sql-database-upgrade-server)
...
To address this feedback, Azure SQL Database service introduced new database service tiers [(Basic, Standard and <!-- deleted by customization Premium)](/documentation/articles/sql-database-service-tiers) --><!-- keep by customization: begin --> Premium)](http://msdn.microsoft.com/zh-cn/library/dn741340.aspx) <!-- keep by customization: end -->, which offer predictable performance and a wealth of new features for business continuity and security. These new service tiers are designed to provide a specified level of resources for a database workload, regardless of other customer workloads running in that environment. This results in highly predictable performance behavior.
...
For example, consider how long backups need to be retained, or if [Standard or Active <!-- deleted by customization Geo-Replication](/documentation/articles/sql-database-business-continuity) --><!-- keep by customization: begin --> Geo-Replication](http://msdn.microsoft.com/zh-cn/library/azure/dn783447.aspx) <!-- keep by customization: end --> features are needed, or the overall maximum database size needed, etc. These requirements determine your minimum service tier choice.
...
|:--|:--|
<!-- deleted by customization
|[Azure SQL Database Service Tiers and Performance Levels](/documentation/articles/sql-database-service-tiers)| Overview, metrics, and capabilities for each service tier (and how to monitor database utilization in the management portal and by using DMVs). |
|[Azure SQL Database Business Continuity](/documentation/articles/sql-database-business-continuity)|Details of the business continuity and disaster recovery features (point-in-time restore, geo-restore, geo-replication) available for the different service tiers.|
-->
<!-- keep by customization: begin -->
...
Use the <!-- deleted by customization Azure preview --><!-- keep by customization: begin --> management <!-- keep by customization: end --> portal for a high-level overview of DTU percentage usage, and then drill into the details using system views.

You can also use the <!-- keep by customization: begin --> new <!-- keep by customization: end --> Azure <!-- deleted by customization preview --><!-- keep by customization: begin --> management <!-- keep by customization: end --> portal to view the recommended service tier for your Web or Business database when you upgrade a server to Azure SQL Database V12 <!-- keep by customization: begin --> ([at preview in some regions](/documentation/articles/sql-database-preview-whats-new#V12AzureSqlDbPreviewGaTable)) <!-- keep by customization: end -->.

### How to view the recommended service tier in the <!-- keep by customization: begin --> new <!-- keep by customization: end --> Azure <!-- deleted by customization Preview --><!-- keep by customization: begin --> Management <!-- keep by customization: end --> Portal
The <!-- deleted by customization Azure Management Portal --><!-- keep by customization: begin --> management portal <!-- keep by customization: end --> recommends the appropriate service tier for your Web or Business database during the process of upgrading a server to <!-- keep by customization: begin --> Azure <!-- keep by customization: end --> SQL Database V12. The recommendation is based on a historical analysis of the resource consumption of the database.

**New Management Portal**

1. Log on to the <!-- deleted by customization [Azure preview --><!-- keep by customization: begin --> [new management <!-- keep by customization: end --> portal](https://manage.windowsazure.cn) and <!-- deleted by customization browse --><!-- keep by customization: begin --> navigate <!-- keep by customization: end --> to a server containing a Web or Business database.
...
DTU information is available in the current Azure Management Portal.
...
| Article | Description |
|:--|:--|
| [Azure SQL Database Performance Guidance](http://msdn.microsoft.com/zh-cn/library/dn369873.aspx) | The 'Tuning Your Application' section provides detailed recommendations and techniques for performance tuning an Azure SQL Database.|
|[Performance Monitoring and Tuning Tools](https://msdn.microsoft.com/zh-cn/library/ms179428.aspx)| Links and descriptions of the available tools for monitoring events in SQL Server and for tuning the physical database design.|
|[Monitor and Tune for Performance](https://msdn.microsoft.com/zh-cn/library/ms189081.aspx)|SQL Server 2014 performance tuning section on MSDN.|
| [Troubleshooting Performance Problems in SQL Server 2008](https://msdn.microsoft.com/zh-cn/library/dd672789.aspx) | Older but still relevant whitepaper providing guidance for troubleshooting common performance problems, including great information on troubleshooting memory and cpu bottlenecks.|
...
| [Azure PowerShell](http://msdn.microsoft.com/zh-cn/library/azure/dn546726.aspx) | use the <!-- deleted by customization [Set-AzureRMSqlDatabase](https://msdn.microsoft.com/zh-cn/library/azure/mt619433.aspx) --><!-- keep by customization: begin --> [Set-AzureSqlDatabase](http://msdn.microsoft.com/zh-cn/library/azure/dn546732.aspx) <!-- keep by customization: end --> cmdlet. |
<!-- deleted by customization
| [REST API](https://msdn.microsoft.com/zh-cn/library/azure/mt163571.aspx) | use the [Create or Update Database](https://msdn.microsoft.com/zh-cn/library/azure/mt163685.aspx) command.|
| [Transact-SQL](http://msdn.microsoft.com/zh-cn/library/azure/bb510741.aspx) | use the [ALTER DATABASE (Transact-SQL)](http://msdn.microsoft.com/zh-cn/library/azure/ms174269.aspx) statement. |

For details, see [Changing Database Service Tiers and Performance Levels](/documentation/articles/sql-database-scale-up)
-->
<!-- keep by customization: begin -->
...
- **Alerts:** Set up 'Alerts' in the Azure Management Portal to notify you when the DTU consumption for an upgraded database approaches certain high level. Database alerts can be setup in the Azure Management Portal for various performance metrics like DTU, CPU, IO, and Log. 
	...
	For example, you can set up an email alert on “DTU Percentage” if the average DTU percentage value exceeds 75% over the last 5 minutes. Refer to <!-- deleted by customization [Receive alert notifications](/documentation/articles/insights-receive-alert-notifications) --><!-- keep by customization: begin --> [How to: Receive Alert Notifications and Manage Alert Rules in Azure](http://msdn.microsoft.com/zh-cn/library/azure/dn306638.aspx) <!-- keep by customization: end --> to learn more about how to configure alert notifications.


- **Scheduled performance level upgrade/downgrade:** If your application has specific scenarios that require more performance only at certain times of the day/week, you can use [Azure <!-- deleted by customization Automation](/documentation/services/automation/) --><!-- keep by customization: begin --> Automation](http://msdn.microsoft.com/zh-cn/library/azure/dn643629.aspx) <!-- keep by customization: end --> to upsize/downsize your database to a higher/lower performance level as a planned operation.
...
## Summary
Azure SQL Database service provides telemetry data and tools to evaluate your Web/Business database workloads and determine the best service tier <!-- deleted by customization to --><!-- keep by customization: begin --> fit for <!-- keep by customization: end --> upgrade <!-- deleted by customization to -->. The upgrade process is quite simple and can be done without taking the database offline and with no data loss. Upgraded databases benefit from the predictable performance and additional features provided by the new service tiers.
