<properties 
   pageTitle="Create an Internet facing load balancer in Resource Manager using PowerShell | Windows Azure"
...
Create a new resource group named *NRP-RG* in the *China North* Azure location.
...
Create a subnet and a virtual network.
	...
	New-AzurevirtualNetwork -Name NRPVNet -ResourceGroupName NRP-RG -Location "China North" -AddressPrefix 10.0.0.0/16 -Subnet $backendSubnet

### Step 2

Create a public IP address (PIP) named *PublicIP* to be used by a frontend IP pool with DNS name *loadbalancernrp.chinanorth.chinacloudapp.cn*. The command below uses the static allocation type.

	$publicIP = New-AzurePublicIpAddress -Name PublicIp -ResourceGroupName NRP-RG -Location "China North" -AllocationMethod Static -DomainNameLabel loadbalancernrp 
...
>In this example, the FQDN will be *loadbalancernrp.chinanorth.chinacloudapp.cn*.
...
You can find guidance on how to create a virtual machine, and assign a NIC in [Create and preconfigure a Windows Virtual Machine with Resource Manager and Azure PowerShell](/documentation/articles/virtual-machines-ps-create-preconfigure-windows-resource-manager-vms#Example), using option 5 in the example. 
