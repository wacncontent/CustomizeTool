...
- **Match the data store to the pattern of use**. Partitioning allows each partition to be deployed on a different type of data store, based on cost and the built-in features that data store offers. For example, large binary data could be stored in a blob data store, while more structured data could be held in a document database. For more information see [Building a Polyglot <!-- deleted by customization Solution] --><!-- keep by customization: begin --> Solution](https://msdn.microsoft.com/zh-cn/library/dn313279.aspx) <!-- keep by customization: end --> in the patterns & practices guide [Data Access for Highly-Scalable Solutions: Using SQL, NoSQL, and Polyglot <!-- deleted by customization Persistence] --><!-- keep by customization: begin --> Persistence](https://msdn.microsoft.com/zh-cn/library/dn271399.aspx) <!-- keep by customization: end --> on the Microsoft website.
...
> For more detailed information and guidance about many of these considerations, and good practice techniques for designing data stores that implement horizontal partitioning, see the [Sharding <!-- deleted by customization Pattern] --><!-- keep by customization: begin --> Pattern](https://msdn.microsoft.com/zh-cn/library/dn589797.aspx) <!-- keep by customization: end -->
...
Azure SQL Database is a relational database-as-a-service that runs in the cloud. It is based on Microsoft SQL Server. A relational database divides information into tables, and each table holds information about entities as a series of rows. Each row contains columns that hold the data for the individual fields of an entity. The <!-- deleted by customization [What is Azure --><!-- keep by customization: begin --> [Azure <!-- keep by customization: end --> SQL <!-- deleted by customization Database?] --><!-- keep by customization: begin --> Database](/documentation/articles/sql-database-technical-overview) <!-- keep by customization: end --> page on the Microsoft website provides detailed documentation on creating and using SQL databases.
...
It is the programmer's responsibility to associate a dataset with a shardlet key. A separate SQL database acts as a global shard-map manager that contains a list of databases (shards) that comprise the entire system together with information about the shardlets in each database. A client application that accesses data first connects to the global shard-map manager database to obtain a copy of the shard-map (listing shards and shardlets) which it caches locally. The application then uses this information to route data requests to the appropriate shard. This functionality is hidden behind a series of APIs contained in the Azure SQL Database Elastic <!-- deleted by customization Database --><!-- keep by customization: begin --> Scale <!-- keep by customization: end --> Client Library, available as a NuGet package. The page <!-- deleted by customization [Elastic --><!-- keep by customization: begin --> [Azure SQL <!-- keep by customization: end --> Database <!-- deleted by customization features overview] --><!-- keep by customization: begin --> Elastic Scale Overview](/documentation/articles/sql-database-elastic-scale-introduction) <!-- keep by customization: end --> on the Microsoft website provides a more comprehensive introduction to Elastic <!-- deleted by customization Database --><!-- keep by customization: begin --> Scale <!-- keep by customization: end -->.
...
As the name implies, Elastic Database enables a system to add and remove shards as the volume of data shrinks and grows. The APIs in the Azure SQL Database Elastic Database Client Library enable an application to create and delete shards dynamically (and transparently update the shard-map manager), but removing a shard is a destructive operation that also requires deleting all the data in that shard. If an application needs to split a shard into two separate shards or combine shards together, Elastic Database provides a separate Split/Merge service. This service runs in a cloud-hosted service (the developer has to create this cloud-hosted service), and takes care of migrating data between shards safely. For more information, see the topic [Scaling using the Elastic Database split-merge tool] on the Microsoft website.
...
The remainder of the data for an entity consists of application-defined fields. No particular schemas are enforced, and each row can contain a different set of application-defined fields. The only limitation is that the maximum size of an entity (including the partition and row keys) is currently 1MB. The maximum size of a table is 200TB, although these figures may change in the future (check the page [Azure Storage Scalability and Performance <!-- deleted by customization Targets] --><!-- keep by customization: begin --> Targets](https://msdn.microsoft.com/zh-cn/library/azure/dn249410.aspx) <!-- keep by customization: end --> on the Microsoft website for the most recent information about these limits. If you are attempting to store entities that exceed this capacity, then consider splitting them into multiple tables; use vertical partitioning and divide the fields into the groups that are most likely to be accessed together.
...
For additional information on partitioning data in Azure table storage, see the article [Azure Storage Table Design Guide] on the Microsoft website.
...
Each DocumentDB account has a quota that limits the number of databases and collections that it can contain and the amount of document storage available. These limits are subject to change, but are described on the page [DocumentDB limits and <!-- deleted by customization quotas] --><!-- keep by customization: begin --> quotas](/documentation/articles/documentdb-limits) <!-- keep by customization: end --> on the Microsoft website. It is theoretically possible that if you implement a system where all shards belong to the same database you might reach the storage capacity limit of the account. In this case, you may need to create additional DocumentDB accounts and databases, and distribute the shards across these databases. However, even if you are unlikely to hit the storage capacity of a database, a good reason for using multiple databases is that each database has its own set of users and permissions. You can use this mechanism to isolate access to collections on a per-database basis.
...
- The resources available to a DocumentDB database are subject to the quota limitations of the DocumentDB account. Each database can hold a number of collections (again, there is a limit) and each collection is associated with a performance level that governs the RU rate limit (reserved throughput) for that collection. For more information, visit the [DocumentDB limits and <!-- deleted by customization quotas] --><!-- keep by customization: begin --> quotas](/documentation/articles/documentdb-limits) <!-- keep by customization: end --> page on the Microsoft website.
...
For an example of a solution that supports online migration, see the [Scaling using the Elastic Database split-merge tool], documented online on the Microsoft website.
...
[Scaling using the Elastic Database split-merge tool]: <!-- deleted by customization sql-database/sql-database-elastic-scale-overview-split-and-merge.md --><!-- keep by customization: begin --> /documentation/articles/sql-database-elastic-scale-overview-split-and-merge <!-- keep by customization: end -->
<!-- deleted by customization
...
<!-- keep by customization: end -->
[Sharding pattern]: https://msdn.microsoft.com/zh-cn/library/dn589797.aspx
