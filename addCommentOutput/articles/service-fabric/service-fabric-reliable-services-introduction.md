...
3. Once the <!-- deleted by customization communication listener --><!-- keep by customization: begin --> CommunicationListener <!-- keep by customization: end --> is created it is opened
  + <!-- deleted by customization Communication listeners --><!-- keep by customization: begin --> CommunicationListeners <!-- keep by customization: end --> have a method called Open(), which is called at this point and which returns the listening address for the service. If your Reliable Service uses one of the built in ICommunicationListeners, then this is handled for you.
...
When the service is being shut down (either when it is deleted or just being moved from a particular location) the call order is the same, first Close() is called on the <!-- deleted by customization communication listener --><!-- keep by customization: begin --> CommunicationListener <!-- keep by customization: end -->, then the cancellation token that was passed to RunAsync() is canceled.
