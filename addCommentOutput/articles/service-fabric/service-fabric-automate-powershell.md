...
Before moving on to the tasks in the article, be sure to [Install the runtime, SDK, and <!-- deleted by customization tools](/documentation/articles/service-fabric-get-started) --><!-- keep by customization: begin --> tools](service-fabric-get-started.md) <!-- keep by customization: end --> which also installs the ServiceFabric and ServiceFabricTestability PowerShell modules. [Enable PowerShell script <!-- deleted by customization execution](/documentation/articles/service-fabric-get-started#enable-powershell-script-execution) --><!-- keep by customization: begin --> execution](service-fabric-get-started.md#enable-powershell-script-execution) <!-- keep by customization: end --> and also [Install and start a local <!-- deleted by customization cluster](/documentation/articles/service-fabric-get-started#install-and-start-a-local-cluster) --><!-- keep by customization: begin --> cluster](service-fabric-get-started.md#install-and-start-a-local-cluster) <!-- keep by customization: end --> so you can run the examples in the article.
...
Before running any PowerShell commands in this article, first connect to the local Service Fabric cluster using <!-- deleted by customization [Connect-ServiceFabricCluster](https://msdn.microsoft.com/zh-cn/library/azure/mt125938.aspx) --><!-- keep by customization: begin --> [Connect-ServiceFabricCluster](https://msdn.microsoft.com/library/azure/mt125938.aspx) <!-- keep by customization: end -->:
...
After you've built the application and the application type has been packaged, you can deploy the application into a local Service Fabric cluster. First, package the HelloWorldStateful application in Visual Studio by right-clicking on **HelloWorldStatefulApplication** in Solution Explorer and selecting **Package**.  See [Model an application in Service <!-- deleted by customization Fabric](/documentation/articles/service-fabric-application-model) --><!-- keep by customization: begin --> Fabric](service-fabric-application-model.md) <!-- keep by customization: end --> for information on the service and application manifests and the package layout.  Deployment involves uploading the application package, registering the application type, and creating the application instance. Use the instructions in this section to deploy a new application to a cluster.

### Step 1: Upload the application package
Uploading the application package to the ImageStore puts it in a location accessible by internal Service Fabric components.  The application package contains the necessary application manifest, service manifest(s), and code/config/data package(s) to create the application and service instances.  The <!-- deleted by customization [Copy-ServiceFabricApplicationPackage](https://msdn.microsoft.com/zh-cn/library/azure/mt125905.aspx) --><!-- keep by customization: begin --> [Copy-ServiceFabricApplicationPackage](https://msdn.microsoft.com/library/azure/mt125905.aspx) <!-- keep by customization: end --> command will upload the package. For example:
...
Registering the application package makes the application type and version declared in the application manifest available for use. The system will read the package uploaded in the previous step, verify the package (equivalent to running <!-- deleted by customization [Test-ServiceFabricApplicationPackage](https://msdn.microsoft.com/zh-cn/library/azure/mt125950.aspx) --><!-- keep by customization: begin --> [Test-ServiceFabricApplicationPackage](https://msdn.microsoft.com/library/azure/mt125950.aspx) <!-- keep by customization: end --> locally), process the package contents, and copy the processed package to an internal system location.  Run the <!-- deleted by customization [Register-ServiceFabricApplicationType](https://msdn.microsoft.com/zh-cn/library/azure/mt125958.aspx) --><!-- keep by customization: begin --> [Register-ServiceFabricApplicationType](https://msdn.microsoft.com/library/azure/mt125958.aspx) <!-- keep by customization: end --> cmdlet:
...
An application can be instantiated using any application type version that has been registered successfully using the <!-- deleted by customization [New-ServiceFabricApplication](https://msdn.microsoft.com/zh-cn/library/azure/mt125913.aspx) --><!-- keep by customization: begin --> [New-ServiceFabricApplication](https://msdn.microsoft.com/library/azure/mt125913.aspx) <!-- keep by customization: end --> command. The name of each application must start with the **fabric:** scheme and be unique for each application instance. The application type name and application type version are declared in the ApplicationManifest.xml file. If there were any default services defined in the application manifest of the target application type, then those will also be created at this time.

```powershell
New-ServiceFabricApplication fabric:/HelloWorldStatefulApplication HelloWorldStatefulApplication 1.0.0.0
```

The <!-- deleted by customization [Get-ServiceFabricApplication](https://msdn.microsoft.com/zh-cn/library/azure/mt163515.aspx) --><!-- keep by customization: begin --> [Get-ServiceFabricApplication](https://msdn.microsoft.com/library/azure/mt163515.aspx) <!-- keep by customization: end --> command lists all applications instances that were successfully created along with their overall status. The <!-- deleted by customization [Get-ServiceFabricService](https://msdn.microsoft.com/zh-cn/library/azure/mt125889.aspx) --><!-- keep by customization: begin --> [Get-ServiceFabricService](https://msdn.microsoft.com/library/azure/mt125889.aspx) <!-- keep by customization: end --> command lists all service instances that were successfully created within a given application instance. Default services (if any) will be listed.
...
You can upgrade a previously deployed Service Fabric application.  This task upgrades the HelloWorldStateful application that was deployed in TASK: Deploy a Service Fabric application. Read through [Application <!-- deleted by customization Upgrade](/documentation/articles/service-fabric-application-upgrade) --><!-- keep by customization: begin --> Upgrade](service-fabric-application-upgrade.md) <!-- keep by customization: end --> for additional information.
...
The application is now built, packaged, and ready to be upgraded. If you open up a PowerShell window as administrator and type <!-- deleted by customization [Get-ServiceFabricApplication](https://msdn.microsoft.com/zh-cn/library/azure/mt163515.aspx) --><!-- keep by customization: begin --> [Get-ServiceFabricApplication](https://msdn.microsoft.com/library/azure/mt163515.aspx) <!-- keep by customization: end -->, you should see that Application Type 1.0.0.0 of **HelloWorldStatefulApplication** is deployed.  For the HelloWorldStateful sample, the application package is found in: *C:\ServiceFabricSamples\Services\VS2015\HelloWorldStateful\HelloWorldStatefulApplication\pkg\Debug*.
...
The next step is to register the new version of the application with Service Fabric, which can be performed using the <!-- deleted by customization [Register-ServiceFabricApplicationType](https://msdn.microsoft.com/zh-cn/library/azure/mt125958.aspx) --><!-- keep by customization: begin --> [Register-ServiceFabricApplicationType](https://msdn.microsoft.com/library/azure/mt125958.aspx) <!-- keep by customization: end --> cmdlet:
...
Various upgrade parameters, timeouts, and health criterion can be applied to application upgrades. Read through [application upgrade <!-- deleted by customization parameters](/documentation/articles/service-fabric-application-upgrade-parameters) --><!-- keep by customization: begin --> parameters](service-fabric-application-upgrade-parameters.md) <!-- keep by customization: end --> and [upgrade <!-- deleted by customization process](/documentation/articles/service-fabric-application-upgrade) --><!-- keep by customization: begin --> process](service-fabric-application-upgrade.md) <!-- keep by customization: end --> to learn more. For this walkthrough, leave the service health evaluation criterion set to the default (and recommended values). All services and instances should be _healthy_ after the upgrade.  However, increase the *HealthCheckStableDuration* to 60 seconds (so that the services will be healthy for at least 20 seconds before the upgrade proceeds to the next upgrade domain).  Also set the *UpgradeDomainTimeout* to be 1200 seconds and the *UpgradeTimeout* to be 3000 seconds. Finally, set the *UpgradeFailureAction* to rollback which requests that Service Fabric rollback the application to the previous version if failures are encountered during upgrade.

You can now start the application upgrade using the <!-- deleted by customization [Start-ServiceFabricApplicationUpgrade](https://msdn.microsoft.com/zh-cn/library/azure/mt125975.aspx) --><!-- keep by customization: begin --> [Start-ServiceFabricApplicationUpgrade](https://msdn.microsoft.com/library/azure/mt125975.aspx) <!-- keep by customization: end --> cmdlet:
...
Note the application name is the same as the previously deployed v1.0.0.0 application name (fabric:/HelloWorldStatefulApplication). Service Fabric uses this name to identify which application is getting upgraded. If you set the timeouts to be too short, you may encounter a time out failure message that states the problem. Refer to [Troubleshoot application <!-- deleted by customization upgrades](/documentation/articles/service-fabric-application-upgrade-troubleshooting) --><!-- keep by customization: begin --> upgrades](service-fabric-application-upgrade-troubleshooting.md) <!-- keep by customization: end -->, or increase the timeouts.

<!-- deleted by customization
You can monitor application upgrade progress using [Service Fabric Explorer](/documentation/articles/service-fabric-visualizing-your-cluster), or using the [Get-ServiceFabricApplicationUpgrade](https://msdn.microsoft.com/zh-cn/library/azure/mt125988.aspx) cmdlet:
-->
...
In a few minutes, <!-- deleted by customization [Get-ServiceFabricApplicationUpgrade](https://msdn.microsoft.com/zh-cn/library/azure/mt125988.aspx) --><!-- keep by customization: begin --> [Get-ServiceFabricApplicationUpgrade](https://msdn.microsoft.com/library/azure/mt125988.aspx) <!-- keep by customization: end --> cmdlet should show that all upgrade domains were upgraded (completed).

## TASK: Test a Service Fabric application

In order to write high quality services, developers need to be able to induce unreliable infrastructure faults to test the stability of their services. Service Fabric gives developers the ability to induce fault actions and test services in the presence of failures using the chaos and failover test scenarios.  Read through [Testability <!-- deleted by customization Overview](/documentation/articles/service-fabric-testability-overview) --><!-- keep by customization: begin --> Overview](service-fabric-testability-overview.md) <!-- keep by customization: end --> for additional information.
...
When an application instance is no longer needed, you can permanently remove it using the <!-- deleted by customization [Remove-ServiceFabricApplication](https://msdn.microsoft.com/zh-cn/library/azure/mt125914.aspx) --><!-- keep by customization: begin --> [Remove-ServiceFabricApplication](https://msdn.microsoft.com/library/azure/mt125914.aspx) <!-- keep by customization: end --> cmdlet. This will automatically remove all services belonging to the application as well, permanently removing all service state. This operation cannot be reversed and application state cannot be recovered.
...
When you no longer need a particular version of an application type, unregister it using the <!-- deleted by customization [Unregister-ServiceFabricApplicationType](https://msdn.microsoft.com/zh-cn/library/azure/mt125885.aspx) --><!-- keep by customization: begin --> [Unregister-ServiceFabricApplicationType](https://msdn.microsoft.com/library/azure/mt125885.aspx) <!-- keep by customization: end --> cmdlet. Unregistering unused types will release storage space used by the application package in the Image Store. An application type can be unregistered as long as there are no applications instantiated against it or pending application upgrades referencing it.
...
After the application type is unregistered the application package can be removed from the ImageStore using the <!-- deleted by customization [Remove-ServiceFabricApplicationPackage](https://msdn.microsoft.com/zh-cn/library/azure/mt163532.aspx) --><!-- keep by customization: begin --> [Remove-ServiceFabricApplicationPackage](https://msdn.microsoft.com/library/azure/mt163532.aspx) <!-- keep by customization: end --> cmdlet.
...
<!-- deleted by customization
[Service Fabric application life-cycle](/documentation/articles/service-fabric-application-lifecycle)

[Manage a Service Fabric service](/documentation/articles/service-fabric-manage-your-service-index)

[Azure Service Fabric Cmdlets](https://msdn.microsoft.com/zh-cn/library/azure/mt125965.aspx)

[Azure Service Fabric Testability Cmdlets](https://msdn.microsoft.com/zh-cn/library/azure/mt125844.aspx)
-->
<!-- keep by customization: begin -->
