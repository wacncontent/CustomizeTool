...
- **Scalable services**: Individual <!-- deleted by customization services --><!-- keep by customization: begin --> Services <!-- keep by customization: end --> can be partitioned allowing for state to be scaled out across the cluster. In addition, individual services can be created and removed on the fly. Services can be quickly and easily scaled out from a few instances on a few nodes to thousands of instances on many nodes, and then immediately scaled down again, depending on your resource needs. You can use Service Fabric to build these service and manage their complete lifecycle.
...
## Designing applications composed of stateless and stateful microservices <!-- keep by customization: begin --> ## <!-- keep by customization: end -->
...
When building applications consisting of microservices you typically have a combination of stateless web sites (ASP.NET, node.js, etc.) calling onto stateless and stateful business middle-tier services, all deployed into the same Service Fabric cluster using the Service Fabric deployment commands. Each of these services is <!-- deleted by customization independent --><!-- keep by customization: begin --> independence <!-- keep by customization: end --> with regards to scale, reliability, and resource usage, greatly improving agility in development and lifecycle management.
