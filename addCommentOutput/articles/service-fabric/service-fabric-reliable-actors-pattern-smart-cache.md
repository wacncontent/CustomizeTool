...
Finally, we implement the IJobQueue interface in the <!-- deleted by customization actor --><!-- keep by customization: begin --> grain <!-- keep by customization: end -->. Note that we omitted the implementation details of the priority queue here for clarity. A sample implementation can be found in the accompanying samples.
...
Actors provide flexibility for the developer to define rich object structures as part <!-- deleted by customization of --> the actors or reference object graphs outside of the actors.
...
And how about populating these actors caches then? There are number of ways to achieve this. Actors provide virtual methods called OnActivateAsync() and <!-- deleted by customization OnDeactivateAsync() --><!-- keep by customization: begin --> OnDectivateAsync() <!-- keep by customization: end --> to let us know when an instance of the actor is activated and deactivated. Note that the actor is activated on demand when a first request is sent to it.
