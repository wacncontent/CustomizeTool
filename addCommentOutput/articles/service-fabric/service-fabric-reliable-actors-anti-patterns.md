...
* Over architect. Let the environment work. It can be hard for developers who are accustomed to worrying about concurrent collections and locks, or using tools to compile objects from XML, to simply just code a class that does simple things like assign a value to a variable or schedule work. Scheduled tasks are built in. Locks are not needed. State is not a mortal enemy. This takes some getting used to for many folks <!-- deleted by customization who've --><!-- keep by customization: begin --> whoâve <!-- keep by customization: end --> done a lot of server side work in large scale environments.
