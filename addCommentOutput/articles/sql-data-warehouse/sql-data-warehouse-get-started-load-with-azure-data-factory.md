<properties
	pageTitle="Load data with Azure Data Factory | Windows Azure"
...
For this tutorial, you will need the following resources:
   ...
   + **Azure Storage Blob**:  Your Azure Storage Blob will be the source of data for the pipeline.  You can use an existing blob or [provision a new one](/documentation/articles/storage-create-storage-account).

   + **SQL Data Warehouse**: In this tutorial you will be moving data to SQL Data Warehouse.  If you do not already have an instance set-up, you can learn how  [here](/documentation/articles/sql-data-warehouse-get-started-provision).  In addition, your instance will need to be set-up with our AdventureWorks DW dataset.  If you didn't provision your data warehouse with the sample data, you can [load it manually](/documentation/articles/sql-data-warehouse-get-started-manually-load-samples).

   + **Azure Data Factory**: Azure Data Factory will complete the actual load and if you need more information on setting Azure Data Factory or creating pipelines then you can see that [here](/documentation/articles/data-factory-build-your-first-pipeline-using-editor).
...
1. First, [download the sample data](https://migrhoststorage.blob.core.chinacloudapi.cn/adfsample/FactInternetSales.csv).  This data works in conjunction with the sample data that is already in your sample data, providing another three years of sales data.
...
2. Once the data is downloaded, you can move it to your blob storage by running the below script in AZCopy:
	...
	See the [AZCopy documentation](/documentation/articles/storage-use-azcopy) for additional information on how to install and work with AZCopy.
...
Now that we've set up all the pieces, we can start to set-up the pipeline by navigating to your Azure Data Factory instance in the Azure Preview Portal.  This can be done by going to the [Azure Management <!-- deleted by customization Portal](portal.azure.com) --><!-- keep by customization: begin --> Portal](https://manage.windowsazure.cn) <!-- keep by customization: end --> and selecting your data factory from the left-hand menu.
...
Finally, we will set-up and run the pipeline in Azure Data Factory.  This is the operation that will complete the actual data movement.  You can find a full view of the operations that you can complete with SQL Data Warehouse and Azure Data Factory [here](/documentation/articles/data-factory-azure-sql-data-warehouse-connector).
