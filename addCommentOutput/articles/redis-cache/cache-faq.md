...
-	**High Availability/SLA**: Azure Redis Cache guarantees that a Standard/Premium cache will be available at least 99.9% of the time. To learn more about our SLA,  see [Azure Redis Cache <!-- deleted by customization Pricing](https://azure.microsoft.com/support/legal/sla/cache/v1_0/) --><!-- keep by customization: begin --> Pricing](/home/features/redis-cache/#price) <!-- keep by customization: end -->. The SLA only covers connectivity to the Cache endpoints. The SLA does not cover protection from data loss. We recommend using the Redis data persistence feature in the Premium tier to increase resiliency against data loss.
...
-	**Maximum number of client connections**: The Premium tier offers the maximum number of clients that can connect to Redis, with a higher number of connections for larger sized caches. [Please refer to the pricing page for <!-- deleted by customization details](/home/features/cache/#price) --><!-- keep by customization: begin --> details](/home/features/redis-cache/#price) <!-- keep by customization: end -->.
...
Azure Redis Cache pricing is <!-- deleted by customization [here](/home/features/cache/#price) --><!-- keep by customization: begin --> [here](/home/features/redis-cache/#price) <!-- keep by customization: end -->. The pricing page lists pricing as an hourly rate. Caches are billed on a per-minute basis from the time that the cache is created until the time that a cache is deleted. There is no option for stopping or pausing the billing of a cache.
...
>
>Managed Cache Service and In-Role Cache service will remain available for existing customers for a maximum of 12 months from the date of this announcement on November 30, <!-- deleted by customization 2015—the --><!-- keep by customization: begin --> 2015âthe <!-- keep by customization: end --> end of service date for both will end on November 30, 2016. After this date, Managed Cache Service will be shut down, and In-Role Cache service will no longer be supported.
>
