<properties 
   pageTitle="Configure an application gateway for SSL offload using classic deployment| Windows Azure"
...
Application Gateway can be configured to terminate the SSL session at the gateway to avoid costly SSL decryption task to happen at the web farm. SSL offload also simplifies the front end server setup and management of the web site.

>[AZURE.IMPORTANT] Before you work with Azure resources, it's important to understand that Azure currently has two deployment models: Resource Manager, and classic deployment model. Make sure you understand [deployment models and tools](/documentation/articles/azure-classic-rm) before working with any Azure resource. You can view the documentation for different tools by clicking the tabs at the top of this article.This document will cover creating an application gateway using Azure classic deployment model . To use the Azure Resource Manager version, go to [Configure application gateway SSL offload using Azure Resource Manager](/documentation/articles/application-gateway-ssl-arm).
...
- **Back end server pool:** The list of IP addresses of the back end servers. The IP addresses listed should either belong to the VNet subnet, or should be a public IP/VIP. 
...
<!-- deleted by customization - --><!-- keep by customization: begin --><!--- <!-- keep by customization: end --> [Azure Load <!-- deleted by customization Balancer](/documentation/services/load-balancer/) --><!-- keep by customization: begin --> Balancer](/documentation/services/load-balancer/)--><!-- keep by customization: end -->
