...
5. Open the file you just saved and look at the contents under **parameters** in line 5. ARM template parameters provide a placeholder for values that can be filled out during deployment.
	...
	| **backendaddress2** | IP address of the second web <!-- deleted by customization server | --><!-- keep by customization: begin --> server| <!-- keep by customization: end -->
...
3. If needed, create a new resource group using `New-AzureResourceGroup` cmdlet.In the example below, you will create a new resource group called AppgatewayRG in China East location:
		...
		ResourceId       : <!-- deleted by customization /subscriptions/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/resourceGroups/AppgatewayRG --><!-- keep by customization: begin --> /subscriptions/################################/resourceGroups/AppgatewayRG <!-- keep by customization: end -->
...
### Step 1 
Using the link [Click to deploy Application <!-- deleted by customization Gateway](https://azure.microsoft.com/documentation/templates/101-application-gateway-public-ip/) --><!-- keep by customization: begin --> Gateway](/documentation/templates/101-application-gateway-public-ip/) <!-- keep by customization: end --> will redirect you to the portal template page for Application Gateway.
