...
The **Azure Resource Group** deployment project template is available in Visual Studio when Azure SDK 2.6 is installed. Azure Resource Group projects let you group together and publish multiple related Azure resources in a single deployment operation. Azure Resource Group projects use a technology called **Azure Resource Manager** to do their work. **Azure Resource Manager** is a REST API service that lets you define Azure resource groups, which contain multiple Azure resources that are typically used together and have a similar lifecycle. By using resource groups, you can operate on all the resources in a group with a single function call, instead of calling different functions for each individual resource. To learn more about Azure resource groups, see [Using <!-- deleted by customization the Azure Preview Portal --><!-- keep by customization: begin --> resource groups <!-- keep by customization: end --> to manage your Azure <!-- deleted by customization resources](/documentation/articles/resource-group-portal) --><!-- keep by customization: begin --> resources](./azure-portal/azure-preview-portal-using-resource-groups/) <!-- keep by customization: end -->. <!-- deleted by customization For a more detailed, end-to-end Azure Resource Group deployment scenario, see [Azure Resource Group for Visual Studio](https://azure.microsoft.com/blog/azure-resource-manager-2-5-for-visual-studio/). -->

Azure Resource Group projects contain Azure Resource Manager JSON templates, which define the elements that are deployed to a resource group. See <!-- deleted by customization [Authoring Azure --><!-- keep by customization: begin --> [Azure <!-- keep by customization: end --> Resource Manager <!-- deleted by customization templates](/documentation/articles/resource-group-authoring-templates) --><!-- keep by customization: begin --> Template Language](/documentation/articles/resource-group-authoring-templates) <!-- keep by customization: end --> for more information.
...
- **Parameter files** (which have the extension <!-- deleted by customization `.param.*.json`) --><!-- keep by customization: begin --> .param.*.json) <!-- keep by customization: end --> contain values for the parameters specified in the configuration file that are needed for each resource provider. In this example, the configuration file for a web app (WebSite.json) defines parameters for <!-- deleted by customization *siteName* --><!-- keep by customization: begin --> siteName <!-- keep by customization: end --> and <!-- deleted by customization *siteLocation* --><!-- keep by customization: begin --> siteLocation <!-- keep by customization: end -->. During deployment, you are asked to provide values for the parameters in the template file, and these values are stored in the parameters file. You can also edit the parameter file <!-- deleted by customization directly. --><!-- keep by customization: begin --> directly." <!-- keep by customization: end -->
...
1. In a browser, open the [Azure <!-- deleted by customization Preview Portal](https://manage.windowsazure.cn/) --><!-- keep by customization: begin --> Management Portal](https://manage.windowsazure.cn) <!-- keep by customization: end -->. Since this is a new change, there should be a new notification message available in the **Notifications** tab. Choose it to view details about the new Azure <!-- deleted by customization resource group --><!-- keep by customization: begin --> Resource Group <!-- keep by customization: end -->. To see a list of all the available resource groups, you can choose the **Browse** tab and then choose **Resource Groups**.
...
If you are using Cloud Deployment projects that were created with Azure SDK 2.5, you'll want to upgrade to Azure SDK 2.6 <!-- deleted by customization or later --> to be able to use the new features for editing and deploying Azure resource templates. The easiest way to reuse the templates you created with Azure SDK 2.5 is to create the Azure SDK 2.6 <!-- deleted by customization or later --> version of the project, move your templates to that project and do some modifications.

### To use Azure SDK 2.5 Cloud Deployment projects with Azure SDK 2.6 <!-- deleted by customization or later -->

1. Add a new Azure SDK 2.6 <!-- deleted by customization or later --> Azure Resource Group project to your solution.
...
1. Copy your template and parameter files from the Azure SDK 2.5 Cloud Deployment project to your Azure SDK 2.6 <!-- deleted by customization or later --> Resource Group project.
...
1. Paste the files into the Templates folder in your new Azure SDK 2.6 <!-- deleted by customization or later --> Resource Group project.

1. If you're also deploying a web application with your template, you’ll need to create a reference from the new Azure SDK 2.6 <!-- deleted by customization or later --> Resource Group project to your Web Application.
...
1. If you don’t plan to use them, you can delete the empty template and parameter files that were automatically added to the Azure SDK 2.6 <!-- deleted by customization or later --> project when you created it

    1. Delete the file DeployTemplate.json.

    1. Delete the file DeploymentTemplate.param.dev.json.

1. If you have made changes to the Publish-AzureResourceGroup.ps1 script in the Azure SDK 2.5 project, you’ll need to move those changes to the Deploy-AzureResourceGroup.ps1 script in the Azure SDK 2.6 <!-- deleted by customization or later --> project.
...
<!-- deleted by customization Some --><!-- keep by customization: begin --> A couple of <!-- keep by customization: end --> changes were made <!-- deleted by customization to the --><!-- keep by customization: begin --> in <!-- keep by customization: end --> templates <!-- keep by customization: begin --> are <!-- keep by customization: end --> deployed in Azure SDK 2.6 that makes Azure SDK 2.5 deployment script and templates incompatible. The first, and biggest change, is the deployment is launched. <!-- keep by customization: begin --> In <!-- keep by customization: end --> Azure SDK 2.5 <!-- keep by customization: begin -->, we <!-- keep by customization: end --> had compiled code that used the Azure REST APIs to upload your template and start the deployment. <!-- deleted by customization Feedback --><!-- keep by customization: begin --> We heard feedback <!-- keep by customization: end --> from <!-- keep by customization: begin --> many <!-- keep by customization: end --> developers <!-- deleted by customization indicated --> that they would <!-- deleted by customization prefer --><!-- keep by customization: begin --> like <!-- keep by customization: end --> Visual Studio to just launch the PowerShell script that is included in the project. So <!-- deleted by customization, --> in Azure SDK 2.6, the deploy command launches the PowerShell script that is included in the project to deploy the template. <!-- deleted by customization This enables --><!-- keep by customization: begin --> That allows <!-- keep by customization: end --> you to customize the deployment and have those customizations always run, whether you are deploying from the command line using Azure PowerShell or deploying through <!-- deleted by customization Visual Studio --><!-- keep by customization: begin --> VS <!-- keep by customization: end --> using the <!-- deleted by customization Deploy --><!-- keep by customization: begin --> deploy <!-- keep by customization: end --> command. To deploy from Visual Studio, you'll need to use the Azure SDK 2.6 <!-- deleted by customization (or later) --> deployment PowerShell script when you have <!-- keep by customization: begin --> the <!-- keep by customization: end --> Azure SDK 2.6 <!-- deleted by customization or later --> installed.

<!-- deleted by customization Adjustments --><!-- keep by customization: begin --> Adjustements <!-- keep by customization: end --> were also made to some variable names and build tasks to better align with naming conventions in TFS automated builds and other projects within Microsoft. The code in Visual Studio that gathers the variables and values needed to launch the PowerShell script will look for these new names.

## Next steps

To learn how to add resources to your Azure Resource Group in Visual Studio, see [Adding resources to an Azure Resource <!-- deleted by customization Group](/documentation/articles/vs-azure-tools-resource-group-adding-resources) --><!-- keep by customization: begin --> Group](https://msdn.microsoft.com/zh-cn/library/azure/mt125415.aspx) <!-- keep by customization: end -->.
