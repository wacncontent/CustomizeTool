...
The number of partitions is specified at the Event Hub creation time and must be between <!-- deleted by customization 2 --><!-- keep by customization: begin --> 8 <!-- keep by customization: end --> and 32 <!-- deleted by customization (the default is 4) -->. Partitions are a data organization mechanism and are more related to the degree of downstream parallelism required in consuming applications than to Event Hubs throughput. This makes the choice of the number of partitions in an Event Hub directly related to the number of concurrent readers you expect to have. After Event Hub creation, the partition count is not changeable; you should consider this number in terms of long-term expected scale. You can increase the 32 partition limit by contacting the Azure Service Bus team.

While partitions are identifiable and can be sent to directly, it is generally best to avoid sending data to specific partitions. Instead, you can use higher level constructs introduced in the [Event <!-- deleted by customization publisher](#event-publisher) --><!-- keep by customization: begin --> publisher](#Event-publisher) <!-- keep by customization: end --> and [Publisher <!-- deleted by customization Policy](#capacity-and-security) --><!-- keep by customization: begin --> Policy](#Capacity-and-security) <!-- keep by customization: end --> sections.
...
For more information about working with SAS, see [Shared Access Signature Authentication with Service <!-- deleted by customization Bus](/documentation/articles/service-bus-shared-access-signature-authentication) --><!-- keep by customization: begin --> Bus](https://msdn.microsoft.com/zh-cn/library/dn170477.aspx) <!-- keep by customization: end -->.
...
For detailed pricing information, see [Event Hubs <!-- deleted by customization Pricing](/home/features/event-hubs/#price) --><!-- keep by customization: begin --> Pricing](/home/features/event-hubs) <!-- keep by customization: end -->.
...
Azure Event Hubs provides a hyper-scale event and telemetry processing service that can be used for common application and user workflow monitoring at any scale. With the ability to provide publish-subscribe capabilities with low latency and at massive scale, Event Hubs serve as the "on ramp" for Big Data. With publisher-based identity and revocation lists, these capabilities are extended into common Internet of Things scenarios. For more information about developing Event Hubs applications. see the [Event Hubs Programming <!-- deleted by customization Guide](/documentation/articles/event-hubs-programming-guide) --><!-- keep by customization: begin --> Guide](https://msdn.microsoft.com/zh-cn/library/dn789972.aspx) <!-- keep by customization: end -->.
