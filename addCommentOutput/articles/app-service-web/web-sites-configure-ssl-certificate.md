<properties
	pageTitle="Secure your app's custom domain with HTTPS | Azure"
...
[Azure App Service](/documentation/articles/app-service-value-prop-what-is/) that uses a custom domain name. 
It covers server-only authentication. If you need mutual authentication (including client authentication), see 
[How To Configure TLS Mutual Authentication for App Service](/documentation/articles/app-service-web-configure-tls-mutual-auth/).
...
>[AZURE.NOTE] You can get help from Azure experts anytime on the [Azure forums](/support/forums/). 
For more personalized support, go to [Azure Support](/support/contact/) and 
click **Get Support**.
...
configured in your app. For instructions, see [Map a custom domain name to an Azure app](/documentation/articles/web-sites-custom-domain-name/). 
- **Scale up to Basic tier or higher** App Service plans in lower pricing tiers don't support custom SSL 
certificates. For instructions, see [Scale up an app in Azure](/documentation/articles/web-sites-scale/). 
...

		...
		[buy one in the Azure portal directly](/documentation/articles/web-sites-purchase-ssl-web-site/). This article shows you 
...
6. If your CA uses intermediate certificates, install them before you proceed. They usually come
...


	><p> Your `openssl -export` command should look like the following example, which creates a .pfx file that includes the 

...
>[AZURE.IMPORTANT] Self-signed certificates are for test purposes only. Most browsers return errors when 
...
1. In your browser, open the **[Azure  Portal.](https://portal.azure.cn/)**  Portal Preview.](https://portal.azure.cn/)** 
...
- You [used an A record to map your custom domain](/documentation/articles/web-sites-custom-domain-name/#a) to your Azure app, and
...
to the dedicated IP address by following these steps:
	...
	2. [Remap the A record for your custom domain name to this new IP address](/documentation/articles/web-sites-custom-domain-name/#a).
...
Therefore, any existing [CNAME mapping from the custom domain](/documentation/articles/web-sites-custom-domain-name/#cname) to
...
**SNI SSL** traffic back to the original shared IP address by following these steps:
	...
	1. Identify all [CNAME mappings of custom domains](/documentation/articles/web-sites-custom-domain-name/#cname) to your app that 
...
[Deploy a secure ASP.NET MVC 5 app to a web app](/documentation/articles/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/)).
