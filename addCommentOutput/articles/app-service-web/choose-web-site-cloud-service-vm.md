...
Azure  Web App  is the best choice for most web apps. Deployment and management are integrated into the platform, sites can scale quickly to handle high traffic loads, and the built-in load balancing and traffic manager provide high availability. You can move existing sites to Azure  Web App  easily with an [online migration tool](https://www.migratetoazure.net/) , use an open-source app from the Web Application Gallery , or create a new site using the framework and tools of your choice. The [WebJobs][] feature makes it easy to add background job processing to your Azure web app.
...
If you have an existing application that would require substantial modifications to run in Azure  Web App  or Service Fabric, you could choose Virtual Machines in order to simplify migrating to the cloud. However, correctly configuring, securing, and maintaining VMs requires much more time and IT expertise compared to Azure  Web App  and Service Fabric. If you are considering Azure Virtual Machines, make sure you take into account the ongoing maintenance effort required to patch, update, and manage your VM environment.
...
If you want to create a line-of-business application, your website might require direct access to services or data on the corporate network. This is possible on Azure Web App, Service Fabric, and Virtual Machines using the [Azure Virtual Network service](/home/features/networking/). On Azure you can use the [VNET integration  feature](https://azure.microsoft.com/blog/2014/09/15/azure-websites-virtual-network-integration/)  feature](/documentation/articles/app-service-vnet-integration-powershell/) , which allows your Azure applications to run as if they were on your corporate network.

