<properties
	pageTitle="Azure App Service, Virtual Machines, Service Fabric, and Cloud Services comparison | Azure"
...
Integrated MySQL-as-a-service  support|X|X|X||Cloud Services can integrate MySQL-as-a-service through ClearDB's offerings, but not as part of the Azure Portal workflow.  support|X|X|X|| 
...
Support for SSL|X|X|X|X|For App Service web apps, SSL for custom domain names is only supported for Basic and Standard mode. For information about using SSL with web apps, see [Configuring an SSL certificate for an Azure Website](/documentation/articles/web-sites-configure-ssl-certificate/).
Visual Studio integration|X|X|X|X|
Remote Debugging|X|X|X||
Deploy code with TFS|X|X|X|X|
Network isolation with [Azure Virtual Network](/home/features/networking/)|X|X|X|X|See also [Azure Websites Virtual Network Integration](/documentation/articles/app-service-vnet-integration-powershell/)
...
If you want to create a line-of-business application, your website might require direct access to services or data on the corporate network. This is possible on App Service, Service Fabric, and Virtual Machines using the [Azure Virtual Network service](/home/features/networking/). On App Service you can use the [VNET integration  feature](https://azure.microsoft.com/documentation/articles/app-service-vnet-integration-powershell/)  feature](/documentation/articles/app-service-vnet-integration-powershell/) , which allows your Azure applications to run as if they were on your corporate network.
...
> [AZURE.NOTE]
> If you want to get started with Azure App Service before signing up for an account, go to <a href="https://trywebsites.chinacloudsites.cn/">https://trywebsites.chinacloudsites.cn</a>, where you can immediately create a short-lived starter app in Azure App Service for free. No credit card required, no commitments.

...
[WebJobs]:  /documentation/articles/websites-webjobs-resources/&clcid=0x409  /documentation/articles/websites-webjobs-resources/ 
