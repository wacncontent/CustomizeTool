...
![an Azure web site and MySQL Database hosted in a single Azure region][basic-diagram]

While this would allow you to scale out your application by creating multiple web sites instances of the site, everything is hosted within the data centers in a specific geographic region. Visitors from outside this region may see slow response times when using the site, and if the data centers in this region go down, so does your application.
...
![an Azure web site, hosted in multiple regions, using CDBR High Availability router to route to MySQL across regions][multi-region-diagram]

Within each region, the WordPress site would still be scaled across multiple web sites instances, but this scaling is region specific; high traffic regions can be scaled differently than low traffic ones.
...
If the site will accept uploads, or host media files, use Azure Blob storage. If you need caching, consider [Redis cache][rediscache] <!-- deleted by customization, [Memcache Cloud](https://azure.microsoft.com/marketplace/partners/garantiadata/memcached/), [MemCachier](https://azure.microsoft.com/marketplace/partners/memcachier/memcachier/), or one of the other caching offerings in the [Azure Store](http://azure.microsoft.com/gallery/store/) -->.

![an Azure web site, hosted in multiple regions, using CDBR High Availability router for MySQL, with Managed Cache, Blob storage, and CDN][performance-diagram]
...
**HTTPS** | [Enable HTTPS for a web site in Azure Websites][httpscustomdomain]
**Pre-production validation** | [Set up staging environments for web sites in Azure Websites][staging] <p>Note that switching a web site from staging to production also moves the WordPress configuration. You should ensure that all settings are updated to the requirements for your production app before switching the staged app into production.</p>
**Monitoring and troubleshooting** | [Enable diagnostics logging for web sites in Azure Websites][log] and [Monitor web sites in Azure Websites][monitor]
**Deploy your site** | [Deploy a web site in Azure Websites][deploy]
...
**Backup and restore** | [Back up a web site in Azure Websites][backup] and [Restore a web site in Azure Websites][restore]

####Performance

Performance in the cloud is achieved primarily through caching and scale out; however the memory, bandwidth, and other attributes of web sites hosting should also be taken into consideration.
...
**Cache resources** | [Redis cache][rediscache] <!-- deleted by customization, [Memcache Cloud](https://azure.microsoft.com/marketplace/partners/garantiadata/memcached/), [MemCachier](https://azure.microsoft.com/marketplace/partners/memcachier/memcachier/), or one of the other caching offerings in the [Azure Store](/gallery/store/) -->
**Scale your application** | [Scale a web site in Azure Websites][websitescale] and [ClearDB High Availability Routing][cleardbscale]. If you choose to host and manage your own MySQL installation, you should consider [MySQL Cluster CGE][cge] for scale out
...
* **Manual migration** - [Back up your site][wordpressbackup] and [database][wordpressdbbackup], then manually restore it to a web site in Azure Websites and associated MySQL database to migrate highly customized sites and avoid the tedium of manually installing plugins, themes, and other customizations.
...
2. Follow the steps in [Create a WordPress web site in Azure Websites][createwordpress] to create a new WordPress web site. When creating the web site, select **Use an existing MySQL Database** and select the database created in step 1.

If you are migrating an existing WordPress site, see [Migrate an existing WordPress site to Azure](#Migrate-an-existing-WordPress-site-to-Azure) after creating a new web site.
-->
...
2. Create a new web site using the steps in the [Create a new WordPress site](#Create-a-new-WordPress-site) section.

3. Login to your WordPress site on web sites and click on **Plugins** -> **Add New**. Search for, and install, the **WordPress Importer** plugin.
...
8. Once you have completed all these steps, restart your site from its web site blade in the [Azure Management Portal][mgmtportal].
...
4. Create a new web site in the Azure Management Portal and publish the WordPress backup.

	1. Create a new web site in the [Azure Management Portal][mgmtportal] with a database using **New** -> **Web + Mobile** -> **Azure Marketplace** -> **Web Apps** -> **web site + SQL** (or **web site + MySQL**) -> **Create**. Configure all the required settings to create an empty web site.
	...
	3. Use the [Deploy a web site in Azure Websites][deploy] information to enable the deployment method you wish to use, and then deploy your WordPress backup to your web site in Azure Websites.

5. Once the WordPress site has been deployed, you should be able to access the new site (as an Azure Websites) using the *.azurewebsite.net URL for the site.
...
To do this... | Use this...
------------- | -----------
**Set App Service plan mode, size, and enable scaling** | [Scale a web site in Azure Websites][websitescale]
...
**Improve performance** | <ul><li><p><a href="http://ppe.blogs.msdn.com/b/windowsazure/archive/2013/11/18/disabling-arr-s-instance-affinity-in-windows-azure-web-sites.aspx">Disable the ARR cookie</a> - can improve performance when running WordPress on multiple web sites instances</p></li><li><p>Enable caching. <a href="/documentation/services/redis-cache">Redis cache</a> (preview) can be used with the <a href="https://wordpress.org/plugins/redis-object-cache/">Redis object cache WordPress plugin</a>, or use one of the other caching offerings from the <a href="/gallery/store/">Azure Store</a></p></li><li><p><a href="http://ruslany.net/2010/03/make-wordpress-faster-on-iis-with-wincache-1-1/">How to make WordPress faster with Wincache</a> - Wincache is enabled by default for web sites</p></li><li><p><a href="/documentation/articles/web-sites-scale">Scale a web site in Azure Websites</a> and use <a href="http://www.cleardb.com/developers/cdbr/introduction">ClearDB High Availability Routing</a> or <a href="http://www.mysql.com/products/cluster/">MySQL Cluster CGE</a></p></li></ul>
...
**Enable HTTPS for a custom domain name** | [Enable HTTPS for a web site in Azure Websites][httpscustomdomain]
...
**Enable automated backups** | [Back up a web site in Azure Websites][backup]
**Enable diagnostic logging** | [Enable diagnostics logging for web sites in Azure Websites][log]
...
* [Hosting WordPress in a subfolder of your web site in Azure Websites](http://blogs.msdn.com/b/webapps/archive/2013/02/13/hosting-wordpress-in-a-subfolder-of-your-windows-azure-web-site.aspx)
...
* [How to set up a WordPress web site in your Azure account](http://www.itexperience.net/2014/01/20/how-to-set-up-a-wordpress-website-in-your-windows-azure-account/)
