...
This tutorial describes options for authoring and configuring a basic Web Server Gateway Interface (WSGI) compliant Python application on [Azure Websites](/documentation/services/web-sites/).
...
If you do not have an existing web site, you can create one from the [Azure Management Portal](https://manage.windowsazure.cn).  Click the NEW button in the bottom left corner <!-- keep by customization: begin -->. A window will appear. Click COMPUTE, WEB SITE <!-- keep by customization: end -->, then <!-- deleted by customization click **Web + Mobile** > **Web Apps** --><!-- keep by customization: begin --> QUICK CREATE <!-- keep by customization: end -->.
