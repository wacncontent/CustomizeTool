...
The charges for Windows Azure subscriptions vary by rate plan. Some rate plans, such as the Visual Studio Ultimate with MSDN, include monthly credits that you can use on any Azure service based on your needs.
...
For more information about consumption <!-- keep by customization: begin -->, usage, <!-- keep by customization: end --> and rate plans, see the [Windows Azure Purchase Options page](/pricing/overview/).
...
In the [Usage and Billing Portal](https://account.windowsazure.cn/subscriptions) you can view current and download past bills.

To view or download a bill:

1. Sign in to the [Usage and Billing Portal](https://account.windowsazure.cn/subscriptions) using your Microsoft Account ID or Work or School Account ID.
...
**Below are some sample statements for two different offers available on Windows Azure.**

 **OFFER TYPE** | **DESCRIPTION** | **DOWNLOAD** |
 :--------- |:-------- | :-------|
Pay-As-You-Go | Pay in arrears monthly | [Sample file](https://azurepricing.blob.core.chinacloudapi.cn/sampleinvoices/Microsoft_Azure_ccinvoice_Sample.pdf)
Commitment offer | Spend deducted from your prepaid commitment | [Sample file](https://azurepricing.blob.core.chinacloudapi.cn/sampleinvoices/Microsoft_Azure_invoice_Sample.pdf)
...
### Bill to
Windows Azure payments address.

### Subscription offer
Type of subscription offer that was purchased (i.e. Pay-As-You-Go, MSDN-Vidual Studio Ultimate, etc.)

### Account Owner Email
The account email address that the Windows Azure account is registered under.
...
### Usage Charges
Usage charges are total monthly charges on a subscription. You are billed in arrears for your past <!-- deleted by customization monthâs --><!-- keep by customization: begin --> month's <!-- keep by customization: end --> usage.
...
Usage charges are total **monthly** charges on a subscription less any credit or discount. You are billed in arrears for your past <!-- deleted by customization monthâs --><!-- keep by customization: begin --> month's <!-- keep by customization: end --> usage.  The top section of the file display the details on the services you are being billed for during the previous month's billing cycle.  The table below lists the names of the columns for each of the .csv version files.
...
Depending on your usage, there can be thousands of rows of daily usage data. If you want to analyze this data, click **Download Usage** and choose a comma-separated variable file (.csv) version to see your daily usage data for the appropriate billing period.  For your reference, you can download a sample .csv file for each version below.
 ...
 NAME | DOWNLOAD |
 :----------:| :-------: |
  Detailed Usage .csv Version 1|  [Sample File](https://azurepricing.blob.core.chinacloudapi.cn/sampleinvoices/Micorosft_Azure_Detailed_Usage_v1.csv)
  Detailed Usage .csv Version 2 | [Sample File](https://azurepricing.blob.core.chinacloudapi.cn/sampleinvoices/Micorosft_Azure_Detailed_Usage_v2.csv)
...
N/A | Resource Group | _**New column addition.**_ The resource group in which the deployed resource is running in. Refer to <!-- deleted by customization /documentation/articles/resource-group-overview/ --><!-- keep by customization: begin --> http://www.windowsazure.cn/documentation/articles/resource-group-overview/ <!-- keep by customization: end -->
...
N/A | Tags | _**New column addition.**_ New resource types in Azure allow you to tag resources. Refer to <!-- deleted by customization http://azure.microsoft.com/updates/organize-your-azure-resources-with-tags/ --><!-- keep by customization: begin --> http://www.windowsazure.cn/updates/organize-your-azure-resources-with-tags/ <!-- keep by customization: end -->
...
- **Instance ID**: The Instance ID field represents the user specified identifier for the service provisioned. Currently, there are two formats in which the Instance ID is represented: it is either the name of the Resource or the fully qualified Resource ID. Windows Azure services are transitioning to represent the Instance ID in a standardized fully qualified Resource ID format _**(/subscriptions/<subscription id>/resourcegroups/<resourcegroupname>/providers/<providername>/<resourcename>)**_. As services transition to the new format you will see the Instance ID data field change from just the resource name to Resource ID. The Resource ID is the format used by the [Azure Resource Manager API](https://msdn.microsoft.com/zh-cn/library/azure/dn790567.aspx) to identify resources in an a subscription.

![instanceid](./media/billing-understand-your-bill/instanceid.png)

- **Additional Info**: The Additional Info column in the usage .csv specifies service specific metadata. For example, an image type for a VM. Currently, a service emits service specific metadata in multiple columns: Additional Info, Service Info1, and Service Info 2 fields. Windows Azure services will be standardizing emitting service specific metadata in the Additional Info column only.  See the below snapshot of the standardized format:

![additionalinfo_csv2](./media/billing-understand-your-bill/AdditionaInfo_csv2.png)

- **Tags**:  This column contains the user specified resource tags. The tags can be used to group billing records. For example, you can use tags to distribute costs by department using the service. Learn more about [using tags to organize Azure <!-- deleted by customization Resources](/documentation/articles/resource-group-using-tags) --><!-- keep by customization: begin --> Resources](/resource-group-using-tags.md/) <!-- keep by customization: end -->.Services that support emitting tags are:
    - Virtual Machines
    - Storage and
...
- [Manage your payment method](https://msdn.microsoft.com/zh-cn/library/azure/dn736054.aspx)

- [Edit payment information for an existing credit card](https://msdn.microsoft.com/zh-cn/library/azure/dn736053.aspx)

- [Add a new credit card to use as a payment method](https://msdn.microsoft.com/zh-cn/library/azure/dn736057.aspx)

- [Change the credit card on your Windows Azure account](https://msdn.microsoft.com/zh-cn/library/azure/dn736050.aspx)

- [What do I do if my Azure subscription become disabled?](https://msdn.microsoft.com/zh-cn/library/azure/dn736049.aspx)
