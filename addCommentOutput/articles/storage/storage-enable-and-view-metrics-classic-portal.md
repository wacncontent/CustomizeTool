...
By default, Storage Metrics is not enabled for your storage services. You can enable monitoring using either the [Azure Management Portal](manage.windowsazure.cn), Windows PowerShell, or programmatically through a storage API.
...
In the [Azure Management Portal](manage.windowsazure.cn), you use the Configure page for a storage account to control Storage Metrics. For monitoring, you can set a level and a retention period in days for each of blobs, tables, and queues. In each case, the level is one of the following:
