<properties
			pageTitle="How to use Azure Files with Windows | Windows Azure"
...
You can create Azure file shares using the <!-- deleted by customization [Azure Management Portal](portal.azure.com) --><!-- keep by customization: begin --> [Management Portal](https://manage.windowsazure.cn) <!-- keep by customization: end -->, the Azure Storage PowerShell cmdlets, the Azure Storage client libraries, or the Azure Storage REST API. Additionally, because file shares are SMB shares, you can access them via standard and familiar file system APIs.
...
<!-- deleted by customization
## Use the Azure Management Portal to manage a file share

The [Azure Management Portal](portal.azure.com) provides a user interface for customers to manage File storage. From the preview portal, you can:
-->
...
To prepare to use PowerShell, download and install the Azure PowerShell cmdlets. See [How to install and configure Azure PowerShell](/documentation/articles/powershell-install-configure) for the install point and installation instructions.
...
Now, create the storage account context. The context encapsulates the storage account name and account key. For instructions on copying your account key from the <!-- deleted by customization [Azure Management Portal](portal.azure.com) --><!-- keep by customization: begin --> [Management portal](https://manage.windowsazure.cn) <!-- keep by customization: end -->, see [View, copy, and regenerate storage access keys](/documentation/articles/storage-create-storage-account#view-copy-and-regenerate-storage-access-keys).
...
1. First, create a new Azure virtual machine by following the instructions in [Create a virtual machine running Windows Server](/documentation/articles/virtual-machines-windows-tutorial-classic-portal).
2. Next, remote into the virtual machine by following the instructions in [How to <!-- deleted by customization log --><!-- keep by customization: begin --> Log <!-- keep by customization: end --> on to a virtual machine running Windows Server](/documentation/articles/virtual-machines-log-on-windows-server).
...
You can enable metrics for File storage from the <!-- deleted by customization [Azure Management Portal](portal.azure.com) --><!-- keep by customization: begin --> [Management portal](https://manage.windowsazure.cn) <!-- keep by customization: end -->. You can also enable metrics programmatically by calling the Set File Service Properties operation via the REST API, or one of its analogues in the Storage Client Library.
...
### Conceptual articles <!-- deleted by customization and videos -->
