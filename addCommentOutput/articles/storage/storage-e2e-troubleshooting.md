<properties 
	pageTitle="End-to-End Troubleshooting using Azure Storage Metrics and Logging, AzCopy, and Message Analyzer | Windows Azure" 
...
Diagnosing and troubleshooting is a key skill for building and supporting client applications with Windows Azure Storage. Due to the distributed nature of an Azure application, diagnosing and troubleshooting errors and performance issues may be more complex than in traditional environments.
...
This tutorial provides a hands-on exploration of an end-to-end troubleshooting scenario. For an in-depth conceptual guide to troubleshooting Azure storage applications, see [Monitor, diagnose, and troubleshoot <!-- deleted by customization Storage](/documentation/articles/storage-monitoring-diagnosing-troubleshooting) --><!-- keep by customization: begin --> Storage](/documentation/articles/storage-monitoring-diagnosing-troubleshooting/) <!-- keep by customization: end -->.

## Tools for troubleshooting Azure Storage applications

To troubleshoot client applications using Windows Azure Storage, you can use a combination of tools to determine when an issue has occurred and what the cause of the problem may be. These tools include:
...
- **The Azure Management Portal**. You can configure metrics and logging for your storage account in the portal. You can also view charts and graphs that show how your application is performing over time, and configure alerts in the portal to notify you if your application performs differently than expected for a specified metric. 
	...
	This tutorial shows how to monitor your storage account using the [Azure Management Portal](https://manage.windowsazure.cn/). See [How to monitor a storage <!-- deleted by customization account](/documentation/articles/storage-monitor-storage-account) --><!-- keep by customization: begin --> account](/documentation/articles/storage-monitor-storage-account/) <!-- keep by customization: end --> for information about configuring monitoring in the portal.
...
To configure logging and metrics for your storage account using the portal, follow the instructions at [How to monitor a storage <!-- deleted by customization account](/documentation/articles/storage-monitor-storage-account) --><!-- keep by customization: begin --> account](/documentation/articles/storage-monitor-storage-account/) <!-- keep by customization: end -->.
...
To get started with PowerShell for Azure, see [How to install and configure Azure <!-- deleted by customization PowerShell](/documentation/articles/install-configure-powershell) --><!-- keep by customization: begin --> PowerShell](/documentation/articles/install-configure-powershell/) <!-- keep by customization: end -->.
...
To configure client-side logging for a .NET application, enable .NET diagnostics in the application's configuration file (web.config or app.config). See [Client-side Logging using the Storage Client Library](http://msdn.microsoft.com/zh-cn/library/azure/dn782839.aspx) and [Client-side Logging with the Microsoft Azure Storage SDK for Java](http://msdn.microsoft.com/zh-cn/library/azure/dn782844.aspx) on MSDN for details.
...
Client-side logging is configured within the app.config or web.config file in your application. For details, see [Client-side logging using the Storage Client Library](http://msdn.microsoft.com/zh-cn/library/azure/dn782839.aspx) on MSDN.
...
For more details on adding metrics to the Monitoring page, see [How to: Add metrics to the metrics <!-- deleted by customization table](/documentation/articles/storage-monitor-storage-account#addmonitoringmetrics) --><!-- keep by customization: begin --> table](/documentation/articles/storage-monitor-storage-account/#addmonitoringmetrics) <!-- keep by customization: end -->.
...
The Azure Storage Client Library automatically generates a unique client request ID for every request. This value is written to the client log, the server log, and the network trace, so you can use it to correlate data across all three logs within Message Analyzer. See [Client request <!-- deleted by customization ID](/documentation/articles/storage-monitoring-diagnosing-troubleshooting#client-request-id) --><!-- keep by customization: begin --> ID](/documentation/articles/storage-monitoring-diagnosing-troubleshooting/#client-request-id) <!-- keep by customization: end --> for additional information about the client request ID.
