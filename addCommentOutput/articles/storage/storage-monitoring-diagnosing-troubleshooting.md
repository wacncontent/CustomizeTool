<properties
	pageTitle="Monitor, diagnose, and troubleshoot Storage | Windows Azure"
...
For a hands-on guide to end-to-end troubleshooting in Azure Storage applications, see [End-to-End Troubleshooting using Azure Storage Metrics and Logging, AzCopy, and Message Analyzer](/documentation/articles/storage-e2e-troubleshooting).
...
You can use the <!-- deleted by customization [Azure --><!-- keep by customization: begin --> Windows Azure <!-- keep by customization: end --> Management <!-- deleted by customization Portal](portal.azure.com) --><!-- keep by customization: begin --> Portal at <a href="https://manage.windowsazure.cn" target="_blank">https://manage.windowsazure.cn</a> <!-- keep by customization: end --> to view the health of the Storage service (and other Azure services) in all the Azure regions around the world. This enables you to see immediately if an issue outside of your control is affecting the Storage service in the region you use for your application.

The [Azure Management <!-- deleted by customization Portal](portal.azure.com) --><!-- keep by customization: begin --> Portal](https://manage.windowsazure.cn) <!-- keep by customization: end --> can also provide notifications of incidents that affect the various Azure services.
Note: This information was previously available, along with historical data, on the Azure Service Dashboard at <a <!-- deleted by customization href="http://status.azure.com" target="_blank">http://status.azure.com</a> --><!-- keep by customization: begin --> href="http://www.windowsazure.cn/support/service-dashboard/" target="_blank">http://www.windowsazure.cn/support/service-dashboard/</a> <!-- keep by customization: end -->.

While the <!-- deleted by customization [Azure Management Portal](portal.azure.com) --><!-- keep by customization: begin --> [Management portal](https://manage.windowsazure.cn) <!-- keep by customization: end --> collects health information from inside the Azure datacenters (inside-out monitoring), you could also consider adopting an outside-in approach to generate synthetic transactions that periodically access your Azure-hosted web site from multiple locations. The services offered by <a href="http://www.keynote.com/solutions/monitoring/web-monitoring" target="_blank">Keynote</a>, <a href="https://www.gomeznetworks.com/?g=1" target="_blank">Gomez</a>, and Application Insights for Visual Studio <!-- deleted by customization Team Services --><!-- keep by customization: begin --> Online <!-- keep by customization: end --> are examples of this outside-in approach. For more information about Application Insights for Visual Studio Team Services, see the appendix "[Appendix 5: Monitoring with Application Insights for Visual Studio Team Services]."
...
> [AZURE.NOTE] You should monitor these values for an early warning that you are approaching the capacity limits of your storage account. In the <!-- deleted by customization Azure --> Management Portal, you can add alert rules to notify you if aggregate storage use exceeds or falls below thresholds that you specify.
...
In the <!-- deleted by customization [Azure Management Portal](portal.azure.com) --><!-- keep by customization: begin --> [Management Portal](https://manage.windowsazure.cn) <!-- keep by customization: end -->, you can add alert rules to notify you if **Availability** for a service falls below a threshold that you specify.
...
- Microsoft Network Monitor (Netmon) (<a <!-- deleted by customization href="http://www.microsoft.com/download/details.aspx?id=4865" target="_blank">http://www.microsoft.com/download/details.aspx?id=4865</a>) --><!-- keep by customization: begin --> href="http://www.microsoft.com/zh-cn/download/details.aspx?id=4865" target="_blank">http://www.microsoft.com/zh-cn/download/details.aspx?id=4865</a>) <!-- keep by customization: end --> and Wireshark (<a href="http://www.wireshark.org/" target="_blank">http://www.wireshark.org/</a>) are free network protocol analyzers that enable you to view detailed packet information for a wide range of network protocols. For more information about Wireshark, see "[Appendix 2: Using Wireshark to capture network traffic]".
...
The illustration below from the <!-- deleted by customization [Azure Management Portal](portal.azure.com) --><!-- keep by customization: begin --> [Management Portal](https://manage.windowsazure.cn) <!-- keep by customization: end --> monitoring tool shows an example where the **AverageE2ELatency** is significantly higher than the **AverageServerLatency**.
...
> [AZURE.NOTE] You can find a comprehensive checklist including other issues to be aware here: [Windows Azure Storage Performance and Scalability Checklist](/documentation/articles/storage-performance-checklist).
