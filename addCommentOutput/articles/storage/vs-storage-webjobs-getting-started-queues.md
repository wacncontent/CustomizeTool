...
To write a function that the WebJobs SDK calls when a queue message is received, use the <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end --> attribute. The attribute constructor takes a string parameter that specifies the name of the queue to poll. You can also [set the queue name <!-- deleted by customization dynamically](/documentation/articles/how-to-set-configuration-options) --><!-- keep by customization: begin --> dynamically](#config) <!-- keep by customization: end -->.

### String queue messages

In the following example, the queue contains a string message, so <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end --> is applied to a string parameter named <!-- deleted by customization **logMessage** --><!-- keep by customization: begin --> `logMessage` <!-- keep by customization: end --> which contains the content of the queue message. The function [writes a log message to the <!-- deleted by customization Dashboard](#how-to-write-logs) --><!-- keep by customization: begin --> Dashboard](#logs) <!-- keep by customization: end -->.
...
Besides <!-- deleted by customization **string** --><!-- keep by customization: begin --> `string` <!-- keep by customization: end -->, the parameter may be a byte array, a <!-- deleted by customization **CloudQueueMessage** --><!-- keep by customization: begin --> `CloudQueueMessage` <!-- keep by customization: end --> object, or a POCO  that you define.

### POCO [(Plain Old CLR Object](http://en.wikipedia.org/wiki/Plain_Old_CLR_Object)) queue messages

In the following example, the queue message contains JSON for a <!-- deleted by customization **BlobInformation** --><!-- keep by customization: begin --> `BlobInformation` <!-- keep by customization: end --> object which includes a <!-- deleted by customization **BlobName** --><!-- keep by customization: begin --> `BlobName` <!-- keep by customization: end --> property. The SDK automatically deserializes the object.
...
The following async function [writes a log to the <!-- deleted by customization Dashboard](#how-to-write-logs) --><!-- keep by customization: begin --> Dashboard](#logs) <!-- keep by customization: end -->.
...
Async functions may take a [cancellation token](http://www.asp.net/mvc/overview/performance/using-asynchronous-methods-in-aspnet-mvc-4#CancelToken), as shown in the following example which copies a blob. (For an explanation of the <!-- deleted by customization **queueTrigger** --><!-- keep by customization: begin --> `queueTrigger` <!-- keep by customization: end --> placeholder, see the <!-- deleted by customization [Blobs](#how-to-read-and-write-blobs-and-tables-while-processing-a-queue-message) --><!-- keep by customization: begin --> [Blobs](#blobs) <!-- keep by customization: end --> section.)
...
<!-- deleted by customization ## --><!-- keep by customization: begin --> ### <!-- keep by customization: end --> Types the QueueTrigger attribute works with

You can use <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end --> with the following types:
...
The same is true when multiple messages are received for a single queue. By default, the SDK gets a batch of 16 queue messages at a time and executes the function that processes them in parallel. [The batch size is <!-- deleted by customization configurable](#how-to-set-configuration-options) --><!-- keep by customization: begin --> configurable](#config) <!-- keep by customization: end -->. When the number being processed gets down to half of the batch size, the SDK gets another batch and starts processing those messages. Therefore the maximum number of concurrent messages being processed per function is one and a half times the batch size. This limit applies separately to each function that has a <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end --> attribute. If you don't want parallel execution for messages received on one queue, set the batch size to 1.
...
To write a function that creates a new queue message, use the <!-- deleted by customization **Queue** --><!-- keep by customization: begin --> `Queue` <!-- keep by customization: end --> attribute. Like <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end -->, you pass in the queue name as a string or you can [set the queue name <!-- deleted by customization dynamically](#how-to-set-configuration-options) --><!-- keep by customization: begin --> dynamically](#config) <!-- keep by customization: end -->.

### String queue messages

The following non-async code sample creates a new queue message in the queue named "outputqueue" with the same content as the queue message received in the queue named "inputqueue". (For async functions use <!-- deleted by customization **IAsyncCollector<T>** --><!-- keep by customization: begin --> `IAsyncCollector<T>` <!-- keep by customization: end --> as shown later in this section.)
...
To create a queue message that contains a POCO rather than a string, pass the POCO type as an output parameter to the <!-- deleted by customization **Queue** --><!-- keep by customization: begin --> `Queue` <!-- keep by customization: end --> attribute constructor.
		public static void CreateQueueMessage(
...
To create multiple messages, make the parameter type for the output queue <!-- deleted by customization **ICollector<T>** --><!-- keep by customization: begin --> `ICollector<T>` <!-- keep by customization: end --> or <!-- deleted by customization **IAsyncCollector<T>** --><!-- keep by customization: begin --> `IAsyncCollector<T>` <!-- keep by customization: end -->, as shown in the following example.
...
Each queue message is created immediately when the <!-- deleted by customization **Add** --><!-- keep by customization: begin --> `Add` <!-- keep by customization: end --> method is called.

### Types that the Queue attribute works with

You can use the <!-- deleted by customization **Queue** --><!-- keep by customization: begin --> `Queue` <!-- keep by customization: end --> attribute on the following parameter types:

* <!-- deleted by customization **out string** --><!-- keep by customization: begin --> `out string` <!-- keep by customization: end --> (creates queue message if parameter value is non-null when the function ends)
<!-- deleted by customization
...
The <!-- deleted by customization **Blob** --><!-- keep by customization: begin --> `Blob` <!-- keep by customization: end --> attribute constructor takes a <!-- deleted by customization **blobPath** --><!-- keep by customization: begin --> `blobPath` <!-- keep by customization: end --> parameter that specifies the container and blob name. For more information about this placeholder, see [How to use Azure blob storage with the WebJobs SDK](/documentation/articles/websites-dotnet-webjobs-sdk-storage-blobs-how-to),

When the attribute decorates a <!-- deleted by customization **Stream** --><!-- keep by customization: begin --> `Stream` <!-- keep by customization: end --> object, another constructor parameter specifies the <!-- deleted by customization **FileAccess** --><!-- keep by customization: begin --> `FileAccess` <!-- keep by customization: end --> mode as read, write, or read/write.

The following example uses a <!-- deleted by customization **CloudBlockBlob** --><!-- keep by customization: begin --> `CloudBlockBlob` <!-- keep by customization: end --> object to delete a blob. The queue message is the name of the blob.
...
If you need to do some work in your function before binding a blob to an object, you can use the attribute in the body of the function, [as shown earlier for the Queue <!-- deleted by customization attribute](#use-webjobs-sdk-attributes-in-the-body-of-a-function) --><!-- keep by customization: begin --> attribute](#ibinder) <!-- keep by customization: end -->.
...
The SDK will call a function up to 5 times to process a queue message. If the fifth try fails, the message is moved to a poison queue. [The maximum number of retries is <!-- deleted by customization configurable](#how-to-set-configuration-options) --><!-- keep by customization: begin --> configurable](#config) <!-- keep by customization: end -->.
...
In the following example the <!-- deleted by customization **CopyBlob** --><!-- keep by customization: begin --> `CopyBlob` <!-- keep by customization: end --> function will fail when a queue message contains the name of a blob that doesn't exist. When that happens, the message is moved from the copyblobqueue queue to the copyblobqueue-poison queue. The <!-- deleted by customization **ProcessPoisonMessage** --><!-- keep by customization: begin --> `ProcessPoisonMessage` <!-- keep by customization: end --> then logs the poison message.
...
You can get the number of times a message has been picked up for processing by adding an <!-- deleted by customization **int** --><!-- keep by customization: begin --> `int` <!-- keep by customization: end --> parameter named <!-- deleted by customization **dequeueCount** --><!-- keep by customization: begin --> `dequeueCount` <!-- keep by customization: end --> to your function. You can then check the dequeue count in function code and perform your own poison message handling when the number exceeds a threshold, as shown in the following example.
...
* Configure <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end --> settings such as maximum dequeue count.
...
<!-- deleted by customization ### Set --><!-- keep by customization: begin --> ###Set <!-- keep by customization: end --> values for WebJobs SDK constructor parameters in code

Sometimes you want to specify a queue name, a blob name or container, or a table name in code rather than hard-code it. For example, you might want to specify the queue name for <!-- deleted by customization **QueueTrigger** --><!-- keep by customization: begin --> `QueueTrigger` <!-- keep by customization: end --> in a configuration file or environment variable.

You can do that by passing in a <!-- deleted by customization **NameResolver** --><!-- keep by customization: begin --> `NameResolver` <!-- keep by customization: end --> object to the <!-- deleted by customization **JobHostConfiguration** --><!-- keep by customization: begin --> `JobHostConfiguration` <!-- keep by customization: end --> type. You include special placeholders surrounded by percent (%) signs in WebJobs SDK attribute constructor parameters, and your <!-- deleted by customization **NameResolver** --><!-- keep by customization: begin --> `NameResolver` <!-- keep by customization: end --> code specifies the actual values to be used in place of those placeholders.

For example, suppose you want to use a queue named logqueuetest in the test environment and one named logqueueprod in production. Instead of a hard-coded queue name, you want to specify the name of an entry in the <!-- deleted by customization **appSettings** --><!-- keep by customization: begin --> `appSettings` <!-- keep by customization: end --> collection that would have the actual queue name. If the <!-- deleted by customization **appSettings** --><!-- keep by customization: begin --> `appSettings` <!-- keep by customization: end --> key is logqueue, your function could look like the following example.
...
You can disable logging by [setting the Dashboard connection string to <!-- deleted by customization null](#how-to-set-configuration-options) --><!-- keep by customization: begin --> null](#config) <!-- keep by customization: end -->.
...
In the WebJobs SDK Dashboard, the output from the <!-- deleted by customization **TextWriter** --><!-- keep by customization: begin --> `TextWriter` <!-- keep by customization: end --> object shows up when you go to the page for a particular function invocation and click **Toggle Output**:
...
This article has provided code samples that show how to handle common scenarios for working with Azure queues. For more information about how to use Azure WebJobs and the WebJobs SDK, see [Azure WebJobs Recommended <!-- deleted by customization Resources](/documentation/articles/websites-webjobs-resources/) --><!-- keep by customization: begin --> Resources](http://www.windowsazure.cn/documentation/articles/websites-webjobs-resources/) <!-- keep by customization: end -->.