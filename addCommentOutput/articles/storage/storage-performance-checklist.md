...
For more information about Azure CDN, see [Azure <!-- deleted by customization CDN](/home/features/cdn/) --><!-- keep by customization: begin --> CDN](/home/features/caching/) <!-- keep by customization: end -->.
...
For more information about CORS, see [Cross-Origin Resource Sharing (CORS) Support for the Azure Storage <!-- deleted by customization Services](http://msdn.microsoft.com/zh-cn/library/azure/dn535601.aspx) --><!-- keep by customization: begin --> Services on MSDN](http://msdn.microsoft.com/zh-cn/library/azure/dn535601.aspx) <!-- keep by customization: end -->.
...
For an example of how to get a blob’s properties to discover the last modified date using .NET, see [Set and Retrieve Properties and <!-- deleted by customization Metadata](/documentation/articles/storage-properties-metadata) --><!-- keep by customization: begin --> Metadata](http://msdn.microsoft.com/zh-cn/library/hh225342.aspx) on MSDN <!-- keep by customization: end -->. For more information about conditional downloads, see [Conditionally Refresh a Local Copy of a Blob](http://msdn.microsoft.com/zh-cn/library/azure/dd179371.aspx) <!-- keep by customization: begin --> on MSDN <!-- keep by customization: end -->.
...
For additional information, see the <!-- deleted by customization blog --> post [Web Services: Concurrent Connections](http://blogs.msdn.com/b/darrenj/archive/2005/03/07/386655.aspx) <!-- keep by customization: begin --> on MSDN <!-- keep by customization: end -->.
...
For more information, see [ThreadPool.SetMinThreads Method](http://msdn.microsoft.com/zh-cn/library/system.threading.threadpool.setminthreads(v=vs.110).aspx) <!-- keep by customization: begin --> on MSDN <!-- keep by customization: end -->.
...
For more information, see the article [An Overview of Performance Improvements in .NET <!-- deleted by customization 4.5](http://msdn.microsoft.com/magazine/hh882452.aspx) --><!-- keep by customization: begin --> 4.5](http://msdn.microsoft.com/zh-cn/magazine/hh882452.aspx) <!-- keep by customization: end --><!-- keep by customization: begin --> on MSDN <!-- keep by customization: end -->.
...
Always use the latest Microsoft provided client libraries and tools. At the time of writing, there are <!-- deleted by customization client --><!-- keep by customization: begin --> RTM <!-- keep by customization: end --> libraries available for .NET, Windows Phone, Windows Runtime, <!-- keep by customization: begin --> and <!-- keep by customization: end --> Java <!-- deleted by customization, and C++ -->, as well as preview libraries <!-- deleted by customization for --><!-- keep by customization: begin --> in <!-- keep by customization: end --> other languages <!-- keep by customization: begin --> like node.js and C++ <!-- keep by customization: end -->. In addition, Microsoft has released PowerShell cmdlets and Azure CLI commands for working with Azure Storage. Microsoft actively develops these tools with performance in mind, keeps them up to date with the latest service versions, and ensures they handle many of the proven performance practices internally.
...
####Useful Resources
For more information about storage error codes, see [Status and Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179382.aspx) on the Windows Azure <!-- deleted by customization web site --><!-- keep by customization: begin -->  Website <!-- keep by customization: end -->.
...
For more information about target throughput for blobs, see [Azure Storage Scalability and Performance <!-- deleted by customization Targets](/documentation/articles/storage-scalability-targets) --><!-- keep by customization: begin --> Targets](http://msdn.microsoft.com/zh-cn/library/azure/dn249410.aspx) on MSDN <!-- keep by customization: end -->.
...
For very large volumes of data (more than 1TB), the Azure Storage offers the Import/Export service, which allows for uploading and downloading from blob storage by shipping hard drives.  You can put your data on a hard drive and send it to Microsoft for upload, or send a blank hard drive to Microsoft to download data.  You can read more about it <!-- deleted by customization [here](/documentation/articles/storage-import-export-service) --><!-- keep by customization: begin --> [here](/documentation/articles/storage-import-export-service/) <!-- keep by customization: end -->.  This can be much more efficient than uploading/downloading this volume of data over the network.

###<a name="subheading20"></a>Use metadata
The blob service supports head requests, which can include metadata about the blob. For example, if your application needed the EXIF data out of a photo, it could retrieve the photo and extract it. To save bandwidth and improve performance, your application could store the EXIF data in the blob’s metadata when the application uploaded the photo: you can then retrieve the EXIF data in metadata using only a HEAD request, saving significant bandwidth and the processing time needed to extract the EXIF data each time the blob is read. This would be useful in scenarios where you only need the metadata, and not the full content of a blob.  Note that only <!-- deleted by customization 8 KB --><!-- keep by customization: begin --> 8KB <!-- keep by customization: end --> of metadata can be stored per blob (the service will not accept a request to store more than that), so if the data does not fit in that size, you may not be able to use this approach.

For an example of how to get a blob’s metadata using .NET, see [Set and Retrieve Properties and <!-- deleted by customization Metadata](/documentation/articles/storage-properties-metadata) --><!-- keep by customization: begin --> Metadata](http://msdn.microsoft.com/zh-cn/library/hh225342.aspx) on MSDN <!-- keep by customization: end -->.
...
To upload many blobs quickly, upload blobs in parallel. This is faster than uploading single blobs at a time with parallel block uploads because it spreads the upload across multiple partitions of the storage service. A single blob only supports a throughput of 60 MB/second (approximately 480 Mbps). At the time of writing, a <!-- deleted by customization US based LRS --><!-- keep by customization: begin --> GRS <!-- keep by customization: end --> account supports up to 20 Gbps ingress which is far more than the throughput supported by an individual blob.  [AzCopy](#subheading18) performs uploads in parallel by default, and is recommended for this scenario.
...
For more information, see [Understanding Block Blobs and Page Blobs](http://msdn.microsoft.com/zh-cn/library/azure/ee691964.aspx) <!-- keep by customization: begin --> on MSDN <!-- keep by customization: end -->.
...
For more information, see the post [Windows Azure Tables: Introducing JSON](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/05/windows-azure-tables-introducing-json.aspx) and [Payload Format for Table Service Operations](http://msdn.microsoft.com/zh-cn/library/azure/dn535600.aspx) <!-- keep by customization: begin --> on MSDN <!-- keep by customization: end -->.
