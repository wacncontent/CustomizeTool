...
#Azure Notification Hubs Notify Users <!-- deleted by customization for Android with .NET backend -->
...
<!-- keep by customization: end -->
	...
	This component implements the REST calls required to contact the app backend, in order to register for push notifications. It also locally stores the *registrationIds* created by the Notification Hub as detailed in [Registering from your app <!-- deleted by customization backend](/documentation/articles/notification-hubs-registration-management#registration-management-from-a-backend) --><!-- keep by customization: begin --> backend](http://msdn.microsoft.com/zh-cn/library/dn743807.aspx) <!-- keep by customization: end -->. Note that it uses an authorization token stored in local storage when you click the **Log in** button.
