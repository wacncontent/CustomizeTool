...
First, create an app in the [App Registration Portal](https://apps.dev.microsoft.com), or follow these [detailed steps](/documentation/articles/active-directory-v2-app-registration).  Make sure to:
...
That's all the time we're going to spend discussing how the REST API works.  Feel free to poke around in the code, but if you want to learn more about securing web APIs with Azure AD, check out [this article](/documentation/articles/active-directory-v2-devquickstarts-dotnet-api). 
...
Congratulations!  Your Azure AD integrated single page app is now complete.  Go ahead, take a bow.  It can authenticate users, securely call its backend REST API using OpenID Connect, and get basic information about the user.  Out of the box, it supports any user with a personal Microsoft Account or a work/school account from Azure AD.  Run the app, and in a browser navigate to `https://localhost:44326/`.  Sign in using either a personal Microsoft account or a work/school account.  Add tasks to the user's to-do list, and sign out.  Try signing in with the other type of account. If you need an Azure AD tenant to create work/school users, [learn how to get one here](/documentation/articles/active-directory-howto-tenant) (it's free).

To continue learning about the app model v2.0 preview, head back to our [v2.0 developer guide](/documentation/articles/active-directory-appmodel-v2-overview).  For additional resources, check out:
