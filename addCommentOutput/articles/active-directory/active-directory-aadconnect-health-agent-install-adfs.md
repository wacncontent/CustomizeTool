...
> [AZURE.WARNING] If you have a group policy that is disabling AD FS auditing then the Azure AD Connect Health Agent will not be able to collect information. Ensure that you <!-- deleted by customization donâ€™t --><!-- keep by customization: begin --> don't <!-- keep by customization: end --> have a group policy that may be disabling auditing.
...
>[AZURE.NOTE]
- Using <!-- deleted by customization â€œNetsh --><!-- keep by customization: begin --> ¡°Netsh <!-- keep by customization: end --> WinHttp set <!-- deleted by customization ProxyServerAddressâ€ --><!-- keep by customization: begin --> ProxyServerAddress¡± <!-- keep by customization: end --> will not work as the agent uses System.Net to make web requests instead of Microsoft Windows HTTP Services.
