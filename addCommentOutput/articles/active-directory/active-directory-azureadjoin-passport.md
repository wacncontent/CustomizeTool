...
The current methods of authentication with passwords alone are not sufficient to keep users safe. Users reuse and forget passwords. Passwords are breachable, phishable, prone to cracks, and guessable. They also get difficult to remember and prone to attacks like <!-- deleted by customization “[pass --><!-- keep by customization: begin --> “pass <!-- keep by customization: end --> the <!-- deleted by customization hash](https://technet.microsoft.com/dn785092.aspx)” --><!-- keep by customization: begin --> hash” <!-- keep by customization: end -->.
...
The private key is made available solely through a “user gesture” like a PIN, biometrics, remote device like a smart card that the user used to <!-- deleted by customization log on --><!-- keep by customization: begin --> sign in <!-- keep by customization: end --> to the device and this information is linked to a certificate or an asymmetrical key pair. This private-key is hardware attested if device has a Trusted Platform Module (TPM) chip. The private key never leaves the device.
...
2. These hardware-bound keys are attested by the TPM <!-- deleted by customization. --><!-- keep by customization: begin --> and enterprises can <!-- keep by customization: end -->

3. Single unlock gesture will unlock the device and this gesture will be allowed to get access to multiple resources if the device is domain-joined or Azure <!-- deleted by customization AD-joined --><!-- keep by customization: begin --> AD Joined <!-- keep by customization: end -->.
...
2. <!-- deleted by customization The -->device then creates the keys, attests the key, takes the public portion of this key, attach it with station statements, signs in and sends it to IDP to register this key.
