...
The table below describes which scenarios are supported for which versions of our sync capabilities.  In general, it is highly recommended that you install the latest version of [Azure AD Connect](/documentation/articles/active-directory-aadconnect#download-azure-ad-connect) if you want to use password writeback.
...
- **Locked down, cryptographically strong, password encryption key** - After the service bus relay is created, we create a strong symmetric key which we use to encrypt the password as it comes over the wire.  This key lives only in your <!-- deleted by customization company's --><!-- keep by customization: begin --> tenant's <!-- keep by customization: end --> secret store in the cloud, which is heavily locked down and audited, just like any password in the directory.
