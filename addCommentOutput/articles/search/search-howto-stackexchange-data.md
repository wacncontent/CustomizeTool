...
This article is a walkthrough that highlights some of the core full-text search capabilities that can done with [Azure <!-- deleted by customization Search](https://azure.microsoft.com/services/search/) --><!-- keep by customization: begin --> Search](https://azure.microsoft.com/en-us/services/search/) <!-- keep by customization: end -->.  It leverages data that Stack Exchange made [available](https://archive.org/details/stackexchange) for Creative Commons usage with the following [attribution](http://blog.stackoverflow.com/2009/06/attribution-required/).
...
<!-- deleted by customization Let's --><!-- keep by customization: begin --> Let’s <!-- keep by customization: end --> start with a really simple full text search query where the users might type the word “azure” to find any StackExchange posts relating to Azure.  Give this a try by clicking on this link to see it in action:
...
-	`Spelling Mistakes`: Our new (preview) support for [Lucene Query <!-- deleted by customization Expressions](https://msdn.microsoft.com/zh-cn/library/mt589323.aspx) --><!-- keep by customization: begin --> Expressions](https://msdn.microsoft.com/library/mt589323.aspx) <!-- keep by customization: end --> also allows you to do some pretty fancy queries such as fuzzy matching of results and limiting search to specific fields.  This example searches the title field for the word “visualize” but the ~ indicates fuzzy matching which means that results like visualise and visualizing will also be returned.
...
-	`Scoring and Tuning`: [Scoring <!-- deleted by customization Profiles](https://msdn.microsoft.com/zh-cn/library/azure/dn798928.aspx) --><!-- keep by customization: begin --> Profiles](https://msdn.microsoft.com/library/azure/dn798928.aspx) <!-- keep by customization: end --> are extremely helpful in helping you tune the results that are returned by the search service.  In fact, now we can also use [Lucene Query <!-- deleted by customization Expressions](https://msdn.microsoft.com/zh-cn/library/mt589323.aspx) --><!-- keep by customization: begin --> Expressions](https://msdn.microsoft.com/library/mt589323.aspx) <!-- keep by customization: end --> to apply scoring to individual fields and terms on the fly.  For example, if we wanted to search for the words “visualize” or “chart” in the title field, yet give more weighting to items that have the word “chart” in them, we could do this:
...
<!-- deleted by customization -	[`Suggestions`](https://msdn.microsoft.com/zh-cn/library/azure/mt131377.aspx) --><!-- keep by customization: begin --> -	[`Suggestions`](https://msdn.microsoft.com/library/azure/mt131377.aspx) <!-- keep by customization: end -->: As the users type in to the search box, it will be convenient to use fields like Title, Tags and <!-- deleted by customization UserName's --><!-- keep by customization: begin --> UserName’s <!-- keep by customization: end --> for autocompletion.
...
The only thing I did beyond what Brent outlined was to create a View in my Azure SQL database which will be used by the [Azure Search <!-- deleted by customization Indexer](https://msdn.microsoft.com/zh-cn/library/azure/dn946891.aspx) --><!-- keep by customization: begin --> Indexer](https://msdn.microsoft.com/library/azure/dn946891.aspx) <!-- keep by customization: end --> to crawl and ingest data from a group of tables that I am interested in using.  Here is how this View is defined.
...
Once this is done, you can then use the [Azure <!-- deleted by customization Management Portal](https://manage.windowsazure.cn) --><!-- keep by customization: begin --> Portal](https://portal.azure.com) <!-- keep by customization: end --> to “Import Data” from the above Azure SQL View which will then create an Azure Search index based on the schema of the fields in the View.  If you would like to use the Azure SQL database that I have staged, here is the Read-Only connection string that you can use:
