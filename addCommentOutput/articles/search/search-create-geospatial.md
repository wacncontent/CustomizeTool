...
This tutorial builds on the [Azure Search <!-- deleted by customization - --><!-- keep by customization: begin --> – <!-- keep by customization: end --> Adventure Works Demo](http://azuresearchadventureworksdemo.codeplex.com). If you <!-- deleted by customization haven't --><!-- keep by customization: begin --> haven’t <!-- keep by customization: end --> walked through that demo yet, start there to get some experience in creating an index and calling the Azure Search API from a web <!-- deleted by customization site --><!-- keep by customization: begin --> app <!-- keep by customization: end -->.
...
[AZURE.INCLUDE [You need an Azure account to complete this <!-- deleted by customization tutorial:](../includes/free-trial-note.md)] --><!-- keep by customization: begin --> tutorial:](../../includes/free-trial-note.md)] <!-- keep by customization: end -->
...
+ **Bing Maps:** When the app runs, <!-- deleted by customization we'll --><!-- keep by customization: begin --> we’ll <!-- keep by customization: end --> use Bing Maps to display store locations, overlayed on top of a Bing Map.
...
This data comes from a CSV file called store_locations.csv located in the source you downloaded earlier. If you open up this file in a text editor or Excel, <!-- deleted by customization you'll --><!-- keep by customization: begin --> you’ll <!-- keep by customization: end --> see that it has an ID column for each store, the name of the store, and its address.

Let's walk through the code that explains how this works.

1. Open the AdventureWorksGeo solution in Visual Studio, expand the project **StoreIndexer** in the Solution Explorer and open Program.cs. Since we already covered index creation in the [Azure Search <!-- deleted by customization - --><!-- keep by customization: begin --> – <!-- keep by customization: end --> Adventure Works Demo](http://azuresearchadventureworksdemo.codeplex.com/), we'll skip the discussion on how that works in Program.cs.
...
8. Before running the application, add your Azure Search and Bing Maps API information by opening App.config and update the values for "SearchServiceName", "SearchServiceApiKey" and "BingMapsAPI" to those of your Azure Search service and Bing Maps API. For Search service name, if your service is <!-- deleted by customization "mysearch.search.chinacloudapi.cn" --><!-- keep by customization: begin --> "mysearch.search.windows.net" <!-- keep by customization: end -->, you would enter "mysearch".
...
3.	Open Web.config in this project and update the values for "SearchServiceName", "SearchServiceApiKey" and "BingMapsAPI" to those of your Azure Search service and Bing Maps API. For Search service name, if your service is <!-- deleted by customization "mysearch.search.chinacloudapi.cn" --><!-- keep by customization: begin --> "mysearch.search.windows.net" <!-- keep by customization: end -->, you would enter "mysearch".
...
3.	This file follows the typical way that you would add Bing Maps to a web <!-- deleted by customization site --><!-- keep by customization: begin --> application <!-- keep by customization: end -->, however a few things to point out are:
