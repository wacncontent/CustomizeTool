...
# Try Azure Search for free with Azure Websites

[Azure Websites](https://tryappservice.azure.com/) is a new, entirely free way to test-drive selected Azure Services, including Azure Search, for up to one hour with no Azure subscription sign-up required.
...
On the [Azure Websites](https://tryappservice.azure.com/) site, Azure Search is part of the web site template - providing a rich, full-text search experience along with a slew of search-centric features available only in this service on the Azure platform.
...
As part of the [Azure Websites](https://tryappservice.azure.com/) offering, the Azure Search service you'll use has already been created for your and is ready to receive search queries. You cannot upload or use your own index or data. The data you'll work with is from the [United States Geological Survey (USGS)](http://geonames.usgs.gov/domestic/download_data.htm), consisting of about 3 million rows of landmarks, historical sites, buildings, and other landmark features across the US.
-->
...
Before moving ahead, you might want to take a few minutes review a few key points about the code, service, and searchable data. Having a little background could prove useful if <!-- deleted by customization you're --><!-- keep by customization: begin --> you’re <!-- keep by customization: end --> not already familiar with Azure Search.
...
In the [Azure Websites](https://tryappservice.azure.com/) option, the "ASP.NET + Azure Search Site" template provides source code for the web site, modifiable in Visual Studio Online (available as a part of the one-hour session). No separate development tools are required to view or change the code.

Code is written in C#, using the [Azure Search .NET client library](https://msdn.microsoft.com/zh-cn/library/dn951165.aspx) to execute queries against the index, provide faceted navigation, and display counts and search results in a web page.
-->
<!-- keep by customization: begin -->
- Data can be auto-loaded by an Indexer (supported for Azure SQL Database or Azure DocumentDB only), or pushed to a search index via one of the Azure Search APIs. When you use the API, you can push data from any data source, as long as it’s in JSON format.
...
1. Go to [https://tryappservice.azure.com](https://tryappservice.azure.com/) and scroll down to select **Web Apps**.
-->
<!-- keep by customization: begin -->
##Get started
...
6. Wait while the site is provisioned. When <!-- deleted by customization it's --><!-- keep by customization: begin --> it’s <!-- keep by customization: end --> ready, you will see a page similar to this one. Each page shows a running clock so that you always know how much time you have left.
...
Ready to move on? <!-- deleted by customization Let's --><!-- keep by customization: begin --> Let’s <!-- keep by customization: end --> change a few lines of code to see the impact on full-text search operations.

<!-- deleted by customization
## Change searchMode.All

Azure Search has a configurable **searchMode** property that you can use to control search operator behavior. Valid values for this property are `Any` (default) or `All`. See [Simple Query Syntax](https://msdn.microsoft.com/zh-cn/library/dn798920.aspx) for more guidance on setting these options.
-->
<!-- keep by customization: begin -->
##Change searchMode.All
...
Admittedly, this is perhaps an unlikely scenario, but it illustrates the effects of **searchMode** on search phrases that include the `NOT` operator, so <!-- deleted by customization it's --><!-- keep by customization: begin --> it’s <!-- keep by customization: end --> useful to understand why the behavior occurs and how to change it if this is not what you want.
...
Filters are specified using OData syntax and are frequently used with faceted navigation or included in the query string to constrain the query. See [OData Filter <!-- deleted by customization Syntax](https://msdn.microsoft.com/zh-cn/library/azure/dn798921.aspx) --><!-- keep by customization: begin --> Syntax](https://msdn.microsoft.com/library/azure/dn798921.aspx) <!-- keep by customization: end --> for more information.
...
The following code adds hit-highlighting. Notice the new properties declared in the <!-- deleted by customization [SearchParameters](https://msdn.microsoft.com/zh-cn/library/microsoft.azure.search.models.searchparameters_properties.aspx) --><!-- keep by customization: begin --> [SearchParameters](https://msdn.microsoft.com/library/microsoft.azure.search.models.searchparameters_properties.aspx) <!-- keep by customization: end -->. There is also a new function that iterates over the collection of results to get the documents that need highlighting, plus HTML that renders the result.
...
Using the read-only service provided in [Try Azure Websites](https://tryappservice.azure.com/) site, you have seen the query syntax and full-text search in action, learned about searchMode and filters, and added hit-highlighting to your search application. As your next step, consider moving on to creating and updating indexes. This adds the ability to:

- [Define scoring profiles](https://msdn.microsoft.com/zh-cn/library/dn798928.aspx) used for tuning search scores so that high-value items show up first.
- [Define Suggesters](https://msdn.microsoft.com/zh-cn/library/mt131377.aspx) that add auto-complete or type-ahead query suggestions in response to user input.
- [Define indexers](https://msdn.microsoft.com/zh-cn/library/dn946891.aspx) that update your index automatically whenever the data source is Azure SQL Database or Azure DocumentDB.

To perform all these tasks, you'll need an Azure subscription so that you can create and populate indexes in a service. For more information about how to sign up for a  trial, visit [https://azure.microsoft.com/pricing/1rmb-trial](/pricing/1rmb-trial/).

To learn more about Azure Search, visit our [documentation page](/documentation/services/search/) on [http://azure.microsoft.com](http://azure.microsoft.com) or check out any number of [samples and videos](/documentation/articles/search-video-demo-tutorial-list) that explore the full range of Azure Search functionality.
-->
<!-- keep by customization: begin -->
##Next steps
...
<!-- keep by customization: end -->
    ...
    {
<!-- deleted by customization
      "@odata.context": "https://tryappservice.search.chinacloudapi.cn/$metadata#indexes/$entity",
-->
<!-- keep by customization: begin -->
