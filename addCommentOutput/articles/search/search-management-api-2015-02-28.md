...
<!-- deleted by customization Azure Search is a hosted cloud search service on Windows Azure. --> This document describes the **2015-02-28* version of the Azure Search Management REST API. It has since been replaced by newer versions. For the latest, see [Azure Search Management REST API <!-- deleted by customization 2015-08-19](https://msdn.microsoft.com/zh-cn/library/dn832684.aspx) --><!-- keep by customization: begin --> 2015-08-19](https://msdn.microsoft.com/library/dn832684.aspx) <!-- keep by customization: end --> on MSDN.
...
To fully administer your service programmatically, you will need two APIs: The Management REST API of Azure Search, plus the common [Azure Resource Manager REST <!-- deleted by customization API](https://msdn.microsoft.com/zh-cn/library/azure/dn790568.aspx) --><!-- keep by customization: begin --> API](https://msdn.microsoft.com/library/azure/dn790568.aspx) <!-- keep by customization: end -->. The Resource Manager API is used for general purpose operations that are not service specific, such as querying subscription data, listing geo-locations, and so forth. To create and manage Azure Search services in your subscription, make sure your HTTP request includes the Resource Manager endpoint, subscription ID, provider (in this case, Azure Search), and the Search service-specific operation.
...
The endpoint for service administration operations is the URL of Azure Resource Manager, <!-- deleted by customization `https://manage.windowsazure.cn` --><!-- keep by customization: begin --> `https://management.azure.com` <!-- keep by customization: end -->.
...
- Data requests against the Azure Search service endpoint, such as Create Index or Search Documents, use an `api-key` in the request header. See [Azure Search Service REST <!-- deleted by customization API](https://msdn.microsoft.com/zh-cn/library/azure/dn798935.aspx) --><!-- keep by customization: begin --> API](https://msdn.microsoft.com/library/azure/dn798935.aspx) <!-- keep by customization: end --> for information about authenticating a data request.
...
Access control for Azure Resource Manager uses the built-in Owner, Contributor, and Reader roles. By default, all service administrators are members of the Owner role. For details, see [Role-based access control in the Azure <!-- deleted by customization Management Portal](/documentation/articles/role-based-access-control-configure) --><!-- keep by customization: begin --> portal](../role-based-access-control-configure.md) <!-- keep by customization: end -->.
...
`resourceGroupName`: Required. The name of the resource group within the <!-- deleted by customization user's --><!-- keep by customization: begin --> user’s <!-- keep by customization: end --> subscription. You can obtain this value from the Azure Resource Manager API or the portal.

`serviceName`: Required. The name of the search service within the specified resource group. Service names must only contain lowercase letters, digits or dashes, cannot use dash as the first two or last one characters, cannot contain consecutive dashes, and must be between 2 and 15 characters in length. Since all names end up being <!-- deleted by customization <name>.search.chinacloudapi.cn --><!-- keep by customization: begin --> <name>.search.windows.net <!-- keep by customization: end -->, service names must be globally unique. No two services either within or across subscriptions and resource groups can have the same name. You cannot change the service name after it is created.
...
`location`: Required. One of the supported and registered Azure Geo Regions (for example, <!-- deleted by customization China North --><!-- keep by customization: begin --> West US <!-- keep by customization: end -->, <!-- deleted by customization China --> East <!-- keep by customization: begin --> US <!-- keep by customization: end -->, <!-- deleted by customization China North --><!-- keep by customization: begin --> Southeast Asia <!-- keep by customization: end -->, and so on). Note that the location of a resource cannot be changed once it is created.
...
`location`:	The location of the resource. This will be one of the supported and registered Azure Geo Regions (for example, <!-- deleted by customization China North --><!-- keep by customization: begin --> West US <!-- keep by customization: end -->, <!-- deleted by customization China --> East <!-- keep by customization: begin --> US <!-- keep by customization: end -->, <!-- deleted by customization China North --><!-- keep by customization: begin --> Southeast Asia <!-- keep by customization: end -->, and so forth).
...
Note that the `nextLink` field is always null because the current version <!-- deleted by customization doesn't --><!-- keep by customization: begin --> doesn’t <!-- keep by customization: end --> support paging. Returning it with an empty value is done to preserve future compatibility.
