<properties
	pageTitle="Scoring profiles (Azure Search REST API Version 2015-02-28-Preview) | Windows Azure | Hosted cloud search service"
...
> [AZURE.NOTE] This article describes scoring profiles in the <!-- deleted by customization [2015-02-28-Preview](/documentation/articles/search-api-2015-02-28-preview) --><!-- keep by customization: begin --> [2015-02-28-Preview](search-api-2015-02-28-preview.md) <!-- keep by customization: end -->. Currently there is no difference between the `2015-02-28` version documented on <!-- deleted by customization [MSDN](http://msdn.microsoft.com/zh-cn/library/azure/mt183328.aspx) --><!-- keep by customization: begin --> [MSDN](http://msdn.microsoft.com/library/azure/mt183328.aspx) <!-- keep by customization: end --> and the `2015-02-28-Preview` version described here.
...
This query searches on the term 'inn' and passes in the current location. Note that this query includes other parameters, such as `scoringParameter`. Query parameters are described in [Search Documents (Azure Search <!-- deleted by customization API)](/documentation/articles/search-api-2015-02-28-preview#SearchDocs.md) --><!-- keep by customization: begin --> API)](search-api-2015-02-28-preview/#SearchDocs.md) <!-- keep by customization: end -->.
...
You should create one or more scoring profiles when the default ranking behavior <!-- deleted by customization doesn't --><!-- keep by customization: begin --> doesn’t <!-- keep by customization: end --> go far enough in meeting your business objectives. For example, you might decide that search relevance should favor newly added items. Likewise, you might have a field that contains profit margin, or some other field indicating revenue potential. Boosting hits that bring benefits to your business can be an important factor in deciding to use scoring profiles.

Relevancy-based ordering is also implemented through scoring profiles. Consider search results pages <!-- deleted by customization you've --><!-- keep by customization: begin --> you’ve <!-- keep by customization: end --> used in the past that let you sort by price, date, rating, or relevance. In Azure Search, scoring profiles drive the 'relevance' option. The definition of relevance is controlled by you, predicated on business objectives and the type of search experience you want to deliver.
...
[Try this example](/documentation/articles/search-get-started-scoring-profiles).
-->
...
Provide a name. Scoring profiles are optional, but if you add one, the name is required. Be sure to follow the naming conventions for fields (starts with a letter, avoids special characters and reserved words). See [Naming <!-- deleted by customization Rules](http://msdn.microsoft.com/zh-cn/library/azure/dn857353.aspx) --><!-- keep by customization: begin --> Rules](http://msdn.microsoft.com/library/azure/dn857353.aspx) <!-- keep by customization: end --> for more information.
      ...
      <td>
        ...
        <p>
          - `freshness` should be used when you want to boost by how new or old an item is. This function can only be used with datetime fields (Edm.DataTimeOffset). Note the `boostingDuration` attribute is used only with the freshness function.
            ...
            <br/>
             Functions can only be applied to filterable fields. See [Create <!-- deleted by customization Index](/documentation/articles/search-api-2015-02-28#createindex) --><!-- keep by customization: begin --> Index](search-api-2015-02-28/#createindex) <!-- keep by customization: end --> for more information about filterable fields.
             <br/>
...
After the index is defined, build the index by uploading the index schema, followed by documents. See [Create <!-- deleted by customization Index](/documentation/articles/search-api-2015-02-28-preview#createindex) --><!-- keep by customization: begin --> Index](search-api-2015-02-28-preview/#createindex) <!-- keep by customization: end --> and [Add or Update <!-- deleted by customization Documents](/documentation/articles/search-api-2015-02-28-preview#AddOrUpdateDocuments) --><!-- keep by customization: begin --> Documents](search-api-2015-02-28-preview/#AddOrUpdateDocuments) <!-- keep by customization: end --> for instructions on these operations. Once the index is built, you should have a functional scoring profile that works with your search data.
...
<tr>
<td>Name</td>	<td>Required. This is the name of the scoring profile. It follows the same naming conventions of a field. It must start with a letter, cannot contain dots, colons or @ symbols, and cannot start with the phrase <!-- deleted by customization 'azureSearch' --><!-- keep by customization: begin --> ‘azureSearch’ <!-- keep by customization: end --> (case-sensitive). </td>
</tr><tr>
...
<td>magnitude | constantBoostBeyondRange</td>	<td>Valid values are true or false (default). When set to true, the full boost will continue to apply to documents that have a value for the target field <!-- deleted by customization that's --><!-- keep by customization: begin --> that’s <!-- keep by customization: end --> higher than the upper end of the range. If false, the boost of this function <!-- deleted by customization won't --><!-- keep by customization: begin --> won’t <!-- keep by customization: end --> be applied to documents having a value for the target field that falls outside of the range.</td>
</tr><tr>
...
<td>distance | referencePointParameter</td>	<td>A parameter to be passed in queries to use as reference location. scoringParameter is a query parameter. See [Search <!-- deleted by customization Documents](/documentation/articles/search-api-2015-02-28-preview#SearchDocs) --><!-- keep by customization: begin --> Documents](search-api-2015-02-28-preview/#SearchDocs) <!-- keep by customization: end --> for descriptions of query parameters.</td>
</tr><tr>
...
<td>tag | tagsParameter</td>	<td>A parameter to be passed in queries to specify tags for a particular request. scoringParameter is a query parameter. See [Search <!-- deleted by customization Documents](/documentation/articles/search-api-2015-02-28-preview#SearchDocs) --><!-- keep by customization: begin --> Documents](search-api-2015-02-28-preview/#SearchDocs) <!-- keep by customization: end --> for descriptions of query parameters.</td>
</tr><tr>
...
`boostingDuration` must be formatted as an XSD "dayTimeDuration" value (a restricted subset of an ISO 8601 duration value). The pattern for this is: <!-- keep by customization: begin --> "[-]P[nD][T[nH][nM][nS]]". <!-- keep by customization: end -->
<!-- deleted by customization
...
**See Also**
[Azure Search Service REST <!-- deleted by customization API](http://msdn.microsoft.com/zh-cn/library/azure/dn798935.aspx) --><!-- keep by customization: begin --> API](http://msdn.microsoft.com/library/azure/dn798935.aspx) <!-- keep by customization: end --> on MSDN <br/>
<!-- deleted by customization
[Create Index (Azure Search API)](http://msdn.microsoft.com/zh-cn/library/azure/dn798941.aspx) on MSDN<br/>
[Add a scoring profile to a search index](http://msdn.microsoft.com/zh-cn/library/azure/dn798928.aspx) on MSDN<br/>
-->
<!-- keep by customization: begin -->
