...
<!-- deleted by customization Azure Search is hosted cloud search service on Windows Azure. --> The following guidance is for customers who built custom applications on the preview versions of Azure Search and are now migrating to the generally available release, 2015-02-28.
...
The suggestion property is the only breaking change. If you <!-- deleted by customization didn't --><!-- keep by customization: begin --> didn’t <!-- keep by customization: end --> use this property you can bump the `api-version` from either `2014-07-31-Preview` or `2014-10-20-Preview` with `2015-02-28`, and then rebuild and redeploy. The application will work as before.
...
In the following section, <!-- deleted by customization we'll --><!-- keep by customization: begin --> we’ll <!-- keep by customization: end --> show a [before](#before) and [after](#after) implementation of suggestions. You can replace the **CreateCatalogIndex()** method with the version in the [after](#after) section, then build and deploy the solution to try the new functionality.
...
After <!-- deleted by customization you've --><!-- keep by customization: begin --> you’ve <!-- keep by customization: end --> ported your solution and verified it runs as expected, you can use these links to read about new features.
...
API version `2015-02-28` is under SLA. Use the support options and links on [this <!-- deleted by customization page](/documentation/articles/contact) --><!-- keep by customization: begin --> page](../support/options/) <!-- keep by customization: end --> to file a support ticket.
