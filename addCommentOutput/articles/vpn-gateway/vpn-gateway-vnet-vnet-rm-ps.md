<properties
   pageTitle="Create a VNet-to-VNet connection using Azure Resource Manager and PowerShell for VNets residing in the same subscription | Windows Azure"
...
If your virtual networks were created using the Classic deployment model, see [Create a VNet-to-VNet connection](/documentation/articles/virtual-networks-configure-vnet-to-vnet-connection). The Classic deployment model supports connecting VNets that reside in different subscriptions.

If you want to connect a virtual network that was created in the Classic deployment model to a virtual network that was created using the Azure Resource Manager model, see [Connecting Classic VNets to new VNets](/documentation/articles/virtual-networks-arm-asm-s2s).
...
First, create a Resource Group. 
			...
			New-AzureResourceGroup -Name testrg1 -Location 'China North'
...
Next, create your virtual network. The sample below creates a virtual network named *VNet1* and two subnets, one called *GatewaySubnet* and the other called *Subnet1*. It's important to create one subnet named specifically *GatewaySubnet*. If you name it something else, your connection configuration will fail. In the example below, our gateway subnet is using a /28. You may choose to use a gateway subnet as small as /29. Note that some features (such as an ExpressRoute/site-to-site coexist connection) require a larger gateway subnet of /27, so you may want to create your gateway subnet to accommodate additional features you may use in the future.
		...
		<!-- deleted by customization New-AzureVirtualNetwork --><!-- keep by customization: begin --> New-AzurevirtualNetwork <!-- keep by customization: end --> -Name VNet1 -ResourceGroupName testrg1 -Location 'China North' -AddressPrefix 10.1.0.0/16 -Subnet $subnet, $subnet1
...
Use the sample below. The Allocation Method for this address must be Dynamic. 
		...
		$gwpip= New-AzurePublicIpAddress -Name gwpip1 -ResourceGroupName testrg1 -Location 'China North' -AllocationMethod Dynamic
...
In this step, you'll create the virtual network gateway for your VNet. VNet-to-VNet configurations require a RouteBased VpnType. Creating a gateway can take a while, so be patient.
		...
		New-AzureVirtualNetworkGateway -Name vnetgw1 -ResourceGroupName testrg1 -Location 'China North' -IpConfigurations $gwipconfig -GatewayType Vpn -VpnType RouteBased
...
**For VNet1 to VNet2**
    ...
    New-AzureVirtualNetworkGatewayConnection -Name conn1 -ResourceGroupName testrg1 -VirtualNetworkGateway1 $vnetgw1 -VirtualNetworkGateway2 $vnetgw2 -Location 'China North' -ConnectionType Vnet2Vnet -SharedKey 'abc123'
...
You can add virtual machines to your virtual networks. [Create a Virtual Machine](/documentation/articles/virtual-machines-windows-tutorial-classic-portal).

For more information about VPN Gateways, see the [VPN Gateway FAQ](/documentation/articles/vpn-gateway-vpn-faq).

For REST API information, see [Azure Network Gateway REST API Reference](https://msdn.microsoft.com/zh-cn/library/azure/mt163859.aspx).

For more information about Virtual Networks, see [Virtual Network Overview](/documentation/articles/virtual-networks-overview).