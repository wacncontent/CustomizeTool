...
Monitoring displays in the <!-- deleted by customization Azure --> Management Portal are highly configurable. You can choose the metrics you want to monitor in the metrics list on the **Monitor** page, and you can choose which metrics to plot in metrics charts on the **Monitor** page and the dashboard.
...
Use the following procedures to configure verbose or minimal monitoring in the <!-- deleted by customization Azure --> Management Portal.
...
1. In the <!-- deleted by customization [Azure Management --><!-- keep by customization: begin --> [Management <!-- keep by customization: end --> Portal](https://manage.windowsazure.cn/), open the **Configure** page for the cloud service deployment.

2. In **Level**, click **Verbose** or **Minimal**. 

3. Click **Save**.

After you turn on verbose monitoring, you should start seeing the monitoring data in the <!-- deleted by customization Azure --> Management Portal within the hour.
...
You can receive alerts based on your cloud service monitoring metrics. On the **Management Services** page of the Azure Management Portal, you can create a rule to trigger an alert when the metric you choose reaches a value that you specify. You can also choose to have email sent when the alert is triggered. For more information, see [How to: Receive Alert Notifications and Manage Alert Rules in <!-- deleted by customization Azure](https://msdn.microsoft.com/zh-cn/library/azure/dn306638.aspx) --><!-- keep by customization: begin --> Azure](/documentation/articles/insights-receive-alert-notification) <!-- keep by customization: end -->.

##How to: Add metrics to the metrics table##

1. In the <!-- deleted by customization [Azure Management --><!-- keep by customization: begin --> [Management <!-- keep by customization: end --> Portal](http://manage.windowsazure.cn/), open the **Monitor** page for the cloud service.

	By default, the metrics table displays a subset of the available metrics. The illustration shows the default verbose metrics for a cloud service, which is limited to the Memory\Available MBytes performance counter, with data aggregated at the role level. Use **Add Metrics** to select additional aggregate and role-level metrics to monitor in the <!-- deleted by customization Azure --> Management Portal.
...
5. Toggle to Verbose mode in the <!-- deleted by customization Azure Management Portal --><!-- keep by customization: begin --> management portal <!-- keep by customization: end --> configuration. If you were in Verbose mode already you will have to toggle to minimal and back to verbose.
...
##How to: Access verbose monitoring data outside the <!-- deleted by customization Azure --> Management Portal##
