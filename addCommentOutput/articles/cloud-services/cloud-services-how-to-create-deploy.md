...
- If you want to deploy a cloud service that uses Secure Sockets Layer (SSL) for data encryption, [configure your <!-- deleted by customization application](/documentation/articles/cloud-services-configure-ssl-certificate#step-2-modify-the-service-definition-and-configuration-files) --><!-- keep by customization: begin --> application](/documentation/articles/cloud-services-configure-ssl-certificate/#step-2-modify-the-service-definition-and-configuration-files) <!-- keep by customization: end --> for SSL.

- If you want to configure Remote Desktop connections to role instances, [configure the roles](/documentation/articles/cloud-services-role-enable-remote-desktop) for Remote Desktop.
...
To create a cloud service with deployments of web roles or worker roles, you must [create the service <!-- deleted by customization package](/documentation/articles/cloud-services-model-and-package#servicepackagecspkg) --><!-- keep by customization: begin --> package](/documentation/articles/cloud-services-model-and-package/#servicepackagecspkg) <!-- keep by customization: end -->.
...
- If any role instances require a certificate, create the certificates. Cloud services require a .pfx file with a private key. You can [upload the certificates to <!-- deleted by customization Azure](/documentation/articles/cloud-services-configure-ssl-certificate#step-3-upload-a-certificate) --><!-- keep by customization: begin --> Azure](/documentation/articles/cloud-services-configure-ssl-certificate/#step-3-upload-a-certificate) <!-- keep by customization: end --> as you create and deploy the cloud service.

- If you plan to deploy the cloud service to an affinity group, create the affinity group. You can use an affinity group to deploy your cloud service and other Azure services to the same location in a region. You can create the affinity group in the **Networks** area of the Azure Management Portal, on the **Affinity Groups** page.


## How to: Create a cloud service using Quick Create

1. In the [Azure Management Portal](http://manage.windowsazure.cn/), click **New**>**Compute**>**Cloud Service**>**Quick Create**.
...
1. In the [Azure Management Portal](http://manage.windowsazure.cn/), click **Cloud Services**, click the name of the cloud service, and then click **Certificates**.
...
1. In the [Azure Management Portal](http://manage.windowsazure.cn/), click **Cloud Services**, click the name of the cloud service, and then click **Dashboard**.
