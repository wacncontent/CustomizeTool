...
> [AZURE.NOTE] This topic connects to a VM using usernames and passwords; to use public and private key pairs to communicate with your VM, see [How to Use SSH with Linux on Azure](/documentation/articles/virtual-machines-linux-use-ssh-key). You can modify the **SSH**  You can modify the **SSH** connectivity of VMs created with the `azure vm quick-create` command by using the `azure vm reset-access` command to reset **SSH** access completely, add or remove users, or add public key files to secure access. This article uses username and password with **SSH** for brevity. 
...
 `azure vm  Attaching a new disk is quick. Just type `azure vm disk attach-new <myuniquegroupname> <myuniq <myuniquegroupname> <myuniquevmname> <size-in-GB>`   to create and attach  [blob-url]`  to create and att  a new GB disk for your VM. It should look something like this:
