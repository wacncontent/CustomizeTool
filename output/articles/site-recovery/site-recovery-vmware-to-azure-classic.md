<properties
	pageTitle="Replicate VMware virtual machines and physical servers to Azure with Azure Site Recovery | Windows Azure" 
...
The Azure Site Recovery service contributes to your business continuity and disaster recovery (BCDR) strategy by orchestrating replication, failover and recovery of virtual machines and physical servers. Machines can be replicated to Azure, or to a secondary on-premises data center. For a quick overview read [What is Azure Site Recovery?](/documentation/articles/site-recovery-overview).
...
>[AZURE.NOTE] This article describes how to replicate to Azure. If you want to replicate VMware VMs or Windows/Linux physical servers to a secondary datacenter, follow the instructions in [this article](/documentation/articles/site-recovery-vmware-to-vmware).

Post any comments or questions at the bottom of this article, or on the [Azure Recovery Services Forum](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr).

## Enhanced deployment 

This article includes contains instructions for an enhanced deployment in the classic Azure Management Portal. We recommend you use this version for all new deployments. If you've already deployed using the earlier legacy version we recommend that you migrate to the new version. Read [more](/documentation/articles/site-recovery-vmware-to-azure-classic-legacy) about migration.
...
Make sure you calculate the bandwidth that would be required for initial replication and delta replication using the [capacity planner tool](/documentation/articles/site-recovery-capacity-planner)
...
1. Open the Windows Azure Backup MMC snap-in on the main management server or on a management server running additional provisioned process servers. By default a shortcut for Windows Azure Backup is created on desktop, or you can find it in: C:\Program Files\Windows Azure Recovery Services Agent\bin\wabadmin.
...
[Learn more](/documentation/articles/site-recovery-capacity-planner) about detailed capacity planning.
...
--- | ---
**Azure account**| You'll need a [Windows Azure](http://azure.microsoft.com/) account. You can start with a [trial](/pricing/1rmb-trial/). [Learn more](/home/features/site-recovery/#price) about Site Recovery pricing. 
**Azure storage** | You'll need an Azure storage account to store replicated data. Replicated data is stored in Azure storage and Azure VMs are spun up when failover occurs. <br/><br/>You need a [standard geo-redundant storage account](/documentation/articles/storage-redundancy#geo-redundant-storage) or a [premium storage account](/documentation/articles/storage-premium-storage-preview-portal), depending on your requirements. The account must in the same region as the Site Recovery service, and be associated with the same subscription. Note that replication to premium storage accounts isn't currently supported and shouldn't be used.<br/><br/>[Read about](/documentation/articles/storage-introduction) Azure storage.
...
**Prerequisite** | **Details**
--- | ---
**Management server** | You need an on-premises Windows 2012 R2 server running on a virtual machine or physical server. All of the on-premises Site Recovery components are installed on this management server<br/><br/> We recommend you deploy the server as a highly available VMware VM. Failback to the on-premises site from Azure is always be to VMware VMs regardless of whether you failed over VMs or physical servers. If you don't configure the Management server as a VMware VM you'll need to set up a separate master target server as a VMware VM to receive failback traffic.<br/><br/>The server should have a static IP address.<br/><br/>The host name of the server should be 15 characters or less.<br/><br/>The operating system locale should be English only.<br/><br/>The management server requires internet access.<br/><br/>You need outbound access from the server as follows: Temporary access on HTTP 80 during setup of the Site Recovery components (to download MySQL); Ongoing outbound access on HTTPS 443 for replication management; Ongoing outbound access on HTTPS 9443 for replication traffic (this port can be modified) 
...
1. In the Azure Management Portal > **Create virtual network** specify the network name. IP address range and subnet name.
...
[Read more](/documentation/articles/virtual-networks-overview) about Azure networks.
...
6. Make sure these URLs are accessible from the management server:
	...
	If you have IP address-based firewall rules on the server, check that the rules allow communication to Azure. You'll need to allow the [Azure Datacenter IP Ranges](https://msdn.microsoft.com/zh-cn/library/azure/dn175718.aspx) and the HTTPS (433) protocol. You'll also need to white list IP address ranges for the Azure region of your subscription, and for China North. In addition, white list this URL for the MySQL download: http://cdn.mysql.com/archives/mysql-5.5/mysql-5.5.37-win32.msi. 
...
The installers are available in C:\Program Files (x86)\Windows Azure Site Recovery\home\svsystems\pushinstallsvc\repository.
...
3. You can modify these settings:
	...
	-  **Azure VM size**: The number of network adapters is dictated by the size you specify for the target virtual machine. [Read more](/documentation/articles/virtual-machines-size-specs#size-tables) about sizes and adapters. Note that:
...
You can customize the plan to create groups and sequence the order in which machines in the recovery plan are failed over. You can also add scripts and prompts for manual actions. Scripts can be created manually or by using by [Azure Automation Runbooks](/documentation/articles/site-recovery-runbook-automation).[Learn more](/documentation/articles/site-recovery-create-recovery-plans) about customizing recovery plans.
...
4. After the failover completes you should also be able to see the replica Azure machine appear in  Azure Management Portal > **Virtual Machines**. If you want to initiate an RDP connection to the Azure VM you'll need to open port 3389 on the VM endpoint. 
...
[Learn more about failback](/documentation/articles/site-recovery-failback-azure-to-vmware-classic) to bring your failed over machines running in Azure back to your on-premises environment.
