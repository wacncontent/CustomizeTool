<properties
	pageTitle="Replicate VMware virtual machines and physical servers to Azure with Azure Site Recovery (legacy) | Windows Azure" 
...
>[AZURE.NOTE] The scenario described in this article includes contains **legacy instructions**. Don't follow this article for new deployments. Instead, use the [enhanced deployment](/documentation/articles/site-recovery-vmware-to-azure-classic) instructions for the Management Portal).  If you've already deployed using the method described in this article, we recommend that you migrate to the new version as described below.
...
1. Read about the [enhanced features](/documentation/articles/site-recovery-vmware-to-azure-classic#enhanced-deployment), make sure you understand the new [architecture](/documentation/articles/site-recovery-vmware-to-azure-classic#scenario-architecture), and check [prerequisites](/documentation/articles/site-recovery-vmware-to-azure-classic#before-you-start-deployment) for the enhanced deployment.
2. Get a [vault registration key](/documentation/articles/site-recovery-vmware-to-azure-classic#step-3:-download-a-vault-registration-key) and [run the unified setup wizard](/documentation/articles/site-recovery-vmware-to-azure-classic#step-4:-install-the-management-server) to install the configuration server, process server, and master target server components on the management server. Read more about [capacity planning](/documentation/articles/site-recovery-vmware-to-azure-classic#capacity-planning).
3. If you have a VMware vCenter server [set up credentials](/documentation/articles/site-recovery-vmware-to-azure-classic#step-5:-set-up-credentials-for-the-vcenter-server) to access it so that Site Recovery can automatically discover VMs that it manages. Read more about the [permissions required](/documentation/articles/site-recovery-vmware-to-azure-classic#vmware-permissions-for-vcenter-access).
4. Add [vCenter servers or ESXi hosts](/documentation/articles/site-recovery-vmware-to-azure-classic#step-6:-add-vcenter-servers-and-esxi-hosts). It can take up to 15 minutes for the portal to refresh so that the credentials appear.
5. Create a [new protection group](/documentation/articles/site-recovery-vmware-to-azure-classic#step-7:-create-a-protection-group). It can take up to 15 minutes for the portal to refresh so that the virtual machines are discovered and appear. If you don't want to wait you can highlight the management server name (don't click it) > **Refresh**.
...
Download the latest update for the **failback tool(vContinuum)** from [here](http://download.microsoft.com/download/2/6/3/263EE895-F104-42E6-8004-7236B6226A6B/Microsoft-ASR_vContinuum_MT_8.4.0.0_Windows_GA_28Jul2015_release.exe)
...
[Learn more](/documentation/articles/site-recovery-failback-azure-to-vmware-classic-legacy) about how to bring your failed over machines running in Azure back to your on-premises environment.
