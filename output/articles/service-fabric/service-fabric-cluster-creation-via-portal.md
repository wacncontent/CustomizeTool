<properties
   pageTitle="Setting up a Service Fabric Cluster from the Azure Management Portal | Windows Azure"
   description="Setting up a Service Fabric Cluster from the Azure Management Portal."
...
# Setting up a Service Fabric Cluster from the Azure Management Portal
...
7. Select a **location** from the dropdown (if you want to create it another location, else it will default to China North)
...
At this time, Service Fabric only supports securing clusters via an X509 certificate. Before starting this process, you will need to upload your certificate to KeyVault. Refer to [Service Fabric Cluster security](/documentation/articles/service-fabric-cluster-security) for more details on how to.
...
Details on security considerations and how to are documented at [Service Fabric Cluster security](/documentation/articles/service-fabric-cluster-security)
...
3. You can now see the details of your cluster in the dashboard, including the cluster's public IP Address. Note that hovering over **Cluster Public IP Address** will bring up a clipboard, which you can click to copy it.
  ...
  The Node Monitor Part on the Cluster Dashboard Blade, indicates the number of VMs that are healthy vs not healthy. you can find more details on the health state in the section [Service Fabric Health model introduction](/documentation/articles/service-fabric-health-introduction) of our documentation
...
```powershell
Connect-serviceFabricCluster -ConnectionEndpoint sfcluster4doc.chinanorth.chinacloudapp.cn:19000 `
            -KeepAliveIntervalInSec 10 `
...
http://sfcluster4doc.chinanorth.chinacloudapp.cn:31000
