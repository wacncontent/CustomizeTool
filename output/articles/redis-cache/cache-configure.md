...
>[AZURE.IMPORTANT] The **maxmemory-reserved** setting is only available for Standard <!-- deleted by customization and Premium --> caches.
...
>[AZURE.IMPORTANT] Keyspace notifications and the **notify-keyspace-events** setting are only available for Standard <!-- deleted by customization and Premium --> caches.
...
The **Users** section provides support for role-based access control (RBAC) in the <!-- deleted by customization preview --> portal to help organizations meet their access management requirements simply and precisely. For more information, see [Role-based access control in the <!-- keep by customization: begin --> Windows <!-- keep by customization: end -->  Azure preview <!-- deleted by customization portal](/documentation/articles/role-based-access-control-configure/) --><!-- keep by customization: begin --> portal](/documentation/articles/role-based-access-control-configure) <!-- keep by customization: end -->.
...
>  
>Any values that are configurable, such as **max-memory-policy**, are configurable through the <!-- deleted by customization preview --> portal.
...
>[AZURE.IMPORTANT] Because configuration and management of Azure Redis Cache instances is done using the <!-- deleted by customization preview portal --><!-- keep by customization: begin --> Azure Management Portal <!-- keep by customization: end --> the following commands are disabled. If you try to invoke them you will receive an error message similar to `"(error) ERR unknown command"`.
>
>-	BGREWRITEAOF
>-	BGSAVE
>-	CONFIG
