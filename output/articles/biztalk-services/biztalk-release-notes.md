<properties 
	pageTitle="Release Notes for Azure BizTalk Services | Windows Azure BizTalk Services" 
	description="" 
...
The release notes for the Windows Azure BizTalk Services contain the known issues in this release.

## What's new in the November update of BizTalk Services
* Encryption at Rest can be enabled in the BizTalk Services Portal. See [Enable Encryption at Rest in BizTalk Services Portal](https://msdn.microsoft.com/zh-cn/library/azure/dn874052.aspx).
 ...
 * **Scenario**: You sign into the Azure Management Portal, create a BizTalk Service, and then select **Manage** for the very first time. When the BizTalk Services portal opens, the BizTalk Service automatically registers and is ready for your deployments.  
 See [Registering and Updating a BizTalk Service Deployment on the BizTalk Services Portal](https://msdn.microsoft.com/zh-cn/library/azure/hh689837.aspx).  

### August 14 Update
* Agreement and bridge decoupling - Trading partner agreements and bridges are now decoupled in the BizTalk Services Portal. You now create agreements and bridges separately, and at runtime bridges resolve to an agreement based on the values in the EDI message. See [Create Agreements in Azure BizTalk Services](https://msdn.microsoft.com/zh-cn/library/azure/hh689908.aspx), [Create an EDI bridge using BizTalk Services Portal](https://msdn.microsoft.com/zh-cn/library/azure/dn793986.aspx), [Create an AS2 bridge using BizTalk Services Portal](https://msdn.microsoft.com/zh-cn/library/azure/dn793993.aspx), and [How do bridges resolve agreements at runtime?](https://msdn.microsoft.com/zh-cn/library/azure/dn794001.aspx)  
* The option to create templates for agreements is discontinued.  
* For the send-side agreement, you can now specify different delimiter sets for each schema. This configuration is specified under protocol settings for send side agreement. For more information, see [Create an X12 Agreement in Azure BizTalk Services](https://msdn.microsoft.com/zh-cn/library/azure/hh689847.aspx) and [Create an EDIFACT Agreement in Azure BizTalk Services](https://msdn.microsoft.com/zh-cn/library/azure/dn606267.aspx). Two new entities are also added to the TPM OM API for the same purpose. See [X12DelimiterOverrides](https://msdn.microsoft.com/zh-cn/library/azure/dn798749.aspx) and [EDIFACTDelimiterOverride](https://msdn.microsoft.com/zh-cn/library/azure/dn798748.aspx).  
* Standard XSD constructs, including Derived Types, are now supported. See [Use standard XSD constructs in your maps](https://msdn.microsoft.com/zh-cn/library/azure/dn793987.aspx) and [Use Derived Types in Mapping Scenarios and Examples](https://msdn.microsoft.com/zh-cn/library/azure/dn793997.aspx).  
* AS2 supports new MIC algorithms for message signing and new encryption algorithms. [See Create an AS2 Agreement in Azure BizTalk Services](https://msdn.microsoft.com/zh-cn/library/azure/hh689890.aspx).  
## Know Issues
...
The X12 Receive and Send settings ([Create an X12 Agreement in Azure BizTalk Services](https://msdn.microsoft.com/zh-cn/library/azure/hh689847.aspx)) provide information on the Protocol stage.  
...
The BizTalk Services bridges continue to process messages for a while, even if the Windows Azure SQL Database (which stores the running information like deployed artifacts and pipelines), is offline. This is because BizTalk Services uses the cached artifacts and bridge configuration.
...
This capability is not available as part of this edition of Windows Azure BizTalk Services. To use these capabilities switch to an appropriate edition.  
...
[BizTalk Services](https://msdn.microsoft.com/zh-cn/library/azure/hh689864.aspx)   
