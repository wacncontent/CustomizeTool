...
In the [Azure Websites](https://tryappservice.azure.com/) option, the "ASP.NET + Azure Search Site" template provides source code for the web site, modifiable in Visual Studio Team Services (available as a part of the one-hour session). No separate development tools are required to view or change the code.
...
Filters are specified using OData syntax and are frequently used with faceted navigation or included in the query string to constrain the query. See [OData Filter Syntax](https://msdn.microsoft.com/zh-cn/library/azure/dn798921.aspx) for more information.
