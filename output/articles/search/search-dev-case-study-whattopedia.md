...
## How [WhatToPedia.com](http://whattopedia.com/) built an infomedia portal on Windows Azure
...
We built a business, not just a site. Supporting the entire effort required a full range of tools and applications. We adopted Visual Studio and Visual Studio Team Services for development, Team Foundation Service (TFS) Online for source control and scrum management, Office 365 for communication and collaboration, and of course Windows Azure for all site-related operations and storage. With Visual Studio, the IDE provided direct provisioning to Azure, with integration to TFS Online providing an additional productivity boost.
...
1. Login to Azure Management Portal and added the Search service to our subscription. We used the shared version (free with our subscription).
...
2. Call the Azure Search Service API to upload resellers one by one (see: http://msdn.microsoft.com/zh-cn/library/azure/dn798930.aspx).
...
This is the code we used to get the results from Azure Search:
    ...
    string requestUrl = 
    string.Format("https://{0}.search.chinacloudapi.cn/indexes/profiles/docs?searchMode=all&$count=true&search={1}&facet=city,count:20&facet=category&$top=10&$skip={2}&api-version=2014-07-31-Preview{3}", Config.SearchServiceName, EscapeODataString(q), skip, filter);
...
Subscriber data in Azure SQL Database is the most sensitive asset. We back this up using the built-in feature (see [Azure SQL Database Backup and Restore](http://msdn.microsoft.com/zh-cn/library/azure/jj650016.aspx)). The backup schedule is full database backup once a week, differential database backups once a day, and transaction log backups every 5 minutes.  Given the size of the data, this solution is more than adequate for our immediate and projected data volumes.
...
We solved this by constraining the facet count parameter. The count parameter imposes a hard limit on the number of facets returned to the user. A link that includes a discussion of the count parameter can be found [here](/documentation/articles/search-faceted-navigation).
...
The following code builds the indexer mentioned in the section on building the prototype.
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var profile in profiles)
            {
                ...
                try
                {
                    ...
                    var response = client.PostAsync("https://" + serviceName + ".search.chinacloudapi.cn/indexes/profiles/docs/index?api-version=2014-10-20-Preview", new StringContent(JsonConvert.SerializeObject(batch), Encoding.UTF8, "application/json")).Result;
                    response.EnsureSuccessStatusCode();
        ...
        static void RecreateIndex( string ServiceName, HttpClient client)
        {
            ...
            var url = "https://" + ServiceName + ".search.chinacloudapi.cn/indexes/?api-version=2014-10-20-Preview";

            var deleteUrl = "https://" + ServiceName + ".search.chinacloudapi.cn/indexes/profiles?api-version=2014-10-20-Preview";
