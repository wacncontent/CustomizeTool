...
This article was written for the classic deployment model using the [Classic Management Portal](https://manage.windowsazure.cn) (not the Azure Portal). When we have an article available for the resource manager deployment model, we will link to it from here. 
