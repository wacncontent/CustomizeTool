...
This article applies to Point-to-Site VPN Gateway connections to a virtual network created using the **classic deployment model** (Service Management) and the Classic Management Portal. When we have steps for the Azure portal, we will link to that article from this page.
...
1. Log in to the [Azure Classic Management Portal](https://manage.windowsazure.cn/). Note that these steps use the Classic Management Portal, not the Azure portal.
