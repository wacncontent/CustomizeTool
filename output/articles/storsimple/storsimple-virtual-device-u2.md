<properties 
   pageTitle="StorSimple virtual device Update 2| Windows Azure"
   description="Learn how to create, deploy, and manage a StorSimple virtual device in a Windows Azure virtual network. (Applies to StorSimple Update 2)."
   services="storsimple"
   documentationCenter=""
   authors="alkohli"
   manager="carmonm"
...
> [AZURE.SELECTOR]
- [Update 2](/documentation/articles/storsimple-virtual-device-u2)
- [Update 1](/documentation/articles/storsimple-virtual-device-u1)
- [GA Release](/documentation/articles/storsimple-virtual-device)

##Overview
The StorSimple virtual device is an additional capability that comes with your Windows Azure StorSimple solution. The StorSimple virtual device runs on a virtual machine in a Windows Azure virtual network, and you can use it to back up and clone data from your hosts. 
...
| **Storage type**          | Uses Azure Standard Storage<br></br> Learn how to [create a Standard Storage account]() | Uses Azure Premium Storage<br></br>Learn how to [create a Premium Storage account](/documentation/articles/storage-premium-storage-preview-portal#create-and-use-a-premium-storage-account-for-a-virtual-machine-data-disk)                                                               |
...
The StorSimple virtual device is a software-only version of StorSimple that runs on a single node in a Windows Azure Virtual Machine. The virtual device supports disaster recovery scenarios in which your physical device is not available, and is appropriate for use in item-level retrieval from backups, on-premises disaster recovery, and cloud dev and test scenarios.
...
The following sections explain the configuration prerequisites for your StorSimple virtual device. Prior to deploying a virtual device, review the [security considerations for using a virtual device](/documentation/articles/storsimple-security#storsimple-virtual-device-security).

#### Azure requirements

Before you provision the virtual device, you need to make the following preparations in your Azure environment:

- For the virtual device, [configure a virtual network on Azure](/documentation/articles/virtual-networks-create-vnet). If using Premium Storage, you must create a virtual network in an Azure region that supports Premium Storage. More information on [regions that currently support Premium Storage](https://azure.microsoft.com/regions/#services).
...
- Add [access control records](/documentation/articles/storsimple-manage-acrs) for the VMs that are going to be host servers for your virtual device.

- Use a [storage account](/documentation/articles/storsimple-manage-storage-accounts#add-a-storage-account) in the same region as the virtual device. Storage accounts in different regions may result in poor performance. You can use a Standard or Premium Storage account with the virtual device. More information on how to create a [Standard Storage account]() or a [Premium Storage account](/documentation/articles/storage-premium-storage-preview-portal#create-and-use-a-premium-storage-account-for-a-virtual-machine-data-disk)
...
- Your Azure Management Portal account with access credentials.
...
You should follow the procedures in [Connecting remotely to your StorSimple device](/documentation/articles/storsimple-remote-connect) to set up remoting for your virtual device.

## Connect directly to the virtual device

You can also connect directly to the virtual device. If you want to connect directly to the virtual device from another computer outside the virtual network or outside the Windows Azure environment, you need to create additional endpoints as described in the following procedure. 
...
Now that you have created and configured the StorSimple virtual device, you are ready to start working with it. You can work with volume containers, volumes, and backup policies on a virtual device just as you would on a physical StorSimple device; the only difference is that you need to make sure that you select the virtual device from your device list. Refer to [use the StorSimple Manager service to manage a virtual device](/documentation/articles/storsimple-manager-service-administration) for step-by-step procedures of the various management tasks for the virtual device.
...
- **Software updates** â You can view the date that the software was last updated, together with any update status messages. You can use the **Scan updates** button at the bottom of the page to perform a manual scan if you want to check for new updates. If updates are available, click **Install Updates** to install. Because there is only a single interface on the virtual device, this means that there will be a slight service interruption when updates are applied. The virtual device will shut down and restart (if necessary) to apply any updates that have been released. For a step-by-step procedure, go to [update your device](/documentation/articles/storsimple-update-device#install-regular-updates-via-the-management-portal).
- **Support package** â You can create and upload a support package to help Microsoft Support troubleshoot issues with your virtual device. For a step-by-step procedure, go to [create and manage a support package](/documentation/articles/storsimple-create-manage-support-package).
...
For a step-by-step procedure, go to [add a storage account](/documentation/articles/storsimple-manage-storage-accounts#add-a-storage-account).
...
For a step-by-step procedure, go to [deactivate your device](/documentation/articles/storsimple-deactivate-and-delete-device#deactivate-a-device).
...
- Learn how to [Use the StorSimple Manager service to manage a virtual device](/documentation/articles/storsimple-manager-service-administration).
 
- Understand how to [Restore a StorSimple volume from a backup set](/documentation/articles/storsimple-restore-from-backup-set). 
