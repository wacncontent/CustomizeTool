...
> [AZURE.NOTE] Once started, a Storm topology runs until stopped (killed.) To stop the topology, use either the `storm kill TOPOLOGYNAME` command from the command-line (SSH session to a Linux clu (SSH session to a Linux cluster for example,)  o or by using the Storm UI, select the topology, and then select the __Kill__ button.
...
> [AZURE.NOTE] Once started, a Storm topology runs until stopped (killed.) To stop the topology, use either the `storm kill TOPOLOGYNAME` command from the command-line (SSH session to a Linux clu (SSH session to a Linux cluster,)  o or by using the Storm UI, select the topology, and then select the __Kill__ button.

