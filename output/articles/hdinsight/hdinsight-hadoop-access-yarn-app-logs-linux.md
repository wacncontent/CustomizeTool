...
> [AZURE.NOTE] The information in this document is specific to Linux-based HDInsight clusters. For information on Windows-based clusters, see [Access YARN application logs on Windows-based HDInsight](hdinsight-hadoop-access-yarn-app-logs)

## Prerequisites

* A Linux-based HDInsight cluster.

* You must [create an SSH tunnel](hdinsight-linux-ambari-ssh-tunnel) before you can access the ResourceManager Logs web UI.
...
The YARN ResourceManager UI runs on the cluster headnode, and can be accessed through the Ambari web UI; however, you must first [create an SSH tunnel](hdinsight-linux-ambari-ssh-tunnel) before you can access the ResourceManager UI.
