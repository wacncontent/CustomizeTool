...
The edge node provided with R Server on HDInsight clusters can also be directly accessed using SSH through port 22. For example, `ssh username@RServer.mycluster.ssh.azurehdinsight.cn` will connect to the edge node for an R Server on HDInsight cluster named __mycluster__. 
