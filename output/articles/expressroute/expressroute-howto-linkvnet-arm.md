...
- You must have an active ExpressRoute circuit. 
	- Follow the instructions to [create an ExpressRoute circuit](/documentation/articles/expressroute-howto-circuit-arm) and have the circuit enabled by your connectivity provider. 
	- Ensure that you have Azure private peering configured for your circuit. See the [configure routing](/documentation/articles/expressroute-howto-routing-arm) article for routing instructions. 
	...
	- You must have a virtual network and a virtual network gateway created and fully provisioned. Follow instructions to create a [vpn gateway](/documentation/articles/vpn-gateway-create-site-to-site-rm-powershell)
...
Yon can connect a virtual network gateway to an ExpressRoute circuit using the following cmdlet. Make sure that the virtual network gateway is created and is ready for linking before you run the cmdlet.
	...
	$connection = New-AzureRmVirtualNetworkGatewayConnection -Name "ERConnection" -ResourceGroupName "MyRG" -Location "China East" -VirtualNetworkGateway1 $gw -PeerId $circuit.Id -ConnectionType ExpressRoute
...
The circuit user can run the following cmdlet to redeem a link authorization.
	...
	$connection = New-AzureRmVirtualNetworkGatewayConnection -Name "ERConnection" -ResourceGroupName "RemoteResourceGroup -Location "China East" -VirtualNetworkGateway1 $gw -PeerId $id -ConnectionType ExpressRoute -AuthorizationKey "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
