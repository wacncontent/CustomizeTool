...
Tenant-independent endpoints provide information that is common to all Azure AD tenants. This information applies to tenants hosted at *login.chinacloudapi.cn* and is shared across tenants. Tenant-independent endpoints are recommended for multi-tenant applications, since they are not associated with any particular tenant.

## Federation Metadata Endpoints

Azure AD publishes federation metadata at *https://login.chinacloudapi.cn/<TenantDomainName>/FederationMetadata/2007-06/FederationMetadata.xml*,where the value of <TenantDomainName> can be "common" or a tenant-specific value.
...
For **tenant-independent endpoints**, the <TenantDomainName> is **common**. This name indicates that only the Federation Metadata elements that are common to all Azure AD tenants that are hosted at login.chinacloudapi.cn.

For example, a tenant-specific endpoint might be *https://login.chinacloudapi.cn/contoso.partner.onmschina.cnFederationMetadata/2007-06/FederationMetadata.xml*. The tenant-independent endpoint is *https://login.chinacloudapi.cn/common/FederationMetadata/2007-06/FederationMetadata.xml*.
