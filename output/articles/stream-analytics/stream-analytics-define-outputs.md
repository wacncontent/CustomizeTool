<properties
	pageTitle="Data transformation outputs: Options for storage, analysis | Windows Azure"
...
[Azure Table storage](/documentation/articles/storage-introduction)  offers highly available, massively scalable storage, so that an application can automatically scale to meet user demand. Table storage is Microsoft's NoSQL key/attribute store which one can leverage for structured data with less constraints on the schema. Azure Table storage can be used to store data for persistence and efficient retrieval.
