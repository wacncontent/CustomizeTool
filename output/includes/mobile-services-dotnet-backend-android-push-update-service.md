...
2. Update the `PostTodoItem` method definition with the following code:  
    ...
    This code will send a push notification (with the text of the inserted item) after inserting a todo item. In the event of an error, the code will add an error log entry which is viewable on the **Logs** tab of the mobile service in the [Azure Management Portal](https://manage.windowsazure.cn/).
