...
This step is performed in the Azure Management Portal. The service administrator can select a device from a displayed list of the devices that are eligible to be authorized. The device is then authorized to start the service data encryption key change process.
...
> [AZURE.NOTE]
> In the Azure Management Portal, StorSimple virtual devices are not shown in the list of devices that can be authorized to start the key change.
...
> [AZURE.NOTE] No operations can be performed in the Azure Management Portal of your StorSimple Manager service until the key rollover is completed.
