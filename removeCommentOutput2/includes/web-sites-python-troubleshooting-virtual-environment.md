...
With an FTP client, connect to the server and you'll be able to delete the env folder.  Note that some FTP clients (such as web browsers) may be read-only and won't allow you to delete folders, so you'll want to make sure to use an FTP client with that capability.  The FTP host name and user are displayed in your web app's dashboard on the [Azure Classic Management Portal](https://manage.windowsazure.cn).
...
1. Switch to a different version of Python (via runtime.txt or the **Configuration** Page in the Azure Classic Management Portal)
