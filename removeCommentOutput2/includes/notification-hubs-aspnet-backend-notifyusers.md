...
17. Add the following code inside the `RegisterController` clthe ss definition. Note that in this code, we add a user tag for the user that has been authenticated by the handler. You can also add optional checks to verify that the user has rights to register for the requested tags.
