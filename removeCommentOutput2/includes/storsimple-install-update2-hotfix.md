...
1. To install the hotfixes, access the Windows PowerShell interface on your StorSimple device serial console. Follow the detailed instructions in [Use PuTTy to connect to the serial console](/documentation/articles/storsimple-deployment-walkthrough#use-putty-to-connect-to-the-device-serial-console). At the command prompt, press **Enter**.
...
5. To install the hotfix, at the command prompt, type:
	...
	We recommend that you use the credential parameter to access shares. Even shares that are open to âeveryoneâ are typically not open to unauthenticated users.
...
1. Place the device in the Maintenance mode. Note that you should not use Windows PowerShell remoting when connecting to a device in Maintenance mode. You will need to run this cmdlet on the device controller when connected through the device serial console. Type:
	...
	A sample output is shown below.
		...
		In maintenance mode, your device will not service IOs and will be disconnected from the Windows Azure StorSimple Manager service. Entering maintenance mode will end the current session and reboot both controllers, which takes a few minutes to complete. Are you sure you want to enter maintenance mode?
		[Y] Yes [N] No (Default is "Y"): Y

		-----------------------MAINTENANCE MODE------------------------
		Windows Azure StorSimple Appliance Model 8100
...
1. The controllers will be rebooted when you exit the Maintenance mode. After the disk firmware updates are successfully applied and the device has exited maintenance mode, return to the Azure Management Portal. Maintenance mode updates are not updated on the portal until 24 hours have elapsed. 
