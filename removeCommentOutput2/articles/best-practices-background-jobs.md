...
- Sensitive data processing where the task is handed off to a more secure location for processing. For example, you may not want to process sensitive data within a web app, and instead use a pattern such as [Gatekeeper](http://msdn.microsoft.com/zh-cn/library/dn589793.aspx) to transfer the data to an isolated background process that has access to protected storage.
...
- By default, WebJobs scale with the web app. However, jobs can be configured to run on single instance by setting the **is_singleton** configuration property to true. Single instance WebJobs are useful for tasks that you do not want to scale or run as simultaneous multiple instances, such as re-indexing, data analysis, and similar tasks.
...
To help you choose when to use Virtual Machines, see [Azure Web App, Cloud Services and Virtual Machines comparison](/documentation/articles/choose-web-site-cloud-service-vm). For information about the options for [Virtual Machines, see Virtual Machine and Cloud Service Sizes for Azure](http://msdn.microsoft.com/zh-cn/library/azure/dn197896.aspx). For more information about the operating systems and pre-built images available for Virtual Machines, see [Azure Virtual Machines Gallery](http://azure.microsoft.com/gallery/virtual-machines/).
...
- By default, WebJobs scale with their associated Azure Web Apps instance. However, if you want a WebJob to run as only a single instance, you can create a Settings.job file containing the JSON data **{ "is_singleton": true }**. This forces Azure to only run one instance of the WebJob, even if there are multiple instances of the associated web app, which can be a useful technique for scheduled jobs that must run as only a single instance.
