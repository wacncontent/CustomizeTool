...
1. Click the **New** icon on the top left of the Portal, then click **Data + Storage** > **Storage Account**.  Click the **Create** button, then give the storage account a unique name and create a new [resource group](../azure-resource-manager/documentation/articles/resource-group-overview) for it. You should choose Classic Deployment model, otherwise you will not be able to manage the storage account in Visual Studio. 
