...
> [AZURE.NOTE]
> Once started, a Storm topology runs until stopped (killed.) To stop the topology, use either the `storm kill TOPOLOGYNAME` command from the command-line or by using the Storm UI, select the topology, and then select the **Kill** button.
> 
> 
