...
Installations are currently only supported by the [Notification Hub SDK for backend operations](https://www.nuget.org/packages/Microsoft.Azure.NotificationHubs/). See the [Installation Class](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.azure.notificationhubs.installation.aspx) for more information. To register from a client device using an installation ID without a backend, you would need to use [Notification Hubs REST API](https://msdn.microsoft.com/zh-cn/library/mt621153.aspx) at this time.

An installation can contain the the following properties. For a complete listing of the installation properties see, [Create or Overwrite an Installation with REST](https://msdn.microsoft.com/zh-cn/library/azure/mt621153.aspx) or [Installation Properties](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.azure.notificationhubs.installation_properties.aspx).
...
Note that each template name maps to a template body and an optional set of tags. Moreover, each platform can have additional template properties. For Windows Store (using WNS) and Windows Phone 8 (using MPNS), an additional set of headers can be part of the template. In the case of APNs, you can set an expiry property to either a constant or to a template expression. For a complete listing of the installation properties see, [Create or Overwrite an Installation with REST](https://msdn.microsoft.com/zh-cn/library/azure/mt621153.aspx) topic.
...
Note that in this case, you will use only Listen rights to access your notification hubs from the device. For more information, see [Security](/documentation/articles/notification-hubs-security).
...
At this time, this is only supported using the [Notification Hubs REST API](https://msdn.microsoft.com/zh-cn/library/mt621153.aspx).
