...
1. Install <a href="/downloads/" target="_blank">Azure PowerShell cmdlets</a> version 0.8.0 or later.
2. If you want the scripts to run on an existing VM, make sure VM Agent is enabled on the VM. If it is not installed, follow these  [steps](/documentation/articles/virtual-machines-windows-classic-agents-and-extensions/). If the VM is created from the Azure portal preview, then VM Agent is installed by default.
...
### Add the Custom Script extension from the Azure portal preview
Go to the VM in the <a href="https://portal.azure.cn/ " target="_blank">Azure portal preview</a> and add the extension by specifying the script file to run.
