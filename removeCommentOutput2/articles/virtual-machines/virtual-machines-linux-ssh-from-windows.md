...
<properties
    pageTitle="Use SSH keys with Windows for Linux VMs | Azure"
...
1. **ssh-rsa** keys are required for any deployment using the [Azure portal preview](https://portal.azure.cn), and Resource Manager deployments using the [Azure CLI](/documentation/articles/xplat-cli-install/).
   * These keys are usually all most people need.
2. `.pem` file is required to create VMs using the [Classic Management Portal](https://manage.windowsazure.cn). These keys are also supported in Classic deployments that use the [Azure CLI](/documentation/articles/xplat-cli-install/).
...
## Next steps
You can also generate the public and private keys [using OS X and Linux](/documentation/articles/virtual-machines-linux-mac-create-ssh-keys/).
...
If you have trouble using SSH to connect to your Linux VMs, see [Troubleshoot SSH connections to an Azure Linux VM](/documentation/articles/virtual-machines-linux-troubleshoot-ssh-connection/).
