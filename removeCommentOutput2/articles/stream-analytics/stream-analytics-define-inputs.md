<properties
	pageTitle="Data connection: Data stream inputs from an event stream | Windows Azure"
...
For scenarios with large amounts of unstructured data to store in the cloud, Blob storage offers a cost-effective and scalable solution. Data in [Blob storage](/home/features/storage/blobs/) is generally considered data âat restâ but it can be processed as a data stream by Stream Analytics. One common scenario for Blob storage inputs with Stream Analytics is log processing, where telemetry is captured from a system and needs to be parsed and processed to extract meaningful data.
