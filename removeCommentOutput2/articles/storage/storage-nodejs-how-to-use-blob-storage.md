...
ForCreate a blank Node.js application. instructions creating a Node.js application, see [Create and deploy a Node.js application to an Azure Web Site], [Node.js Cloud Service][Node.js Cloud Service] (using Windows PowerShell), or [Web Site with WebMatrix].
...
Optional filtering operations tocan be applied operations performed using **BlobService**. Filtering operations can include logging, automatically retrying, etc. Filters are objects that implement a method with the signature:

		function handle (requestOptions, next)

After doing its preprocessing on the request optionsthe method needs to call "next", passing a callback with the following signature:
...
## UploadHow to: a blob into a container
...
## ListHow to: the blobs in a container
...
If the value has been modified, this indicates that another client or instance modifiedhas the blob or container since you obtained the ETag value.
...
## WorkHow to: with shared access signatures
...
You can also use an Access Control List (ACL) to set the access policy for SAS.a This is useful if you wish to allow multiple clients to access a container but, provide different access policies for each client.
