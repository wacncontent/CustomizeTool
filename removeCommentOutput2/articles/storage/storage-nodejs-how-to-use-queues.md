...
To dequeue a message, use **getMessage**. This makes the message invisible in the queue, so no other clients can process it. Once your the pplication has processed a message, call **deleteMessage** to delete it from the queue. The following example gets a message, then deletes it:
