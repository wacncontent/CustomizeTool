...
Azure File storage offers shared storage for applications using the standard SMB protocol.2.1 Windows Azure virtual machines and cloud services can share file data across application components via mounted shares, and on-premises applications can access file data in a share via the File storage API or Azure PowerShell.
