...
3. Once the CommunicationListener s are created, it is opened
  + CommunicationListeners have a method called OpenAsync(), which is called at this point and which returns the listening address for the service. If your Reliable Service uses one of the built in ICommunicationListeners, then this is handled for you.
...
When the service is being shut down (either when it is deleted, upgraded, just being moved from a particular location) the call order is the same, first CloseAsync() is called on the CommunicationListener s, then the cancellation token that was passed to RunAsync() is canceled.
