<properties 
	pageTitle="Power BI tutorial for DocumentDB connector | Windows Azure" 
...
- Access to our demo account or data in your Azure DocumentDB account. 
	...
	- Or, to create your own account, see [Create a DocumentDB database account using the Azure Management Portal](/documentation/articles/documentdb-create-account/). Then, to get sample volcano data that's similar to what's used in this tutorial (but does not contain the GeoJSON blocks), see the [NOAA site](https://www.ngdc.noaa.gov/nndc/struts/form?t=102557&s=5&d=5) and then import the data using the [DocumentDB data migration tool](/documentation/articles/documentdb-import-data/).
...
5. Click on **Azure**, select **Windows Azure DocumentDB (Beta)**, and then click **Connect**.  The **Windows Azure DocumentDB Connect** window should appear.

	![Power BI Desktop Get Data](./media/documentdb-powerbi-visualize/power_bi_connector_pbigetdata.png)

6. Specify the DocumentDB account endpoint URL you would like to retrieve the data from as shown below, and then click **OK**. You can retrieve the URL from the URI box in the **Keys** blade of the Azure Management Portal or you can use the demo account information provided above. For more information, see [Keys](/documentation/articles/documentdb-manage-account#keys).
