...
The configuration of endpoint priorities is carried out differently in the ARM APIs (and Azure portal Preview) vs the ASM APIs (and Classic Management Portal):
...
- In the ASM APIs, the endpoint priority is configured implicitly, based on the order in which the endpoints are listed in the profile definition.  You can also configure the failover order in the Azure Classic Management Portal, on the Configuration page for the profile.
...
Weighted traffic routing can be configured via the Azure portal Preview, however weights cannot be configured via the Classic Management Portal.  It can also be configured via both ARM and ASM using Azure PowerShell, Azure CLI and the Azure REST APIs.
