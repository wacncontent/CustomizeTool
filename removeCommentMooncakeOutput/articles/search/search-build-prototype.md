...
1. [Create an Azure Search service in the Azure Management Portal](/documentation/articles/search-create-service-portal).
...
3. Edit app.config to add configuration settings that target your Search service and api-key. 
	...
	You can get the URL and the admin api-key from [service dashboard in the portal](/documentation/articles/search-create-service-portal). For the URL, type the full path of the service name, including the https prefix and the `search.chinacloudapi.cn` domain.
...
6. Stop the application and delete the index to make room for yours. 
    ...
    > [AZURE.TIP] You can use [the portal](https://manage.windowsazure.cn) to delete the index. Click the index name to open the index blade and use the **Delete** command.
...
At this point, you might want to switch to [Fiddler](/documentation/articles/search-fiddler) or [Chrome Postman](/documentation/articles/search-chrome-postman) to run queries, modify any of the sample applications to use your Search service and index, or add custom code that provides data visualization.

## Add a scoring profile

We suggest that you experiment with [scoring profiles](/documentation/articles/search-get-started-scoring-profiles) either through the portal or through code. Scoring profiles boost the importance of a search term being found in a specific field. For example, if the search term is a city name, you would expect documents having *Seattle* in the City field to appear higher in the results than documents having the *Seattle* in a Description field.
...
See [Add a scoring profile](https://msdn.microsoft.com/zh-cn/library/dn798928.aspx) for the reference documentation on scoring profiles.
...
- [Typical workflows for development projects using Azure Search](/documentation/articles/search-workflow)
