...
[AZURE.INCLUDE [azure-arm-classic-important-include](../includes/azure-arm-classic-important-include.md)] This article covers the Resource Manager deployment model. You can also [Learn how to create an Internet facing load balancer using classic deployment model](/documentation/articles/load-balancer-get-started-internet-classic-cli).
...
Create a public IP address (PIP) named *PublicIP* to be used by a frontend IP pool with DNS name *loadbalancernrp.chinanorth.chinacloudapp.cn*. The command below uses the static allocation type.
	...
	$publicIP = New-AzureRmPublicIpAddress -Name PublicIp -ResourceGroupName NRP-RG -Location "China North" âAllocationMethod Static -DomainNameLabel loadbalancernrp 
