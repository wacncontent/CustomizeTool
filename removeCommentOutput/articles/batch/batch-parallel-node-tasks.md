<properties
	pageTitle="Maximize Batch node use with parallel tasks | Azure"
...
As an example of how this feature is valuable, consider the pool of [Standard\_D14](/documentation/articles/cloud-services-sizes-specs/#memory-intensive-d) nodes (in the example above) that is configured with a [CloudPool.MaxTasksPerComputeNode][maxtasks_net] value of 16. If the [CloudPool.TaskSchedulingPolicy][task_schedule] is configured with a [ComputeNodeFillType][fill_type] of *Pack*, it would maximize usage of all 16 cores of each node and allow an [autoscaling pool](/documentation/articles/batch-automatic-scaling/) to prune unused nodes from the pool (nodes without any tasks assigned). This minimizes resource usage and saves money.
