...
Suppose that you deployed an application in the following Azure regions: China North, West Europe, and China East. You use Traffic Manager's 'Performance' traffic-routing method to distribute traffic to the region closest to the user.
...
However, suppose you prefer the West Europe traffic failover to China North, and only direct traffic to other regions when both endpoints are unavailable. You can create this solution using a child profile with the 'Priority' traffic-routing method.

!['Performance' traffic routing with preferential failover][6]

Since the West Europe endpoint has higher priority than the China North endpoint, all traffic is sent to the West Europe endpoint when both endpoints are online. If West Europe fails, its traffic is directed to China North. With the nested profile, traffic is directed to China East only when both West Europe and China North fail.
