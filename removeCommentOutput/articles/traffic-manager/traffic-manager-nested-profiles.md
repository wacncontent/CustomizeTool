...
However, suppose you prefer the West Europe traffic fail over to China North by preference, and only direct elsewhere if both of those endpoints are unavailable. You can do this by creating a child profile which uses the 'Priority' traffic-routing method, as shown:

!['Performance' traffic routing with preferential failover][6]

Since the West Europe endpoint has higher priority than the China North endpoint, all traffic will be sent to the West Europe endpoint when both are online.  If West Europe fails, its traffic is directed to China North.  Only if China North also fails would West Europe traffic be directed to China East.
