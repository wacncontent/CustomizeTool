...
1. In the Azure classic portal, click the name of the virtual network, and then click the **Configure** tab to [reconfigure the DNS server IP addresses for your virtual network](/documentation/articles/virtual-networks-manage-dns-in-vnet) to use the static IP addresses assigned to the replica DCs instead of the IP addresses of an on-premises DNS servers.
