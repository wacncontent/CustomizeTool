...
+	An [Azure subscription](../includes/free-trial-note.md). If you aren't ready to sign up for a trial subscription, you can skip this tutorial and opt for [Try Azure Websites](https://tryappservice.azure.com/) instead. This alternative option gives you Azure Search with an ASP.NET web site for free - one hour per session - no subscription required.
...
3.	Open **App.config** in this project and update the values for "SearchServiceName" and "SearchServiceApiKey" to those of your Azure Search service. For Search service name, if your service is "mysearch.search.chinacloudapi.cn", you would enter "mysearch".
...
To verify index creation and document upload, go to your Search service dashboard in the [Azure management portal](https://manage.windowsazure.cn). In Usage, the index count should be up by one, and you should have 294 documents, one for each product in the database.
...
3.	Open **Web.config** in this project and update the values for "SearchServiceName" and "SearchServiceApiKey" to those of your Azure Search service. For Search service name, if your service is "mysearch.search.chinacloudapi.cn", you would enter "mysearch".
...
The project AdventureWorksWeb shows us how ASP.NET MVC 4 can be used to interact with Azure Search from a web site. In this section, we'll review individual parts of the application code to see what they do.
...
6.	Open **HomeController.cs** and move to the Suggest function. This code is very similar to the Search function that uses the `_catalogSearch` object to call a function in **CatalogSearch.cs** called `Suggest`. Rather than making a search query, the `Suggest` function makes a call to the [Suggestions API](http://msdn.microsoft.com/zh-cn/library/azure/dn798936.aspx). This uses the terms entered in the text box and build out a list of potential suggestions. The values are returned to the **Index.cshtml** file that are automatically listed in the Search box as type-ahead options.
...
+	Read up on the [Lookup API](http://msdn.microsoft.com/zh-cn/library/azure/dn798929.aspx) that allows you to make a query to Azure Search to bring back a specific document (for example you could pass the productID).
...
You can also review the [Azure Search REST API](http://msdn.microsoft.com/zh-cn/library/azure/dn798935.aspx) on MSDN.
