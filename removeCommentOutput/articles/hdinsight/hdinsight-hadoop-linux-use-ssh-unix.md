...
>
> If you do not have SSH installed on your Windows-based client, use the steps in [Use SSH with Linux-based HDInsight (Hadoop) from Windows](/documentation/articles/hdinsight-hadoop-linux-use-ssh-windows/) for information on installing and using PuTTY.
...
* **SSH address** - There are two addresses that may be used to connect to a cluster using SSH:
    ...
    * **Connect to the headnode**: The cluster name, followed by **-ssh.azurehdinsight.cn**. For example, **mycluster-ssh.azurehdinsight.cn**.
