...
<properties
	pageTitle="App Model v2.0 OAuth Protocol | Windows Azure"
	description="Building web sites using Azure AD's implementation of the OAuth 2.0 authentication protocol."
...
| scope | required | A space-separated list of scopes.  A single scope value indicates to the v2.0 endpoint both the resource and the permissions to that resource being requested.  Scopes take the form `<app identifier URI>/<scope value>`.  In the example above, the app identifier for the Azure AD Graph API is used, `https://graph.chinacloudapi.cn`, and two permissions are requested: `directory.read` and `directory.write`.  For a more detailed explanation of scopes, refer to the [app model v2.0 scope reference](/documentation/articles/active-directory-v2-scopes).  |
...

{
	...
	"scope": "https://graph.chinacloudapi.cn/directory.read https://graph.chinacloudapi.cn/directory.write",
...
| client_secret | required for web sites | The application secret that you created in the app registration portal for your app.  It should not be used in a native  app, because client_secrets cannot be reliably stored on devices.  It is required for web sites and web APIs, which have the ability to store the client_secret securely on the server side. |
...
| id_token | An unsigned JSON Web Token (JWT). The  app can base64Url decode the segments of this token to request information about the user who signed in. The  app can cache the values and display them, but it should not rely on them for any authorization or security boundaries.  For more information about id_tokens see the [v2.0 app model token reference](/documentation/articles/active-directory-v2-tokens). |
...
```
GET /contoso.partner.onmschina.cn/users
Host: https://graph.chinacloudapi.cn
