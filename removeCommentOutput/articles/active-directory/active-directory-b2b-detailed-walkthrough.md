<properties
   pageTitle="Detailed walkthrough of using the Azure Active Directory B2B collaboration preview | Windows Azure"
...
After invitations are sent out to partner users, we can configure them in Azure AD to grant access to apps and membership to groups through the Azure Management Portal. Let's start by adding Alice.
...
2. In the Azure Management Portal, add a user into the Contoso directory (Active Directory > Contoso > Users > Add User). In the "Type of User" drop down, select "Users in partner companies". Upload the .csv file. Make sure that the .csv file is closed prior to uploading.  
...
This is the simplest form of B2B collaboration. As a user in Contoso's Azure AD directory, Alice can be granted access to applications and groups through the Azure Management Portal. Now let's add Bob, who needs access to the applications Moodle and Salesforce.
...
3. Upload the .csv file through the Azure Management Portal just as it was done for Alice. Bob is now an external user in Contoso's Azure AD directory.
...
1. Use Windows PowerShell with the Azure AD Module installed to find the application IDs and group IDs within Contoso.
 ...
 - Retrieve ObjectId for groups using cmdlet `Get-MsolGroup | fl DisplayName, ObjectId` This brings up a list of all groups in Contoso and their ObjectIds. Group IDs can also be retrieved as the Object ID in the Properties tab of the group in the Azure Management Portal.  
...
3. Upload the .csv file through the Azure Management Portal.

4. Carol is a user in Contoso's directory and is also a member of the groups MyGroup1 and Externals, as seen in the Azure Management Portal.  
