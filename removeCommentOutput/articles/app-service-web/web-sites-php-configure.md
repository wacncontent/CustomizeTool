...
Instead of the default PHP runtime, Azure Websites can use a PHP runtime that you provide to execute PHP scripts. The runtime that you provide can be configured by a `php.ini` file that you also provide. To use a custom PHP runtime with web sites, follow the steps below.
...
2. Modify the `php.ini` file for your runtime. Note that any configuration settings that are system-level-only directives will be ignored by web sites. (For information about system-level-only directives, see [List of php.ini directives]).
