...
1. Sign in to the Windows Azure [Management Portal](http://manage.windowsazure.cn). Check out the [Trial](http://www.windowsazure.cn/zh-cn/pricing/1rmb-trial/) offer if you do not have a subscription yet.
...
6. The third configuration screen lets you specify attributes for **resources connected to the virtual machine** such as the cloud service and the storage account. For this tutorial, fill in the fields as shown in the image below. Once this screen is complete, click the arrow to continue. 
	...
	Note that the **Cloud Service DNS Name** is the global DNS name that becomes part of the URI that is used to contact the virtual machine. You will need to create your own cloud service name since cloud service names must be globally unique. We will use MyTestService in this tutorial. Cloud services are important for more complex scenarios using [multiple virtual machines](/documentation/articles/cloud-services-connect-virtual-machine/).
 
	For **Region/Affinity Group/Virtual Network**, we will use China East, but you can use a region that is more appropriate to your location. You can also choose to specify a virtual network instead.
 
	>[WACOM.NOTE]If you want a virtual machine to use a virtual network, you must specify the virtual network when you create the virtual machine. For more information, see [Azure Virtual Network Overview](https://msdn.microsoft.com/zh-cn/library/azure/jj156007.aspx).
...
7. The fourth configuration screen lets you configure the **VM Agent and endpoints**. For this tutorial, do not make any changes to this screen. Click the check mark to create the virtual machine.
	...
	>[WACOM.NOTE] The VM agent provides the environment for you to install extensions that can help you interact with the virtual machine. For details, see [Using Extensions](http://msdn.microsoft.com/zh-cn/library/dn606311.aspx). For details about configuring endpoints, see [How to Set Up Endpoints to a Virtual Machine](/documentation/articles/virtual-machines-set-up-endpoints/).
...
This section will show you how to attach an empty data disk to the virtual machine. See the [Attach a Data Disk Tutorial] (/documentation/articles/storage-windows-attach-disk/) for more information on attaching empty disks as well as how to attach existing disks.
...
[How to Connect Virtual Machines in a Cloud Service](/documentation/articles/cloud-services-connect-virtual-machine/)

[How to Create and Upload your own Virtual Hard Disk containing the Windows Server Operating System](/documentation/articles/virtual-machines-create-upload-vhd-windows-server/)

[Attach Data Disks to a Virtual Machine](/documentation/articles/storage-windows-attach-disk/)
